...
def build() -> tuple[str, dict]:
    ...
    if len(symbols) != 208 or len(set(symbol_ids)) != 208:
        errors.append("expected 208 unique controlled symbols")
    pin_count = sum(row["pin_count"] for row in symbols)
    if pin_count != 1518:
        errors.append(f"expected 1518 unique electrical-pad pins, got {pin_count}")
...
def main() -> int:
    ...
    if stale:
        ...
        return 1
    print("ok: 208 controlled R2 symbols, 1518 exact pad pins, 3 external-interface metadata entries")
    return 0
