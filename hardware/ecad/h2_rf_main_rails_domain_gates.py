...
def footprint_for(instance: str, device_key: str) -> str:
    ...
    if device_key.startswith(("tdk_c1005", "yageo_cc0402", "murata_grm155", "kemet_c0402")):
        return "Capacitor_SMD:C_0402_1005Metric"
    if device_key.startswith("murata_grm188"):
        return "Capacitor_SMD:C_0603_1608Metric"
    if device_key.startswith(("yageo_rc0402", "uniroyal_0402wgf", "yageo_rt0402", "vishay_tnpw0402")):
...
def reference_prefix(instance: str, device_key: str) -> str:
    ...
    if device_key.startswith(("tdk_c", "yageo_cc", "murata_grm", "kemet_c")):
        return "C"
    if device_key.startswith(("yageo_rc", "uniroyal_0402wgf", "yageo_rt", "vishay_tnpw")):
        return "R"
...
def main() -> int:
    ...
    if args.write:
        ...
        if stale:
            ...
            return 1
        print("ok: H2.3.4 main rails and domain gates sheet reproduces its historical R1 input (not current R2 evidence)")
    if args.kicad_check:
        kicad_check()
    return 0
