#!/usr/bin/env python3
"""Generate and verify the exact H2.2.8 UI-side 80-contact M1 sheet."""
...
SHEET_ID = "UI_40_INTERBOARD_M1"
PROJECT_ID = "LESHY2-UI"
PROJECT_DIR = ECAD / f"kicad/{PROJECT_ID}"
OUTPUT_SCH = PROJECT_DIR / f"{SHEET_ID}.kicad_sch"
OUTPUT_MANIFEST = ECAD / "generated/H2-UI40-interboard-m1.json"
SYMBOL_LIBRARY = ECAD / "libraries/leshy2.kicad_sym"
SYMBOL_NAMESPACE = "UI40"
FOOTPRINT = "Connector_Hirose_FX8:Hirose_FX8-80P-SV_2x40_P0.6mm"
...
def build() -> tuple[dict[Path, str], dict]:
    ...
    if len(rows) != 1 or rows[0]["instance"] != "m1_ui_plug":
        raise ValueError(f"{SHEET_ID} must own only m1_ui_plug, got {rows}")
    ...
    library, coords, _ = library_symbol(
        "m1_ui_plug", pins, "J", FOOTPRINT, rows[0]["role"],
    ...
    lines = [
        ...
        '\t\t(title "Leshy2 — exact UI-side 80-contact M1 plug")',
        '\t\t(rev "H2.2.8")', "\t)", "\t(lib_symbols", library, "\t)",
        schematic_symbol(
            "m1_ui_plug", pins, reference, device["mpn"], FOOTPRINT, rows[0]["role"],
    ...
    for source, pin in zip(pin_map, pins):
        ...
        lines += [
            ...
            f"\t\t{effects(justify)}",
            f'\t\t(uuid "{stable_uuid(f"label:m1_ui_plug:{pin.number}:{net}")}")',
            "\t)",
        ]
        contact_rows.append({
    ...
    if hierarchy_used != interfaces:
        raise ValueError(f"UI40 does not terminate hierarchy interfaces: {sorted(interfaces - hierarchy_used)}")
    if set(row["net"] for row in pin_map) != interfaces:
        raise ValueError("UI40 hierarchy differs from the exact M1 net set")
    lines += [
    ...
    manifest = {
        "schema_version": 1,
        "stage": "H2.2.8",
        "status": "reviewed_exact_ui_interboard_m1_sheet",
        ...
        "instances": [{
            "instance": "m1_ui_plug", "symbol_uuid": stable_uuid("symbol:m1_ui_plug"),
        ...
        "footprint_evidence": [{
            ...
            "fit": "official FX8/FX8C common plug land pattern: 2x40 at 0.6-mm pitch, 0.35x1.50-mm lands, asymmetric 1.1/0.7-mm locating holes",
        }],
        "corrections_closed": [
            "M1 accounting now reports the 20 POWER_GROUND contacts physically present in the 80-contact map instead of the stale value 22",
            "all repeated rails and returns remain separate physical connector pins; none is hidden behind one aggregate symbol pin",
            "the UI symbol uses all 80 numbered contacts and the exact same net, direction and signal-class records as the architecture contract",
        ],
        "review_boundary": {
            "complete": [
                "all 80 UI-plug contacts are explicit numbered symbol pins",
                "all 51 unique M1 nets terminate the exact UI hierarchy interfaces",
                "connector contact, return and rail accounting is derived and machine-checked",
                "the official Hirose FX8C plug drawing and common FX8/FX8C land pattern support the selected KiCad footprint",
                "native KiCad parses the live hierarchy with the exact M1 sheet",
            ],
            "deferred": [
                "RF-side mating receptacle sheet and cross-project contact equality in H2.3",
                "connector temperature, contact-drop, simultaneous-load and hot/unpowered boundary HIL",
                "PCB placement, fan-out, impedance, return geometry and DRC in H6",
            ],
        },
    }
    generated = {
...
def structural_check(generated: dict[Path, str], manifest: dict) -> None:
    ...
    if summary != {
        ...
        "reserved_contacts": 0, "intentional_no_connect_pins": 0,
        "pcb_files_created": 0,
    }:
        raise ValueError(f"H2.2.8 accounting drifted: {summary}")
    schematic = generated[OUTPUT_SCH]
    if schematic.count("\n\t(symbol\n") != 1:
        raise ValueError("UI40 must contain exactly one physical connector symbol")
    if schematic.count("\n\t(hierarchical_label \"") != 51:
        raise ValueError("UI40 hierarchy interface count drifted")
    contacts = manifest["contacts"]
    if [row["contact"] for row in contacts] != list(range(1, 81)):
        raise ValueError("UI40 physical contact ordering drifted")
    if len({row["symbol_pin"] for row in contacts}) != 80:
        raise ValueError("UI40 collapsed physical contacts")
...
def kicad_check() -> None:
    ...
    if result.returncode:
        raise RuntimeError(f"KiCad rejected UI40 hierarchy:\n{result.stdout}{result.stderr}")
    print("ok: KiCad parsed H2.2.8 UI-side M1 hierarchy")
...
def main() -> int:
    ...
    if args.write:
        ...
        if stale:
            ...
            return 1
        print("ok: H2.2.8 UI-side M1 sheet reproduces its historical R1 input (not current R2 evidence)")
    if args.kicad_check:
        kicad_check()
    return 0
