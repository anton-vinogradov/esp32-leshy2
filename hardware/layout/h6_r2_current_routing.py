...
def build(drc_paths: dict[str, Path] | None, existing: dict | None) -> dict:
    ...
    return {
        ...
        "board_size_review": {
            ...
            "evidence": f"all 1210 currently modeled footprints place without a same-face hard conflict; all {placement_audit['summary']['locality_pair_count']} local-owner constraints and {placement_audit['summary']['critical_pad_pair_count']} critical pad-pair limits pass; both native DRC reports are clean; the accepted 5-mm routing corridor remains usable. The separate native interface review still contains footprint, cutout and assembly-access findings; these checks do not certify the modeled geometry against every real part",
...
def doc(audit: dict, manual_copper: dict, ru: bool) -> str:
    ...
    if ru:
        ...
        notes = (
            ...
            "## Что получили\n\n"
            f"Размещены все 1 208 моделируемых посадочных мест PCB; {summary['placement_locality_pair_count']} пар local-part → owner "
            ...
            "Общие GPIO13/GPIO14 теперь образуют нативную пару `C5_USB_SDIO_COMMON_N/P` и "
            "защищены от обычной автоматической разводки. Это не меняет GPIO, функции или готовую медь. "
            "Отдельное [ревью управления мультиплексором](h6-r2-c5-mux-control-review.ru.md) обнаружило "
            "отсутствие драйвера SEL и невозможность отключить mux при OWNER=0. Исправление имён "
            "не закрывает эту схемную ошибку; режим SDIO и переходы USB/SDIO ещё не подтверждены.\n\n"
            "## Предел текущего ERC\n\n"
        ...
        notes = (
            ...
            "## What we obtained\n\n"
            f"All 1,208 modeled PCB footprints are placed; all {summary['placement_locality_pair_count']} local-part → owner pairs "
            ...
            "The shared GPIO13/GPIO14 nets now form the native `C5_USB_SDIO_COMMON_N/P` pair and "
            "are excluded from ordinary automatic routing. GPIOs, functions and existing copper are unchanged. "
            "A separate [mux-control review](h6-r2-c5-mux-control-review.md) found no SEL driver and "
            "no way to disconnect the mux while OWNER=0. The alias correction does not fix this "
            "schematic defect; SDIO operation and USB/SDIO transitions remain unverified.\n\n"
