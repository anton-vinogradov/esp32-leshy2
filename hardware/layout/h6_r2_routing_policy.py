...
def doc(audit: dict, bootstrap: dict, *, ru: bool) -> str:
    ...
    if ru:
        ...
        lead = (
            ...
            "Страница сохраняет принятый срез `GENERAL_CONTROL`, всех 12 кварцевых/генераторных сетей и "
            "`SAFETY_CONTROL` на прежней 75-мм геометрии как историческое evidence. Актуальное состояние "
            "80-мм плат публикуется в отдельном срезе H6.0.3."
        )
        ...
        notes = (
            ...
            "Автопредложения `GENERAL_CONTROL` и `SAFETY_CONTROL` после импорта исправлены и проверены в KiCad; генераторные "
            ...
            "результат к точным хешам прежних PCB и freeze всех 1 208 позиций. Это сохранённый исторический "
            "срез внутри H6.0.2, а не состояние текущих 80-мм плат. "
            "[Актуальные числа H6.0.3 и DRC](h6-r2-current-routing.ru.md).\n\n"
            "### Как сейчас выглядит реальная разводка\n\n"
            "Синим показана медь верхней стороны, красным — нижней; отверстия и переходные via видны в обоих слоях. "
            "В отличие от исторических чисел выше, картинки ниже — прямой экспорт из текущих 80-мм `.kicad_pcb`. "
            "Они содержат хеши исходных плат и автоматически считаются устаревшими после любого изменения PCB.\n\n"
            "**Передняя/UI-плата**\n\n"
        ...
        lead = (
            ...
            "This page preserves the accepted `GENERAL_CONTROL`, all 12 crystal/oscillator nets and "
            "`SAFETY_CONTROL` slice on the former 75-mm geometry as historical evidence. The live "
            "80-mm board state is published by the separate H6.0.3 checkpoint."
        )
        ...
        notes = (
            ...
            "The imported `GENERAL_CONTROL` and `SAFETY_CONTROL` proposals were repaired and reviewed in KiCad; oscillator branches "
            ...
            "results to the former PCB hashes and to the 1,208-position freeze. This is preserved historical "
            "evidence inside H6.0.2, not the state of the live 80-mm boards. See the "
            "[current H6.0.3 counts and DRC](h6-r2-current-routing.md).\n\n"
            "### What the real routing looks like now\n\n"
            "Blue is front copper and red is back copper; holes and vias appear across both layers. Unlike the "
            "historical counts above, the images below are direct exports from the live 80-mm `.kicad_pcb` files. "
            "Each image carries the source board hash and becomes stale automatically after any PCB change.\n\n"
            "**Front/UI board**\n\n"
...
def main() -> int:
    ...
    if bootstrap.get("status") != "pass":
        raise SystemExit("accepted H6.0.2 routing audit is missing or failed")
    outputs = {
