...
def render_doc(result: dict, ru: bool) -> str:
    ...
    if ru:
        ...
        next_text = "**Downstream-результат:** [`H3-R2.1`](power-dc-source-result.ru.md) отдельно показывает текущие численные и модельные ограничения. Полнота учёта нагрузок не подтверждает применимость модели питания и не закрывает H3."
        ...
        next_text = "**Downstream result:** [`H3-R2.1`](power-dc-source-result.md) reports current numerical and model-applicability findings separately. Complete load accounting does not qualify the power model or close H3."
