...
The existing domain-crossing evaluator screens a commanded state; disappearance
of two direct paths is not voltage, leakage, timing or circuit qualification.
"""
...
def overlay_metadata(ledger, original):
    return {"artifact": "C5-isolation-experimental-" + ledger,
            ...
            "experimental_origin": ORIGIN}


def templates(data, reviews):
    ...
    review = reviews.get(PARTS[BUFFER][2], {})
    expected_types = {"1": "input", "2": "open_collector", "3": "input", "4": "power_in",
                      "5": "open_collector", "6": "input", "7": "open_collector", "8": "power_in"}
    require(review.get("mpn") == PARTS[BUFFER][3] and {p: r["type"] for p, r in review.get("pins", {}).items()} == expected_types,
            "candidate requires exact noninverting open-drain pin review")
    require(data["devices"]["devices"][PARTS[BUFFER][2]]["electrical_contract"]["function"] == "three non-inverting buffers with open-drain outputs",
            "candidate polarity/function differs")
    return result


def make_candidate(data, reviews):
    seeds = templates(data, reviews)
    ...
    for ledger in ("instances", "nets"):
        ...
        result[ledger] = {**overlay_metadata(ledger, data[ledger]), "rows": result[ledger]["rows"]}
...
def validate_delta(baseline, candidate, reviews):
    seeds = templates(baseline, reviews)
    for ledger in ("instances", "nets"):
        require({k: v for k, v in candidate[ledger].items() if k != "rows"} == overlay_metadata(ledger, baseline[ledger]),
    ...
    for endpoint, (old_net, new_net) in OUTPUTS.items():
        ...
        input_contact = "1A" if endpoint == "c5.GPIO23" else "2A"
        require(after == (before - {(PROJECT, endpoint)}) | {(PROJECT, BUFFER + "." + input_contact)}, "external AON membership changed")
        external[old_net] = {"retained_original_endpoints": len(before) - 1, "added_buffer_input": BUFFER + "." + input_contact}
...
def build():
    ...
    variants, results = [], []
    for _ in range(2):
        candidate = make_candidate(data, reviews)
        delta = validate_delta(data, candidate, reviews)
        scan = crossings.evaluate(candidate, reviews, state, crossings.OFF_INPUT_PROOFS, before)
        crossings.validate_result(scan)
        variants.append(digest(candidate))
        results.append((delta, scan))
    require(variants[0] == variants[1] and results[0] == results[1], "candidate deterministic replay differs")
    require(digest(data) == original_hash, "candidate mutated baseline inputs")
    delta, scan = results[0]
    removed = {(PROJECT, "c5", name.split(".")[1]) for name in OUTPUTS}
    def key(row):
        receiver = row["receiver"]
        return receiver["project"], receiver["instance"], receiver["contact"]
    require({key(r) for r in baseline["crossings"] if key(r) in removed} == removed, "baseline C5 crossing evidence missing")
    require(scan["crossings"] == [r for r in baseline["crossings"] if key(r) not in removed]
            and scan["unknown_supply_ownership"] == baseline["unknown_supply_ownership"], "unrelated crossing result changed")
    require(snapshot() == before, "candidate source inputs changed during experiment")
    return {"status": "not_qualified", "mechanics_status": "pass", "qualified": False, "production_promoted": False,
            ...
            "deterministic_replay": {"runs": 2, "pass": True, "candidate_semantic_sha256": variants},
            "scope": "In-memory ledger topology experiment only; no native CAD generation, electrical solver or production writes",
            "selection_status": "not_selected_slew_and_output_leakage_unqualified",
            "selection_reason": "Not selected for production: input slew and powered-on output leakage/MAIN-off voltage unresolved",
...
def main():
    ...
    try:
        ...
        with path.open("x") as output:
            ...
            output.write("\n")
        print(json.dumps({"status": report["status"], "mechanics_status": report["mechanics_status"],
