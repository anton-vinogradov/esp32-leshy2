...
def render_doc(manifest: dict, russian: bool) -> str:
    ...
    if russian:
        ...
        marker = "**Статус исторической R1-цепочки:** `H3.0.2-R1` завершено и проверено; последующий маркер этой цепочки — `H3.6.1-R1`. Текущий аппаратный маркер — `H1-R2.38`."
        ...
        marker = "**Historical R1-chain status:** `H3.0.2-R1` is reviewed; the later marker in that chain is `H3.6.1-R1`. The current hardware marker is `H1-R2.38`."
