...
def render_doc(manifest: dict, russian: bool) -> str:
    if russian:
        ...
        current = "**Текущий маркер:** `H3.4.4` — [analog corners](analog-corner-result.ru.md) проверены; проверяются digital levels, pulls, reset defaults и no-back-power."
    else:
        ...
        current = "**Current marker:** `H3.4.4` — [analog corners](analog-corner-result.md) are reviewed; digital levels, pulls, reset defaults and no-back-power are being verified."
    rows = "\n".join(
