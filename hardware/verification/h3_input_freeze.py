...
def render_doc(manifest: dict, russian: bool) -> str:
    if russian:
        ...
        current = "**Текущий маркер:** `H3.3.5` — [battery analog](battery-analog-verification.ru.md) проверен; выполняется сводная проверка analog corners."
    else:
        ...
        current = "**Current marker:** `H3.3.5` — [battery analog](battery-analog-verification.md) is reviewed; analog-corner evidence is being consolidated."
    rows = "\n".join(
