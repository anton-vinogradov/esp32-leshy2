...
def render_doc(manifest: dict, russian: bool) -> str:
    if russian:
        ...
        marker = "**Статус:** `H3.0.3` завершено и проверено. Текущий точный маркер — `H3.2.1`, startup, orderly shutdown и hard `FAULT_KILL`."
        ...
        marker = "**Status:** `H3.0.3` is reviewed. The current exact marker is `H3.2.1`, startup, orderly shutdown and hard `FAULT_KILL`."
