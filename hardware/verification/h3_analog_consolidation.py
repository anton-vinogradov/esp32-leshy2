...
def build() -> tuple[dict[Path, str], dict]:
    ...
    manifest = {
        ...
        "next": {"stage": "H3.4.2", "action": "verify voltage levels, pull states, reset defaults and no-back-power across every digital interface"},
    }

    en = f"""# Consolidated analog-corner result

H3.3 is reviewed: all four leaf packages and `{total_checks}` leaf checks pass, followed by `{len(checks)}` consolidation checks. Fourteen source corrections are closed, no analytical finding remains open and the total quantity-100 BOM delta is only `{q(total_cost)} USD`. The exact current marker is `H3.4.2`.
...
`H3.3` проверено: проходят все четыре leaf-пакета, `{total_checks}` их checks и `{len(checks)}` сводных checks. Закрыты четырнадцать source-исправлений, незакрытых аналитических findings нет, суммарная дельта BOM на количестве 100 — лишь `{q(total_cost)} USD`. Точный текущий маркер — `H3.4.2`.
...
def main() -> int:
    ...
    print(f"ok: H3.3 reviewed; {manifest['consolidated']['leaf_checks']} leaf + {manifest['consolidated']['consolidation_checks']} consolidation checks, next H3.4.2")
    return 0
