...
Итоговая аналоговая конфигурация добавляет `{cost['total_delta_per_board']} USD` на устройство при количестве 100. **H3.3.2 проверено; текущий точный маркер — `H3.3.4`.**
...
The final analog configuration adds `{cost['total_delta_per_board']} USD` per unit at quantity 100. **H3.3.2 is verified; the exact current marker is `H3.3.4`.**
