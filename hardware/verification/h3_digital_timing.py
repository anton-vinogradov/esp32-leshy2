...
def build() -> tuple[dict[Path, str], dict]:
    ...
    en = f"""# Digital bandwidth, latency and timing · historical R1

`H3.4.2` is reviewed with `{len(checks)}` machine checks and no open analytical finding. The historical R1 progression marker is `H3.6.1`.
...
"""
    ru = f"""# Digital bandwidth, latency и timing · historical R1

`H3.4.2` проверено: `{len(checks)}` машинных checks, незакрытых аналитических findings нет. Исторический маркер прогресса R1 — `H3.6.1`.
...
def main() -> int:
    ...
    print(f"ok: H3.4.2 reviewed; {len(manifest['checks'])} checks, next H3.6.1")
    return 0
