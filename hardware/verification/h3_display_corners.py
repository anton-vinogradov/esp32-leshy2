...
def render_doc(manifest: dict, russian: bool) -> str:
    ...
    if russian:
        ...
        marker = f"Три замены добавляют `{cost['delta_per_board']} USD` на устройство при количестве 100. **H3.3.1 проверено; текущий точный маркер — `H3.3.4`.**"
        ...
        marker = f"The three replacements add `{cost['delta_per_board']} USD` per unit at quantity 100. **H3.3.1 is reviewed; the exact current marker is `H3.3.4`.**"
