...
def build() -> tuple[dict[Path, str], dict]:
    ...
    checks = {
        ...
        "current_r2_preorder_contract_is_fail_closed": preorder["contract_id"] == "LESHY2-PREORDER-R2" and next(row for row in preorder["gates"] if row["id"] == "P0_REQUIREMENTS_ARCHITECTURE")["status"] == "reviewed" and next(row for row in preorder["gates"] if row["id"] == "P1_CURRENT_PHYSICAL_DESIGN")["status"] == "in_progress" and all(next(row for row in preorder["gates"] if row["id"] == gate_id)["status"] == "blocked" for gate_id in ("P2_R2_PRODUCTION_SCHEMATIC", "P3_R2_VIRTUAL_ELECTRICAL", "P4_JOINED_PRE_LAYOUT_REVIEW", "P5_EXACT_PRODUCTION_SOURCING", "P6_ROUTED_PRODUCTION_PACKAGE", "P7_FIRST_SPIN_DIAGNOSTIC")),
        "orders_layout_and_fabrication_remain_blocked": plan["authorization"] == {"joined_read_only_review": True, "component_purchase": False, "pcb_placement_and_routing": False, "fabrication": False} and next(row for row in preorder["gates"] if row["id"] == "P8_IMMUTABLE_EXACT_ONE_RELEASE")["status"] == "not_authorized" and preorder["procurement_boundary"]["assembled_device_quantity"] == 1,
