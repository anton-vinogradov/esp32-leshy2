...
def render_doc(manifest: dict, russian: bool) -> str:
    ...
    if russian:
        ...
        end = f"**Результат:** `{s['passed_scenarios']}/{s['scenarios']}` сценариев и `{s['topology_endpoints']}` endpoint-проверок проходят. H3-R2.3 и [цифровая проверка H3-R2.4](digital-electrical-verification.ru.md) проведены ревью; **текущий маркер — `H3-R2.5`**. Заказ и трассировка всё ещё запрещены.\n\n[Машинный отчёт](../hardware/verification/generated/H3-R2-transition-sequences.json)."
    else:
        ...
        residual = "\n".join(f"- {row}" for row in manifest["physical_residuals"])
        end = f"**Result:** `{s['passed_scenarios']}/{s['scenarios']}` scenarios and `{s['topology_endpoints']}` endpoint checks pass. H3-R2.3 and [H3-R2.4 digital verification](digital-electrical-verification.md) are reviewed; the **current marker is `H3-R2.5`**. Ordering and routing remain forbidden.\n\n[Machine report](../hardware/verification/generated/H3-R2-transition-sequences.json)."
