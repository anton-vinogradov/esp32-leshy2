...
def build() -> tuple[dict[Path, str], dict]:
    ...
    required_routes = {
        ...
        "voice_direct_to_sma": ("voice.ANT", "voice_external_sma.RF", "VOICE_EXTERNAL_RF_50R"),
    ...
    paths.extend([
        ...
        {"id": "VOICE-V/U", "mode": "50_ohm_tx_rx", "band_mhz": [136, 470], "connector": "RFPC-SMA31-FN-175-A SMA", "ideal_0p17pf_esd_loss_db_at_470mhz": q(voice_ideal_esd_loss_db, "0.000001"), "complete_feed_loss_target_db_max": "0.750", "return_loss_target_db_min": "10.000"},
    ...
    checks = {
        ...
        **route_checks,
        ...
        "antenna_policy_has_exactly_nine_onboard_ports": candidate["antenna_policy"]["base_onboard_sma_count"] == 9 and len(candidate["antenna_policy"]["base_onboard_sma_paths"]) == 9,
        "all_nine_paths_are_instantiated_once": len(paths) == 9 and {row["id"] for row in paths} == set(candidate["antenna_policy"]["base_onboard_sma_paths"]),
        "seven_tx_capable_and_two_receive_only_paths": sum("tx_rx" in row["mode"] for row in paths) == 7 and sum(row["id"].startswith("RX-") for row in paths) == 2,
    ...
    residual = [
        ...
        "measure SA518 VHF/UHF feed insertion/return loss, output power and harmonics at both power settings",
    ...
    manifest = {
        ...
        "provenance": SOURCES,
        "summary": {"external_ports": 9, "tx_capable_paths": 7, "receive_only_paths": 2, "generic_50_ohm_paths": 7, "matched_or_special_receive_paths": 2},
        ...
        "shared_constraints": {
            ...
            "matching_rule": "reserve tuneable 0402 positions where a discrete match exists; do not add an unproven match to module-native 50-ohm S3/C5/nRF/SA518 paths",
        },
        "derived": {
            ...
            "voice_ideal_esd_loss_db_at_470mhz": q(voice_ideal_esd_loss_db, "0.000001"),
...
| VOICE-V/U | native 50-омный ANT SA518 -> короткая защищённая трасса -> SMA | <=0,75 дБ и return loss >=10 дБ в диапазонах 136-174 и 400-470 МГц |
