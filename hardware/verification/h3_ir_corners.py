...
Исправления добавляют всего `{cost['total_delta_per_board']} USD` на устройство при количестве 100. **H3.3.3 проверено; текущий точный маркер — `H3.3.4`.**
...
The corrections add only `{cost['total_delta_per_board']} USD` per unit at quantity 100. **H3.3.3 is reviewed; the exact current marker is `H3.3.4`.**
