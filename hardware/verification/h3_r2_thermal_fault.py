...
def render_doc(result: dict, russian: bool) -> str:
    ...
    if russian:
        ...
        intro = f"`H3-R2.6` проведён ревью: **{s['checks']} checks**, `{s['thermal_profiles']}` thermal-профилей и `{s['single_fault_cases']}` single-fault сценариев проходят без открытых аналитических findings. Итог H3-R2.7 также проведён ревью; текущий маркер — `H4-R2.0.1`."
        thermal = ("## Тепло\n\n"
        ...
        intro = f"`H3-R2.6` is reviewed: **{s['checks']} checks**, `{s['thermal_profiles']}` thermal profiles and `{s['single_fault_cases']}` single-fault cases pass with no open analytical finding. H3-R2.7 is also reviewed; the current marker is `H4-R2.0.1`."
