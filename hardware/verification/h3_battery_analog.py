...
Open/short/swapped/reversed/missing/imbalanced cell states use a current-limited cell simulator; temperature faults and thresholds use an NTC fixture. Real cells remain inside the exact MPN voltage, current and temperature limits. Hobby-grade mechanical verification uses the one assembled prototype: geometry/DRC, enclosure load-path review, manufacturer-sourced static limits where available, ordinary assembly/disassembly, continuity and visual inspection. It requires no drop, vibration, prescribed cycle count or consumed assembled device. A 24/48-hour powered run is ordinary non-destructive qualification. The generated evidence is [`H3-VRF34-battery-analog.json`](../hardware/verification/generated/H3-VRF34-battery-analog.json).
"""
...
Open/short/swapped/reversed/missing/imbalanced состояния банков задаёт current-limited cell simulator, температурные faults и thresholds — NTC fixture. Реальные банки остаются внутри ограничений точного MPN по напряжению, току и температуре. Hobby-grade механическая проверка использует единственный собранный прототип: geometry/DRC, анализ load path корпуса, статические пределы из источников при их наличии, обычную сборку/разборку, continuity и визуальный осмотр. Падения, vibration, заданное число циклов и расходование собранных устройств не требуются. 24/48-часовой прогон под питанием — обычная неразрушающая qualification. Машинный результат: [`H3-VRF34-battery-analog.json`](../hardware/verification/generated/H3-VRF34-battery-analog.json).
"""
