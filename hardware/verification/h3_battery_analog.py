...
H3.3.4 is reviewed with `{len(checks)}` machine checks and four source corrections. No component or BOM-cost change is required. The exact current marker is `H3.4.2`: digital levels/defaults and no-back-power.
...
`H3.3.4` проверено: `{len(checks)}` машинных проверок и четыре исправления по первичным источникам. Компоненты и стоимость BOM не изменились. Точный текущий маркер — `H3.4.2`, digital levels/defaults и no-back-power.
