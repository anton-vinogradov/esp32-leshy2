...
H3.3.4 is reviewed with `{len(checks)}` machine checks and four source corrections. No component or BOM-cost change is required. The exact current marker is `H3.3.5`: analog-corner consolidation.
...
`H3.3.4` проверено: `{len(checks)}` машинных проверок и четыре исправления по первичным источникам. Компоненты и стоимость BOM не изменились. Точный текущий маркер — `H3.3.5`, сводная проверка analog corners.
