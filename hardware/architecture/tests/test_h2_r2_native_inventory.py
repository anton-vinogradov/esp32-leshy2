...
class H2R2NativeInventoryTests(unittest.TestCase):
    ...
    def test_native_project_graph_has_no_historical_cap_board(self):
        ...
        self.assertEqual(22, self.actual["summary"]["sheet_count"])
        self.assertNotIn(
            "LESHY2-LORA-CAP-01",
            {row["id"] for row in self.actual["projects"]},
        )
    ...
    def test_exact_component_group_and_pack_delta_are_frozen(self):
        self.assertEqual(252, self.actual["summary"]["component_group_count"])
        self.assertEqual(1220, self.actual["summary"]["component_quantity_per_product"])
    ...
    def test_cli_check_is_reproducible(self):
        ...
        self.assertIn("252 exact component groups", result.stdout)
