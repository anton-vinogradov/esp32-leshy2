...
class PreorderGateTests(unittest.TestCase):
    ...
    def test_gate_reports_actual_unfinished_state(self):
        ...
        self.assertIn("scoped C5 TS3/LV control ECO", truth["current_ecad"])
        self.assertIn("1210 schematic instances", truth["current_ecad"])
        self.assertIn("4321 physical pins", truth["current_ecad"])
        self.assertIn("789 canonical nets", truth["current_ecad"])
        self.assertIn("Typed native ERC retains 24 openly triaged findings", truth["current_ecad"])
        self.assertIn("not completed routing or electrical qualification", truth["current_ecad"])
        self.assertIn("173 controller pins", truth["current_ecad"])
        self.assertIn("H6 continues on 80 x 150-mm boards", truth["current_ecad"])
        ...
        self.assertIn("H6 routed release", truth["joined_release"])
        self.assertIn("H5 reconciles 250 purchasable groups / 1218 articles", truth["joined_release"])
        self.assertIn("zero-stock Pre-order MOQ21", truth["joined_release"])
        self.assertIn("252 exact groups and 1210 schematic instances", truth["virtual_electrical"])
        self.assertIn("Current H3 power applicability", truth["virtual_electrical"])
        self.assertIn("No regenerated diagnostic authorizes an order", truth["virtual_electrical"])
