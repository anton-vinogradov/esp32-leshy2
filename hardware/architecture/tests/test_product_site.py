...
class ProductSiteTests(unittest.TestCase):
    ...
    def test_roadmap_reports_current_truth_and_complete_route(self):
        pages = {
            "docs/roadmap.md": (
                "Current hardware boundary: `H1-R2.26`", "H0 is reviewed",
                "firmware F1-R2 reviewed", "F2-R2.0",
                "H9 · Manufacturing release", "Production ECAD",
            ),
            "docs/roadmap.ru.md": (
                "Текущая аппаратная граница: `H1-R2.26`", "H0 проведено ревью",
        ...
        landing_pages = {
            "README.md": ("Roadmap and current position", "Current hardware marker: `H1-R2.26`", "fabrication"),
            "README.ru.md": ("Роадмап и текущее положение", "Текущий маркер железа: `H1-R2.26`", "печати прототипа"),
        }
    ...
    def test_h5_0_3_pcba_platform_baseline_is_complete_and_honest(self):
        ...
        if plan["current_substep"] in {"H5.0.1-R1", "H5.0.2-R1"}:
            ...
            self.assertEqual(210, len(self.read("hardware/architecture/generated/G2F-3I-target-bom.csv").splitlines()) - 1)
            self.assertFalse(plan["authorization"]["sample_or_component_purchase"])
            return
        subprocess.run(
        ...
        self.assertEqual(13, evidence["summary"]["critical_lines_spot_checked"])
        self.assertEqual(8, evidence["summary"]["public_stock_exact_or_revision_explicit"])
        ...
        self.assertEqual(145, evidence["summary"]["current_public_stock_lines_before_outlier_resolution"])
        self.assertEqual(33, evidence["summary"]["current_preorder_lines_before_outlier_resolution"])
        ...
        self.assertEqual(
            {"J0": 157, "J1": 0, "J2": 37, "J3": 11, "J4-F": 3, "J4-P": 1, "J5-U": 1},
    ...
    def test_current_hardware_substep_is_visible_and_synchronized(self):
        ...
        for name in pages:
            ...
            self.assertIn("H1-R2.26", page, name)
    ...
    def test_current_mockup_review_scope_is_explicit(self):
        expectations = {
            "docs/roadmap.md": (
                "H1-R2.26", "Functional-island placement", "RF and antenna locality",
            ...
            "docs/roadmap.ru.md": (
                "H1-R2.26", "Размещение функциональных островов", "Локальность RF и антенн",
    ...
    def test_h3_plan_and_accepted_input_freeze_are_current(self):
        ...
        self.assertEqual("H1-R2.26", state["current_substep"])
        ...
        self.assertEqual(73, inventory["summary"]["used_types_with_structured_electrical_contract"])
        self.assertEqual(145, inventory["summary"]["used_types_requiring_parameter_extraction"])
    ...
    def test_external_face_acceptance_package_is_complete(self):
        ...
        self.assertEqual(6, len(package["rear"]["antenna_ports"]))
    ...
    def test_cross_view_acceptance_reconciles_physical_and_pin_fit(self):
        ...
        self.assertEqual(143, physical["inner_body_count"])
        self.assertEqual(145, physical["total_inner_component_count_including_adapter"])
    ...
    def test_internal_layout_is_dimensioned_and_separates_devices(self):
        ...
        for token in (
            ...
            "opposing inner faces: 38 non-mating XY pairs checked; minimum Z gap 3.31 mm",
        ...
        self.assertEqual(143, audit["inner_body_count"])
        self.assertEqual(145, audit["total_inner_component_count_including_adapter"])
        ...
        self.assertEqual(143, len(audit["individual_body_clearances"]))
        self.assertTrue(
            all(
        ...
        self.assertEqual(38, audit["opposing_non_mating_pair_count"])
        self.assertEqual(3.31, audit["minimum_opposing_pair"]["remaining_z_clearance_mm"])
        self.assertEqual(38, len(audit["opposing_non_mating_pairs"]))
        self.assertTrue(
            all(
    ...
    def test_top_edge_view_has_true_axes_and_both_antenna_banks(self):
        ...
        for token in (
            ...
            'id="rear-antenna-bank" data-count="6"',
