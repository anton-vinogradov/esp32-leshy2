...
class H6R2RoutingPolicyTests(unittest.TestCase):
    ...
    def test_live_80mm_routing_checkpoint_is_hash_bound_and_non_closing(self):
        ...
        self.assertEqual(237, audit["summary"]["track_via_item_count"])
        self.assertEqual(57, audit["summary"]["resolved_connection_count"])
        self.assertEqual(3208, audit["summary"]["current_total_unconnected_count"])
        self.assertEqual(260, audit["summary"]["analog_remaining_connection_count"])
    ...
    def test_reviewed_manual_power_routes_are_reproducible_and_drc_bound(self):
        ...
        self.assertEqual(40, audit["summary"]["route_count"])
        self.assertEqual(186, audit["summary"]["segment_count"])
        self.assertEqual(57, audit["summary"]["resolved_connection_count"])
        self.assertEqual(51, audit["summary"]["via_count"])
        self.assertEqual(28, audit["summary"]["manual_only_route_count"])
        self.assertEqual(12, audit["summary"]["local_ground_join_route_count"])
        self.assertTrue(
        ...
        if KICAD_PYTHON.is_file():
            ...
            self.assertEqual(0, result.returncode, result.stdout)
            self.assertIn("40 routes; 186 segments; 57 resolved connections", result.stdout)
    ...
    def test_native_placement_and_routing_guards_are_current(self):
        ...
        for script, expected in (
            ...
            (CURRENT_ROUTING_SCRIPT, "237 copper items; 57 resolved; 3208 remain"),
        ):
            result = subprocess.run(
