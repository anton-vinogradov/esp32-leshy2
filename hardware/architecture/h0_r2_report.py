...
def render_svg(data: dict) -> str:
    ...
    out = [
        ...
        '<text x="630" y="66" text-anchor="middle" font-family="sans-serif" font-size="12" fill="#526076">Reviewed H1-R2.37 projection; current H2-R2.0.2 closes the service-VBUS detector/latch before native R2 ECAD.</text>',
...
def render_report(data: dict, ru: bool) -> str:
    ...
    if ru:
        ...
        current = "Текущий точный маркер — **H2-R2.0.2**. Физическая проекция H1-R2.37 с двумя независимыми RP2354B, точными GPIO0..47, четырьмя M1 endpoint-группами и 226 корпусами принята и прошла ревью 2026-08-30. В неё входят все восемь TX-детекторов, пять coupler и восемь локальных evidence-островов; шесть AD8314 используют принятый `AD8314ARMZ-REEL` / `C652687`. Серийный дисплей EastRising, U219, NFC-loop и swept volume антенны закрыты. Старая single-RP G2F/H2-проекция остаётся только историческим R1 evidence. Live route C11355 прошёл ревью как H2-R2.0.1; H2-R2.0.2 закрывает точный MPN detector/latch service-VBUS перед powered-off-Ioff границей Pack/Safety I2C и native R2 ECAD/KiCad."
        sections = {
