...
def render_report(data: dict, ru: bool) -> str:
    ...
    if ru:
        ...
        current = "Текущий точный маркер — **H2-R2.1.3**. Физическая проекция H1-R2.37 с двумя независимыми RP2354B, точными GPIO0..47, четырьмя M1 endpoint-группами и 226 корпусами принята и прошла ревью 2026-08-30. В неё входят все восемь TX-детекторов, пять coupler и восемь локальных evidence-островов; шесть AD8314 используют принятый `AD8314ARMZ-REEL` / `C652687`. Серийный дисплей EastRising, U219, NFC-loop и swept volume антенны закрыты. Старая single-RP G2F/H2-проекция остаётся только историческим R1 evidence. Live route C11355 прошёл ревью как H2-R2.0.1; точный detector/latch/release service-VBUS — как H2-R2.0.2; TCA9803DGKR/C2687966 Pack/Safety powered-off boundary — как H2-R2.0.3. Native R2 inventory из 3 проектов, 23 sheets и 213 exact MPN-групп прошёл ревью как H2-R2.1.1; exact ledger для 208 board groups, пяти явных non-PCBA groups и 1 555 логических контактов прошёл ревью как H2-R2.1.2. Сейчас материализуются controlled definitions и joined native nets до export/KiCad placement."
        sections = {
