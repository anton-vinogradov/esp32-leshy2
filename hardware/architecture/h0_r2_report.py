...
def render_report(data: dict, ru: bool) -> str:
    ...
    if ru:
        ...
        current = "Текущий точный маркер — **H3-R2.7**. Физическая проекция H1-R2.37 с двумя независимыми RP2354B, точными GPIO0..47, M1 и 226 корпусами принята и прошла ревью 2026-08-30. Native R2 H2 материализует 1 185 экземпляров и 823 nets в трёх проектах KiCad без замечаний ERC. H3-R2.0 фиксирует входы, provenance и методы; H3-R2.1–R2.6 проведены ревью. H3-R2.6 сводит 56 thermal-профилей, 30 single-fault сценариев и unattended-policy без обещания автономности. H3-R2.7 выполняет итоговый cross-check и назначает физические остатки H6/H8."
        sections = {
        ...
        current = "The exact current marker is **H3-R2.7**. The H1-R2.37 physical projection with two independent RP2354B domains, exact GPIO0..47 maps, M1 and 226 bodies was accepted and reviewed on 2026-08-30. Native R2 H2 materializes 1,185 instances and 823 nets in three KiCad projects with zero ERC findings. H3-R2.0 freezes inputs, provenance and methods; H3-R2.1 through R2.6 are reviewed. H3-R2.6 joins 56 thermal profiles, 30 single-fault cases and the extended-operation policy without an autonomy claim. H3-R2.7 performs the final cross-check and assigns physical residuals to H6/H8."
        sections = {
