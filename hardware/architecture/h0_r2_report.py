...
def render_svg(data: dict) -> str:
    ...
    out = [
        ...
        '<text x="630" y="66" text-anchor="middle" font-family="sans-serif" font-size="12" fill="#526076">Current H1-R2.22 projection: direct i8080 display, camera RX and radio payload remain local; M1 is fully counted.</text>',
...
def render_report(data: dict, ru: bool) -> str:
    ...
    if ru:
        ...
        current = "Текущий точный маркер — **H1-R2.22**: физическая модель содержит двойную post-PCBA-зону K331/AWM666V, i8080-8, локальные острова 5+5 и полную 80-контактную M1; инженерных блокеров H1 не осталось, но полный мокап ещё должен быть явно принят и не разрешает KiCad routing или заказ R2."
        sections = {
