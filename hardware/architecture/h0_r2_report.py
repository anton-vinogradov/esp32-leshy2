...
def render_svg(data: dict) -> str:
    ...
    out = [
        ...
        '<text x="630" y="66" text-anchor="middle" font-family="sans-serif" font-size="12" fill="#526076">Current H1-R2.16 projection: radio payload, display traffic and the 11-line video bus never compete on M1.</text>',
...
def render_report(data: dict, ru: bool) -> str:
    ...
    if ru:
        ...
        current = "Текущий точный маркер — **H1-R2.16**: физическая модель уже пересчитана под локальные функциональные острова 5+5, но H1 ещё не завершён и не разрешает KiCad routing или заказ R2."
        sections = {
        ...
        current = "The exact current marker is **H1-R2.16**: the physical model is repartitioned into local 5+5 functional islands, but H1 remains open and does not authorize R2 KiCad routing or ordering."
        sections = {
