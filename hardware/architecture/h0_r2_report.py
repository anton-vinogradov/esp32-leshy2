...
def render_svg(data: dict) -> str:
    ...
    out = [
        ...
        '<text x="630" y="66" text-anchor="middle" font-family="sans-serif" font-size="12" fill="#526076">Reviewed H1-R2.37 projection; current H2-R2.0.3 closes the Pack/Safety powered-off-Ioff boundary before native R2 ECAD.</text>',
...
def render_report(data: dict, ru: bool) -> str:
    ...
    if ru:
        ...
        current = "The exact current marker is **H2-R2.0.3**. The H1-R2.37 physical projection with two independent RP2354B domains, exact GPIO0..47 maps, four M1 endpoint groups and 226 bodies was accepted and reviewed on 2026-08-30. It includes all eight TX detectors, five couplers and eight local evidence islands; all six AD8314 positions use the accepted `AD8314ARMZ-REEL` / `C652687`. The serial EastRising display, U219, NFC loop and antenna swept volume are closed. The old single-RP G2F/H2 projection remains historical R1 evidence only. The live C11355 route was reviewed as H2-R2.0.1 and the exact service-VBUS detector/latch/release implementation as H2-R2.0.2. H2-R2.0.3 closes the Pack/Safety I2C powered-off-Ioff boundary before native R2 ECAD/KiCad."
        sections = {
