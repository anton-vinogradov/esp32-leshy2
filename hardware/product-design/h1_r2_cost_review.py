...
LANES_RU = {
    ...
    "main-rf-mechanics": (
        "Сохранить GCT-пару после провала полной проверки дешёвой сквозной пары в геометрии 5+5",
        "Десять GCT RFPC-SMA31/32 стоят $24,65 на устройство. Точные чертежи закрыли привлекательную альтернативу DreamLNK: четыре внешних корпуса конфликтуют с keep-out головок принятых верхних compression-stop, пять групп портов — с телами или резервами на внутренних сторонах, а фабричный маршрут остаётся manualWeld без цены для ровно одного устройства.",
...
def render_doc(result: dict, ru: bool) -> str:
    ...
    if ru:
        lines += [
            ...
            f'- Отдельная плата-адаптер и оба DF40 удалены: высота стека снижается с `{display["paper_fit"]["removed_stack_height_mm"]:.1f}` до `{display["paper_fit"]["direct_zif_height_mm"]:.1f} мм`, а цена компонентов одного прототипа — на `${-display["paper_fit"]["bom_change_usd"]:.2f}`. Панель держит один готовый 3M (TC) `4910SQ-2(5)` 50,8×50,8-мм PSA-квадрат; контакты ZIF нагрузки не несут. Его условные `$22.12` не добавлены в замороженный BOM до проверки стека ≤0,714 мм и принятия фабрикой.',
        ]
    else:
        lines += [
            ...
            f'- The adapter PCB and both DF40 parts are removed: stack height falls from `{display["paper_fit"]["removed_stack_height_mm"]:.1f}` to `{display["paper_fit"]["direct_zif_height_mm"]:.1f} mm`, and one-prototype component cost falls by `${-display["paper_fit"]["bom_change_usd"]:.2f}`. One ready-made 3M (TC) `4910SQ-2(5)` 50.8×50.8-mm PSA square carries the panel; ZIF contacts carry no mechanical load. Its provisional `$22.12` is not added to the frozen BOM until the ≤0.714-mm stack and factory-acceptance gates pass.',
        ]
    footer = (
