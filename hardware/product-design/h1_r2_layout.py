...
PUBLIC_ASSET_REV = "h1-r2.38-one-piece-psa-1"
...
def render_doc(model: dict, result: dict, ru: bool) -> str:
    ...
    if ru:
        ...
        bullets = [
            ...
            "Панель приклеивается одной цельной вырубной деталью из двусторонней акриловой foam-ленты, которая сначала устанавливается на PCB по собственной шелкографической рамке. Сплошное нижнее поле 54,54×58,46 мм и два соединённых с ним боковых плеча поддерживают около 69,8% площади экрана и верхние углы без отдельной верхней полоски. После прохода хвоста через щель и снятия верхнего лайнера панель ставится по второй рамке DISPLAY 56,54×84,96 мм с меткой FPC↑. Широкая компонентная часть FPC остаётся в свободном от клея кармане; только хвост 25,50±0,15 мм проходит через скруглённую щель PCB 27,00×1,20 мм к внутреннему ZIF в позиции [24,0; 36,0]. Точная толщина проставки должна превышать измеренный максимум сложенного FPC минимум на 0,20 мм; показанный 1,00-мм габарит пока не является выбранным материалом. Пять U.FL заканчиваются на y=17,1 мм, щель начинается на y=34,0 мм, поэтому между ними остаётся 16,9 мм. FPC и ZIF нагрузки панели не несут.",
        ...
        bullets = [
            ...
            "The panel bonds through one carrier-backed die-cut piece of double-coated acrylic foam that is first located on the PCB by its own silkscreen frame. A solid 54.54×58.46-mm lower field and two connected side shoulders support about 69.8% of the panel plan and both upper corners without a separate top tape strip. After the tongue crosses the slot and the upper liner is removed, a second DISPLAY 56.54×84.96-mm frame with an FPC-UP mark locates the panel. The wide component-bearing FPC remains in an adhesive-free pocket; only its 25.50±0.15-mm tongue crosses one rounded 27.00×1.20-mm PCB slot into the inner ZIF at [24.0, 36.0]. Released spacer thickness must exceed the measured folded-FPC maximum stack by at least 0.20 mm; the drawn 1.00-mm envelope is not yet a selected material. All five U.FL bodies end at y=17.1 mm and the slot begins at y=34.0 mm, leaving 16.9 mm between them. Neither the FPC nor ZIF carries panel load.",
