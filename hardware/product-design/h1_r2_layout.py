...
PUBLIC_ASSET_REV = "h1-r2.38-stock-psa-rectangle-1"
...
def render_doc(model: dict, result: dict, ru: bool) -> str:
    ...
    if ru:
        ...
        bullets = [
            ...
            "The panel bonds through one ready-stock 50.00×50.00-mm rectangle of double-coated acrylic foam that is first located on the PCB by its own silkscreen frame at [12.50, 44.46]. It supports 2500 mm², or about 52.0% of the stiff panel plan; a custom die-cut, side shoulders, upper tape strip and separate clamp are unnecessary. After the tongue crosses the slot and the upper liner is removed, a second DISPLAY 56.54×84.96-mm frame with an FPC-UP mark locates the panel. The upper FPC zone remains adhesive-free; only its 25.50±0.15-mm tongue crosses one rounded 27.00×1.20-mm PCB slot into the inner ZIF at [24.0, 36.0]. Released stock-pad thickness must exceed the measured folded-FPC maximum stack by at least 0.20 mm; the drawn 1.00-mm envelope is not yet a selected MPN. All five U.FL bodies end at y=17.1 mm and the slot begins at y=34.0 mm, leaving 16.9 mm between them. Neither the FPC nor ZIF carries panel load. A custom contour is allowed only if the PCBA factory itself confirms manufacture and installation inside the order; no external converter is required.",
