...
**Mounting: nine board-mounted vertical SMA jacks along the top edge — every antenna is removable** (screw on a whip, telescopic or wire). Bare-chip radios (CC1101, SA868) route their 50 Ω straight to the SMA; module radios (3× nRF24, E22 LoRa, S3, C5) reach theirs by a short internal **u.FL → SMA jumper** (the module carries the u.FL). Si4732 gets a removable SMA too — a long **telescopic** for HF/CB, RX-only. **Isolation is by placement:** the three parallel-scanning nRF24 sit at the ends + centre of the 80 mm top (~40 mm apart) with frequency-diverse radios (Si4732, SA868, CC1101, LoRa) as spacers between; the 2 W SA868 is at one end, away from GPS. External whips radiate above the ground plane, so ~40 mm base spacing is enough for the scan-RX. GPS keeps its own patch on the u-blox module. All matching is tuned by hand on a VNA.
