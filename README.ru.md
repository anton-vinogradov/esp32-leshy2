...
**Точный маркер: `H2.3.2`** — реализовать и проверить USB-PD input и зарядку
двух элементов на `RF_01_USB_PD_CHARGE`.
...
- ▶️ `H2.3` — реализовать и проверить листы RF/power PCB.
  - ✅ `H2.3.1` — проверен `RF_00_ROOT`: 12 дочерних листов, 134 точные
    межлистовые цепи и 303 явных pins/labels; все 303 текущие ERC-находки точно
    соответствуют пока пустым функциональным child-листам.
  - ▶️ **`H2.3.2` — сейчас:** USB-PD input и зарядка двух элементов.
  - ⏳ `H2.3.3` — допуск pack и always-on safety.
