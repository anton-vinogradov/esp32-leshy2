...
**Точный маркер: `H1.1.2`** — нормализовать размеры производителя, начала
координат и внешние направления всех корпусов, используемых мокапом. Уже
отрисованные виды остаются черновыми выходами и не завершают поздние подэтапы.
...
- `H1.1` — собрать реестр физических первоисточников.
  ...
  - ▶️ **`H1.1.2` — сейчас:** сверить размеры, origin, ориентацию и направление
    connector/actuator с evidence производителя.
  - ⏳ `H1.1.3` — классифицировать каждую оставшуюся неопределённость как
    blocker H1 или gate проверки полученного образца H5.
  - ⏳ `H1.1.4` — зафиксировать таблицу размеров и источников для renderer.
- ⏳ `H1.2` — создать единую систему координат двух плат и корпуса.
- ⏳ `H1.3` — закрыть внешние стороны, органы управления, стрелки и читаемую
  шелкографию.
- ⏳ `H1.4` — закрыть зеркальные внутренние стороны и межплатный stack.
- ⏳ `H1.5` — закрыть настоящий вид от антенного торца, разрезы, U214 и доступ
  к батареям.
- ⏳ `H1.6` — пройти проверки коллизий, зазоров, видимости и service access.
- ⏳ `H1.7` — повторить pin/resource fit и собрать acceptance package.
- ⏳ `H1.8` — получить явную пользовательскую приёмку целостного мокапа.

`H1.1.2` завершается, когда у каждого отрисованного корпуса есть sourced
`L×W×H`, datum, ориентация и направление интерфейса либо явный, невыдуманный
sample gate. После закрытия любой подзадачи этот маркер и обе страницы
роадмапа обновляются тем же commit до перехода дальше.
