...
**Точный маркер: `H2.3.3`** — реализовать и проверить допуск аккумуляторного
блока, always-on safety и независимый надзор на `RF_02_PACK_SAFETY_AON`.
...
- ▶️ `H2.3` — реализовать и проверить листы RF/power PCB.
  - ✅ `H2.3.1` — проверен `RF_00_ROOT`: 12 дочерних листов, 133 точные
    межлистовые цепи и 301 явный pin/label; текущие ERC-находки точно
  ...
  - ▶️ **`H2.3.3` — сейчас:** допуск pack и always-on safety.
  - ⏳ `H2.3.4` — основные rails и отключаемые домены.
