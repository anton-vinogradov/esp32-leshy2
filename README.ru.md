...
> **Статус проекта: R1 — дизайн и механика устройства.** Текущий мокап не
> принят; production ECAD, PCB routing и закупка заблокированы. Полный путь и
> текущая позиция показаны в [роадмапе до готового продукта](docs/roadmap.ru.md).
...
|---|---|---|
| R0 · Требования и архитектура | ✅ Проведено ревью | возможности, пять доменов, владельцы, шины и pin/resource budget |
| **R1 · Дизайн и механика** | **▶️ Сейчас** | принятые внешние и внутренние стороны, вид сверху, разрезы и порядок сборки |
| R2 · Production ECAD-схема | ⏳ Ожидает R1 | точная схема, footprints, номиналы, net map и чистый ERC |
| R3 · Виртуальная электрическая проверка | ⏳ Ожидает R2 | power, transient, thermal, timing, RF и fault evidence |
| R4 · Переносимая исполняемая модель | ✅ Проведено ревью | 24 host-сценария и чистые ASan/UBSan |
| R5 · Target-сборки и эмуляция | ⏳ Ожидает R2; использует R4 | пять target-образов, size gates, S3 QEMU и dev-board matrix |
| R6 · Объединённое pre-layout ревью | 🔒 Заблокировано R1–R5 | закрытые виртуальные blockers и названные физические неопределённости |
| R7 · Образцы компонентов | 🔒 Заблокировано R6 и одобрением стоимости | MPN, размеры и первые HIL-свидетельства полученных деталей |
| R8 · PCB placement и routing | 🔒 Заблокировано R7 | проверенная разводка двух плат, DRC и manufacturing package |
| R9 · Печать прототипа и bring-up | 🔒 Заблокировано R8 и одобрением заказа | платы-прототипы, rails, boot/recovery и smoke tests интерфейсов |
| R10 · Физическая квалификация | 🔒 Заблокировано R9 | RF, power, thermal, safety, endurance и полный HIL 3×nRF24 |
| R11 · Полная прошивка продукта | ⏳ Стартует после R5; закрывается после R9/R10 | функции пяти target, UI, подписанный update, recovery и уровни безопасности |
| R12 · Release и производство | 🔒 Заблокировано R10/R11 | воспроизводимый BOM/fab/test/firmware/site release без blocker |

**Мы на R1.** Актуальной production-схемы, PCB layout и target-прогонов в
...
![Внешние стороны Leshy2](docs/images/current-clamshell.svg?layout=14)
