...
> **Сейчас: H1-R2.6 · доказаны точное положение MMCX и зоны бокового подключения.**
...
> физическую проекцию. K331 вписывается в зарезервированные GPIO и питание; его
...
> обязательными у последующих владельцев H3/H5/H6/H8. MMCX исправлен: вместо целиком вынесенного за плату
...
> 3,75 А continuous / 4,25 А step; оставшаяся компоновка, схемы и firmware-контракты
...
Компактная схема ниже — текущая R2-архитектура. Следующий чертёж H1-R2.6 —
...
![Внутреннее размещение Leshy2 H1-R2.6](docs/images/h1-r2-inner-placement.svg)

![Положение и зоны подключения MMCX Leshy2 H1-R2.6](docs/images/h1-r2-mmcx-service.svg)
...
![Тракт аналогового FPV Leshy2 H1-R2.6](docs/images/h1-r2-fpv-path.svg)
...
| **H1 · Физический дизайн устройства** | **▶️ Сейчас: H1-R2.6, механика MMCX и внутреннее размещение** | [Текущий результат](docs/h1-r2-physical-layout.ru.md) |
...
**Железо находится на H1-R2.6.** H0-R2 фиксирует шесть вычислительных доменов,
...
поверхности. Компактный генерируемый LC-кандидат Airband проходит номинальную
...
коллизий корпусов. `AKK K331` проходит проверку зарезервированных GPIO Hub и
5-В бюджета, а `TBS5G8MMCXA` стала точной 13-й антенной комплекта для
...
ручной установки. Точные запросы механических/assembly-данных и sourcing-маршрута
отправлены AKK и JLCPCB 27 августа 2026 года; ожидаемые ответы не закрывают ни
...
удержание и strain остаются evidence H5. H3 ещё доказывает эффективную ёмкость, load-step, switching loss и
тепло корпуса. H1 должен пересобрать полные физические виды. Заказ не разрешён.

<!-- current-substep: H1-R2.6 -->

**Точный маркер: `H1-R2.6`** — [текущее размещение](docs/h1-r2-physical-layout.ru.md),
[результат питания](docs/h1-r2-power-thermal.ru.md) и [результат FPV](docs/h1-r2-fpv.ru.md)
проходят machine-аудиты.
Далее закрываются оставшиеся корпуса R2, затем перегенерируются все
