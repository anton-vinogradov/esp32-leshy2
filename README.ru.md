...
> **Сейчас: H1-R2.0 · принятая функциональная архитектура превращается в новый физический дизайн.**
...
> receive-only Airband AM. Владение GPIO и фабричный active-BOM Airband уже
> закрыты; прежние H1–H5 R1 сохранены только как переиспользуемые evidence.
> Питание, компоновка, схемы и firmware-контракты R2 пересчитываются.
...
Компактная схема ниже — текущая R2-архитектура. Следующие за ней размерные
виды — принятая геометрия R1, которая пересобирается на H1-R2.0: шелкография
общего приёмного порта уже обновлена, но размещение новых Hub, FPV и Airband
не заявляется до проверки коллизий и питания.
...
| **H1 · Физический дизайн устройства** | **▶️ Сейчас: H1-R2.0, полная перекомпоновка** | [План H1](docs/stage-results.ru.md#h1) |
...
**Железо находится на H1-R2.0.** H0-R2 фиксирует шесть вычислительных доменов,
...
стоит `$20.2038` до passives/assembly. Старый R1 rail 2,5 А явно отменён:
H1 должен закрыть ≥3,5 А continuous / ≥4,0 А step и пересобрать все физические
виды до возобновления production ECAD. Ни один заказ не разрешён.

<!-- current-substep: H1-R2.0 -->

**Точный маркер: `H1-R2.0`** — закрыть физическую карту компонентов R2,
collision checks, rail envelope и все актуальные виды. Маркер, machine-state и
обе языковые страницы меняются вместе в каждом commit.
