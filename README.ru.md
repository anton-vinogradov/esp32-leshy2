...
**Точный маркер: `H1.1.3.3`** — закрыть четыре классифицированных механических
blocker H1: хвост/заказную идентичность дисплея, три nRF IPX-тракта, mating
stack U214 и custom D-pad actuator. Уже отрисованные виды остаются черновыми.
...
- `H1.1` — собрать реестр физических первоисточников.
  ...
  - `H1.1.3` — классифицировать каждую оставшуюся физическую неопределённость.
    ...
    - ▶️ **`H1.1.3.3` — сейчас:** закрыть три evidence-blocker дисплея, nRF и
      U214, а также спроектировать custom D-pad actuator.
...
`H1.1.3.3` завершается, когда по дисплею, nRF и U214 получены controlled или
принятые received evidence, а D-pad имеет размерный проверяемый дизайн. Старое
правило, откладывающее любые evidence samples до P1-P6, сейчас создаёт циклическую
зависимость и требует явного решения. После закрытия любой подзадачи этот
маркер и обе страницы роадмапа обновляются тем же commit до перехода дальше.
Поздняя правка повторно открывает затронутый пользовательский gate и все
зависящие от него gates.
