...
**Точный маркер: `H1.1.3`** — классифицировать каждую оставшуюся механическую
неопределённость как blocker H1 или явный gate проверки полученного образца
H5. Уже отрисованные виды остаются черновыми выходами и не завершают поздние
подэтапы.
...
- `H1.1` — собрать реестр физических первоисточников.
  ...
  - ▶️ **`H1.1.3` — сейчас:** классифицировать каждую оставшуюся
    неопределённость как blocker H1 или gate проверки полученного образца H5.
...
`H1.1.3` завершается, когда у каждой неопределённости реестра физических
источников есть один явный статус, граница доказанного и gate закрытия; blocker
H1 нельзя скрыть за будущей проверкой образца. После закрытия любой подзадачи
этот маркер и обе страницы роадмапа обновляются тем же commit до перехода
дальше. Поздняя правка, изменяющая уже согласованный вид, повторно открывает
его пользовательский gate и все зависящие от него gates.
