...
> **Сейчас: H1-R2.3 · принятая функциональная архитектура превращается в новый физический дизайн.**
...
> и увеличенную ячейку настройки; питание, оставшаяся компоновка, схемы и firmware-контракты
...
Компактная схема ниже — текущая R2-архитектура. Следующий чертёж H1-R2.3 —
...
![Внутреннее размещение Leshy2 H1-R2.3](docs/images/h1-r2-inner-placement.svg)

[Открыть читаемый результат размещения H1-R2.3](docs/h1-r2-physical-layout.ru.md) ·
...
| **H1 · Физический дизайн устройства** | **▶️ Сейчас: H1-R2.3, фильтр Airband и внутреннее размещение** | [Текущий результат](docs/h1-r2-physical-layout.ru.md) |
...
**Железо находится на H1-R2.3.** H0-R2 фиксирует шесть вычислительных доменов,
...
текстовые коды; цвет только дублирует их. Старый rail R1 2,5 А остаётся отменённым: H1 ещё должен закрыть ≥3,5 А
continuous / ≥4,0 А step и пересобрать все полные физические виды. Заказ не разрешён.

<!-- current-substep: H1-R2.3 -->

**Точный маркер: `H1-R2.3`** — [текущее размещение](docs/h1-r2-physical-layout.ru.md)
