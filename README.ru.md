...
- ✅ `H2.2` — реализовать и проверить листы UI/control PCB.
  - ✅ `H2.2.1` — проверен корень UI: девять дочерних листов, 95 точных
    межлистовых цепей и 232 именованных pins/child labels; прямые rails приняты KiCad.
  ...
  - ✅ `H2.2.3` — проверены 49 точных экземпляров display/touch/storage:
    ...
    изоляция данных и все 18 hierarchy-интерфейсов.
  ...
  - ✅ `H2.2.6` — проверены 59 точных BOM-экземпляров C5/IR/service плюс
    ...
    fail-closed IR TX, data-only USB, recovery и 18 hierarchy-интерфейсов.
  - ✅ `H2.2.7` — проверены 32 точных компонента приёмника FM/AM/SW/LW:
    два разных антенных порта, полные цепи питания/control/clock/audio Si4732,
  ...
  - ✅ `H2.2.10` — проверены 11 точных физических manufacturing/test-площадок
    1,0 мм; у каждой одна реальная цепь, стабильное имя и нет ложного BOM/MPN.
- ▶️ `H2.3` — реализовать и проверить листы RF/power PCB.
  - ✅ `H2.3.1` — проверен `RF_00_ROOT`: 12 дочерних листов, 133 точные
    межлистовые цепи и 305 явных pin/label; текущие ERC-находки точно
    соответствуют пока пустым функциональным child-листам.
  - ✅ `H2.3.2` — проверен `RF_01_USB_PD_CHARGE`: 52 точных компонента,
    208 физических контактов корпусов, защищённый sink-only USB-PD,
    2S/750-кГц NVDC-зарядка, девять hierarchy-интерфейсов и десять
    объяснённых NC-контактов.
  - ✅ `H2.3.3` — проверен `RF_02_PACK_SAFETY_AON`: 61 точный symbol,
    198 физических контактов корпусов/интерфейсов, полный fail-closed допуск
    2S pack, восемь hierarchy-интерфейсов и шесть объяснённых NC-контактов.
  - ✅ `H2.3.4` — проверены 69 точных компонентов основных rails/domain gates,
    186 физических контактов, независимые защищённые AON/main/accessory rails,
    20 hierarchy-интерфейсов и три объяснённых NC-контакта.
  - ▶️ **`H2.3.5` — сейчас:** RP2354 core, flash и service access.
  - ⏳ `H2.3.6` — три независимых полнофункциональных nRF24-тракта.
  - ⏳ `H2.3.7` — Sub-GHz и VHF/UHF voice-тракты.
  - ⏳ `H2.3.8` — U214 и защищённое M5 Unit expansion.
  - ⏳ `H2.3.9` — задние органы управления и энкодер.
  - ⏳ `H2.3.10` — динамик, микрофон и усиление audio.
  - ⏳ `H2.3.11` — RF/power-сторона точного 80-контактного M1.
  - ⏳ `H2.3.12` — RF-side TX safety и физическое evidence.
  - ⏳ `H2.3.13` — manufacturing/test-площадки RF/power-платы.
