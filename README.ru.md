...
- Батарейная система имеет два слота для отдельно заменяемых квалифицированных
  18650. Переполюсовка исключается механически; аппаратная часть наблюдает и
  допускает каждую ячейку до её подключения к системе и отказывает опасному
  сочетанию вместо принудительной работы или выравнивания.
...
flowchart TD
  ...
  CHARGER["BQ25798RQMR<br/>1–4-cell-capable buck-boost зарядник и NVDC системный power path"]
  ...
  PACKMGR["MPN TBD<br/>допуск и защита ячеек, оценка состояния и управление топологией"]
  ...
  CELL0 --> PACKMGR
  CELL1 --> PACKMGR
  PACKMGR <-->|"квалифицированная контролируемая battery boundary"| CHARGER
  S3 <-->|"1-bit SDIO"| C5
