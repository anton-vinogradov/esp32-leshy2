...
> **Сейчас: H1-R2.13 · K331 остаётся основным; открыт один gate production-пакета AKK.**
...
> вписывается в зарезервированные GPIO и питание. JLCPCB подтвердила отсутствие
> модуля в Parts Library и Global Sourcing, не нашла прямой замены и принимает
> оригинальные модули AKK через Consigned Parts. Единственный оставшийся блокер
> H1 — официальный production-пакет AKK с максимальными XYZ, посадкой и
> soldering/reflow; он же открывает заявку consignment. Точная линейная антенна
> FPV выбрана;
...
Компактная схема ниже — текущая R2-архитектура. H1-R2.13 теперь генерирует
...
плоскости разреза. До превращения резерва K331 в контролируемый точный корпус они остаются
в работе, а не проведённым ревью.

![Функциональная архитектура Leshy2 H0-R2](docs/images/h0-r2-functional-architecture.svg)

![Полное внутреннее размещение Leshy2 H1-R2.13](docs/images/h1-r2-inner-complete.svg)

![Положение и зоны подключения MMCX Leshy2 H1-R2.13](docs/images/h1-r2-mmcx-service.svg)
...
| **H1 · Физический дизайн устройства** | **▶️ Сейчас: H1-R2.13, K331 основной после поиска полнофункциональных fallback** | [Текущий результат](docs/h1-r2-physical-layout.ru.md) |
...
**Железо находится на H1-R2.13.** H0-R2 фиксирует шесть вычислительных доменов,
...
ключованного MMCX `FPV RX 5.8G`. JLCPCB подтвердила отсутствие K331 в Parts
Library и Global Sourcing, не нашла прямой замены и выбрала маршрут оригинальной
поставки AKK через Consigned Parts. K331 остаётся физическим резервом только до
получения единого контролируемого production-пакета AKK с максимальными XYZ,
посадкой и soldering/reflow. Контролируемый производителем `AWM666V RX` входит в ту же ячейку 30×24×4 мм и
...
Точный mechanical/assembly-запрос остаётся у AKK. Ответ JLCPCB переносит
Consigned Parts application, финальный DFM по Gerber/BOM/CPL и дополнительный
function test 5 В/channel-select/RSSI/CVBS в H5/H6/H7, а не оставляет их
блокерами H1. Точный `DL-MMCX-KWE-90` теперь правильно пересекает правую кромку
...
progress до превращения резерва K331 в контролируемый точный корпус.
Заказ не разрешён.

<!-- current-substep: H1-R2.13 -->

**Точный маркер: `H1-R2.13`** — [текущее размещение](docs/h1-r2-physical-layout.ru.md),
[результат питания](docs/h1-r2-power-thermal.ru.md) и [результат FPV](docs/h1-r2-fpv.ru.md)
проходят machine-аудиты. Далее нужно получить единый контролируемый production-пакет
K331 с максимальными XYZ, посадкой и soldering/reflow; после этого
сгенерированные виды можно перевести в reviewed. Маркер, machine-state
