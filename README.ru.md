...
- **Куда более читаемый экран** — **4.0″ 320×480 IPS цветной** с яркой подсветкой и цветным спектр-водопадом на аппаратном верт. скролле: крупнее *и* чётче, чем маленький простой экран DIV.
- **Полноценное управление** — **5-поз. D-pad + BACK + OPTIONS + паник-STOP + F1 / F2 + энкодер-колесо** (у DIV кнопки минимальные); длинный текст печатается с телефона.
- **Честные «в эфире»-индикаторы** — янтарный TX-LED на антенну (пассивный аппаратный детектор огибающей, 0 GPIO), показывает передачу тракта даже при зависшей прошивке.
...
- **Настоящая одновременная работа нескольких радио.** Тракты делят одну SPI-шину и тесный пин-бюджет, поэтому одно радио за раз (параллельный скан 3× nRF24 — исключение). *(пин / шина бюджет)*
...
**Честные пределы.** 5 ГГц — **только разведка**, весь HF Si4732 — **только приём**, **одно радио за раз** (тракты делят SPI). Полный список того, что намеренно **вне scope и почему** (raw 5 ГГц, аналитика уровня Linux, широкополосный SDR, глушение…), — в этапе 1.
