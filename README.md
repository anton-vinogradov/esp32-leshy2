...
- **Two boards in a clamshell, split by processor (this reverses the earlier single-board plan).** Both boards face their components inward; the outer faces carry only the display + controls (front) and the battery pack (back), so the electronics live protected in the gap. Each MCU keeps its own radios on its board — **with one deliberate exception: the 3× nRF24 move to the C5 board** to unclog the front and split the 2.4 GHz cluster (see antennas). The **S3 main board** holds the S3, the wired radios (bar the nRF), their antennas, the display, controls and SD; the **C5 board** holds the C5, the 3× nRF24, their antennas, the charger and power. Accepted cost: roughly double the fabrication, and the nRF's SPI2 branch crosses the mezzanine (below). We take that for the protected packaging and the C5 as a self-contained module.
- **Antennas by board — the three nRF24 move to the C5 board.** Most antennas sit with their driving MCU, but the **3× nRF24 (radios + antennas) go to the C5 board**: this unclogs the front top edge (eight jacks was too tight) and **splits the 2.4 GHz cluster** — Wi-Fi 2.4 stays with the S3, the three nRF go with the C5 — for real isolation. So the **S3 main board** carries five removable SMA — Wi-Fi 2.4, CC1101 (sub-GHz), SA868 (UHF), LoRa, Si4732 (HF/CB/FM, + telescopic whip) — plus IR and the GPS patch; the **C5 board** carries four — 3× nRF24 + the C5 dual-band 5 GHz. All staggered along their edge, the **nRF24 spread apart** (never adjacent), labelled in-place on the render. Cost: the nRF's SPI2 branch now crosses the mezzanine (below).
...
- **Mezzanine: the S3↔C5 link + power + the nRF's SPI2 branch (~20 lines).** With the three nRF on the C5 board, the shared **SPI2 that feeds them** (SCK/MOSI/MISO + 3× CE + the combined IRQ + 3× CS) crosses the board-to-board connector alongside the SPI3 link (SCK/MOSI/MISO/CS/DRDY), the power rails (3V3/GND) and the C5 control lines — on the order of **~20 lines**. Crossing SPI2 is the accepted cost of moving the nRF (front unclogged, the 2.4 GHz cluster split for isolation); it is kept short with interleaved grounds (signal-integrity care). Everything else — I²C, the display, and the other radios' SPI2 — stays local to the main board.
...
- **Recovery buttons: S3 gets RESET + BOOT, the C5 just RESET.** The S3's **RESET + BOOT** (physical, service — on an inner face, pinhole access) drive its USB flashing and recovery. The C5 needs **no BOOT** — its native USB-Serial-JTAG auto-enters download over its own USB-C, and the S3 already drives `C5_EN` / `C5_BOOT` through a PCA9555 — so the C5 board carries just a **RESET** for a manual reboot at bring-up.
- **Mockup drawing conventions — how the layout render is (re)drawn.** The [clamshell render](docs/img/layout-clamshell.en.svg) is generated to a fixed spec:
    ...
    - **every component from the board checklist below** — so nothing is dropped between redraws:
        - **Main board (S3):** S3 · CC1101 + SP4T · SA868 · SX1262/LoRa · Si4732 + audio · GPS · microSD · display + FPC + touch · 74HC138 · 2× PCA9555 · backlight driver · **RESET + BOOT** · controls (D-pad, BACK, OPTIONS, encoder, F1/F2, PTT, STOP) · TX LEDs · WS2812 · IR TX/RX · speaker · mic · 3.5 mm jack · 2× Grove · master toggle · USB-C (J1) · 5 antennas + GPS patch · mezzanine · 4 mounting holes.
        - **C5 board:** C5 · 3× nRF24 · PCA9555 · BQ25887 charger · 2× buck + LDO · S-8252A protection · 2× 18650 holder + BT1 · USB-C (J2) · **C5 RESET** · 4 antennas · mezzanine · 4 mounting holes.
...
- **Main board (S3) — outer front:** 4.0″ capacitive-touch display + D-pad (5-way) + BACK + OPTIONS; speaker + mic (lower). **Left edge:** encoder, F1 / F2, 3.5 mm jack. **Right edge:** PTT + panic STOP, 2× Grove (I²C). **Top edge:** IR TX/RX + five antennas (Wi-Fi 2.4, CC1101, SA868, LoRa, Si4732). **Bottom edge:** microSD. 4 corner mounting holes.
- **Main board (S3) — inner:** ESP32-S3 + the wired radios (CC1101 + SP4T, SA868, SX1262 / LoRa, Si4732 + audio) + GPS + the buses (74HC138, 2× PCA9555) + display driver + RESET / BOOT; five antennas (Wi-Fi 2.4, CC1101, SA868, LoRa, Si4732) + GPS patch; the mezzanine connector.
- **C5 board — inner:** ESP32-C5 + the **3× nRF24** + a PCA9555 + the charger and power (2× buck, LDO, S-8252A protection) + a C5 RESET; four antennas (3× nRF24 + the C5 dual-band 5 GHz); the mezzanine connector.
