...
| **H1 · Physical product design** | **▶ Ready for acceptance · `H1-R2.35`** | [Current placement](docs/h1-r2-physical-layout.md) · [grouped cost ranking](docs/h1-r2-cost.md) |
...
- ✅ The 210-line `H1-R2.35` base BOM is grouped by exact MPN and cost-ranked for one fitted prototype; R2 overrides now honestly count the second RP and its complete reference support, four DBG10 headers and 1,096 fitted parts. The electronics planning floor is $283.84 before five unpriced lines, boards and assembly; already identified no-loss replacements leave $259.33, so the $220–260 target is only partially demonstrated. The former five-board BOM Tool run remains historical evidence only. Procurement targets exactly one factory-assembled prototype without batteries. The HMX donor route is rejected; the selected EastRising panel is a customer-supplied final-assembly part.
