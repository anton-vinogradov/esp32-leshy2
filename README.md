...
- **Two boards in a clamshell, split by role.** Both boards face their components inward; the outer faces carry only the display + controls (front) and the battery pack (back), so the electronics live protected in the gap. The **S3 main board** holds the S3, the wired radios (CC1101 + SP4T, SA868, LoRa, Si4732 + audio), GPS, the display, microSD, the buses and every user control; the **C5 board** holds the C5 and the whole **2.4 / 5 GHz + IR radio block** — its 5 GHz, the 3× nRF24 and the IR — plus the charger, power and battery. Each board keeps its own radios local, so the board-to-board connector stays thin (below). ~75 × 150 mm per board.
- **Antennas by board — five and five, all pointing up.** The **S3 main board** carries five removable SMA — Wi-Fi 2.4, CC1101 (sub-GHz), SA868 (UHF), LoRa, Si4732 (HF/CB/FM, + telescopic whip) — plus the GPS patch; the **C5 board** carries four — 3× nRF24 + the C5 dual-band 5 GHz — plus **IR**, so each board's top edge holds **five interfaces**. This splits the 2.4 GHz cluster — Wi-Fi with the S3, the three nRF with the C5 — for real isolation. All antennas **point up**; to stop the two boards' antennas colliding, the **front board's exit at the seam** (toward the mezzanine) and the **back board's at the back edge**, offset in depth rather than stacked. Labelled in-place on the render.
...
- **Recovery buttons: S3 gets RESET + BOOT, the C5 just RESET.** The S3's **RESET + BOOT** (inner face, slot access) drive its USB flashing and recovery. The C5 needs **no BOOT** — its native USB-Serial-JTAG auto-enters download over its own USB-C — so the C5 board carries just a **RESET** for a manual reboot at bring-up.
- **Mockup drawing conventions — how the layout render is (re)drawn.** The [clamshell render](docs/img/layout-clamshell.en.svg) is generated to a fixed spec:
    ...
    - **every component from the board checklist below** — so nothing is dropped between redraws:
        ...
        - **C5 board:** C5 · **3× nRF24 (C5-driven)** · **IR TX/RX (C5-driven)** · 74HC139 (nRF CS) · PCA9555 · BQ25887 charger · 2× buck + LDO · S-8252A protection · 2× 18650 holder + BT1 · **USB-C ×2 (J1 + J2)** · **master switch** · **C5 RESET** · **4 antennas + IR** · mezzanine · 4 mounting holes.
...
- **Main board (S3) — outer front:** 4.0″ capacitive-touch display + 5-way D-pad + BACK + OPTIONS; speaker + mic (lower); the TX-live LEDs + RGB status. **Left edge:** encoder, F1 / F2, 3.5 mm jack. **Right edge:** PTT + panic STOP, 2× Grove (I²C). **Top edge:** five antennas (Wi-Fi 2.4, CC1101, SA868, LoRa, Si4732). **Bottom edge:** microSD. 4 corner mounting holes.
...
- **C5 board — inner:** ESP32-C5 + the **C5-driven 3× nRF24** (+ 74HC139 for their CS) + the **C5-driven IR TX/RX** + a PCA9555 + the charger and power (2× buck, LDO, S-8252A protection); **both USB-C + the master switch** (inner, slot-accessed) + a C5 RESET; four antennas (3× nRF24 + the C5 dual-band 5 GHz) + IR; the mezzanine connector.
- **C5 board — outer back:** the plain 2× 18650 holder (keep-out, no parts under cells). 4 corner mounting holes.
