...
- **User controls on the outer front / edges; only service parts go inside.** What the operator presses stays reachable on the **outer front**: a real **5-way D-pad** — a proper navigation switch, drawn as one, not loose keys — with **BACK + OPTIONS as separate keys** beside it; encoder + F1 / F2 + 3.5 mm jack on the left edge; PTT + panic-STOP + 2× Grove on the right edge; the **speaker front-lower-left** (clear of the gripping hand) and the mic lower-right. Every TX-capable chain shows its **amber TX-live LED** (hardware envelope detector) and the **RGB status** WS2812 sits on the front — all marked on the render. Buttons ride a PCA9555; the encoder A/B are on direct GPIO for clean quadrature. Capacitive touch is an addition, never the only way in.
...
- **Four corner mounting holes, aligned on both boards.** M2.5 holes at the four corners of both boards; standoffs between them set the mezzanine gap and carry the display and battery loads. The standoffs / spacers are **no shorter than the DIV's** — the proven gap that clears the inner components. ~75 × 150 mm per board, ~34 mm total thickness (the 18650 pack is the thickest outer layer).
...
- **Mockup drawing conventions — how the layout render is (re)drawn.** The [clamshell render](docs/img/layout-clamshell.en.svg) is generated to a fixed spec:
    ...
    - **connectors on the inner faces, at the edge they exit, with a direction arrow** — every connector / switch / service button lives on an **inner** face (the outer faces stay display / battery) and sits **on the board edge it reaches** (a part mid-board can't meet the case slot). The **direction arrow points out from the part into the margin** — never onto a label (checked). Main board: jack (left edge), 2× Grove (right edge), microSD + `RESET` + `BOOT` (bottom edge). C5 board: both USB-C, master slide, `C5 RESET`, `C5 BOOT` (bottom edge);
    ...
    - **locked positions** (a redraw keeps the same map): antennas on the outer faces across the top; display outer-front upper; D-pad lower-centre with BACK / OPTIONS flanking; the LED row below the display; encoder + F1 / F2 on the outer-front left edge, PTT + STOP on the right edge; speaker lower-left, mic lower-right; **main-inner connectors — jack (left edge), 2× Grove (right edge), microSD + RESET + BOOT (bottom edge)**; **C5-inner bottom edge — USB-C ×2 + master + C5 RESET + C5 BOOT**; battery centred on the C5 outer back with BT1 below; the mezzanine interior; mounting holes at the four corners;
    - **overlap-checked programmatically** before use — parse the finished SVG and confirm no rectangle / text collisions (never by eye);
    - **accessibility-checked programmatically** — every **interface** (blue) on an **inner** face must sit at a board edge; the generator flags any buried connector / switch / button and refuses the render, the same gate as the overlap check (an interface the user can't reach is a bug);
    - **bilingual** (EN + RU labels);
    - **every component from the board checklist below** — so nothing is dropped between redraws:
        - **Main board (S3):** S3 · CC1101 + SP4T · SA868 · SX1262/LoRa · Si4732 + audio · GPS · microSD · display + FPC + touch · 74HC138 · 2× PCA9555 · backlight driver · **RESET + BOOT** · controls (D-pad, BACK, OPTIONS, encoder, F1/F2, PTT, STOP) · TX LEDs · WS2812 · speaker · mic · 3.5 mm jack · 2× Grove · **5 antennas + GPS patch** · mezzanine · 4 mounting holes.
        - **C5 board:** C5 · **3× nRF24 (C5-driven)** · **IR TX/RX (C5-driven)** · 74HC139 (nRF CS) · PCA9555 · BQ25887 charger · 2× buck + LDO · S-8252A protection · 2× 18650 holder + BT1 · **USB-C ×2 (J1 + J2)** · **master switch** · **C5 RESET + BOOT** · **4 antennas + IR** · mezzanine · 4 mounting holes.
...
- **Main board (S3) — outer front:** 4.0″ (320 × 480 portrait) capacitive-touch display + 5-way D-pad + BACK + OPTIONS; encoder + F1 / F2 on the left edge; PTT + panic STOP on the right edge; speaker + mic (lower); the TX-live LEDs + RGB status; **five antenna SMA across the top** (Wi-Fi 2.4, CC1101, SA868, LoRa, Si4732, whips up). Only what you look at or press lives on the front. 4 corner mounting holes.
- **Main board (S3) — inner:** ESP32-S3 + the wired radios (CC1101 + SP4T, SA868, SX1262 / LoRa, Si4732 + audio) + GPS + the buses (74HC138, 2× PCA9555) + display driver; **the connectors sit here on the inner face, at the edges** — 3.5 mm jack (left edge), 2× Grove I²C (right edge), microSD + `RESET` + `BOOT` (bottom edge) — reached through case slots; **u.FL pigtails up to the five outer SMA**; the mezzanine connector.
- **C5 board — inner:** ESP32-C5 + the **C5-driven 3× nRF24** (+ 74HC139 for their CS) + the **C5-driven IR TX/RX** + a PCA9555 + the charger and power (2× buck, LDO, S-8252A protection); **the bottom edge carries both USB-C, the master slide, `C5 RESET` and `C5 BOOT`**; **u.FL up to the four outer SMA + IR**; the mezzanine connector.
