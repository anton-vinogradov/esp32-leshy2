...
Здесь собраны актуальные принципиальные схемы `H0-R2`/`H1-R2.17` готового
...
```mermaid
flowchart TD
  S3["ESP32-S3-WROOM-1U-N16R8<br/>UI, экран, прямые клавиши"]
  ...
  M1["Hirose FX8C-80<br/>один CVBS + ограниченные IPC/control<br/>питание, земля, восемь резервных сигналов"]

  S3 <-->|"1-bit SDIO"| C5
  S3 <-->|"локальный high-speed link"| FRP
  FRP <-->|"проверенный RP-link 1,5 МБ/с"| RRP
  K331 -->|"75-омный CVBS"| M1
  M1 --> TVP
  TVP -->|"локальный LCD_CAM"| S3
```
...
декодированная шина остаётся на передней плате.
