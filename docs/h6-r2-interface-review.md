...
| S3 recovery fallback | The 2026-09-14 review found UI J2 connected to GPIO7/8, not ROM UART0 GPIO43/44; the latter are used by the Hub bus without the required isolation. Native USB remains the direct loader path, subject to power/reset qualification. Correct the UART fallback; the current six-target scope is recorded in [programming and recovery](service-recovery.md). |
