...
**2026-09-13: the encoder is under Cap; see the [placement repair](h6-r2-placement-repair.md) for current coordinates and verification. H6.0.3-R1 remains open, not fabrication-ready.**
...
Current coordinates, retained controls, measured gaps and the under-Cap repack
have one owner: the [placement repair](h6-r2-placement-repair.md), backed by the
[finite machine review](../hardware/layout/h6-r2-encoder-under-cap-review.json).
Older encoder/audio snapshots retain their historical scope; they do not set
today's placement. This page tracks the remaining interface/assembly boundaries.
...
| --- | --- |
| Native placement | Current results and exact scope: [placement repair](h6-r2-placement-repair.md). Earlier test results are not transferred to a changed PCB. |
| Copper / routing | Copper is preserved; live connectivity counts and hash-bound DRC evidence belong to the [current-routing audit](../hardware/layout/generated/H6-R2-current-routing-audit.json). Pad-centre distances are not routed lengths or electrical qualification. |
...
| Labels and images | All **35/35 required RF labels** match native text, with **0 collision candidates**. Current views and test results are refreshed. Fab bodies and assembly annotations are not silkscreen or fitted parts. |
...
| Alps EC11E18244AU encoder | The [reviewed engineering footprint](h6-r2-encoder-engineering-fit.md) retains the exact nominal 12.5 mm mounting-lug pitch. The current left-side shaft is **[9.25,54] under Cap**, not the earlier left/down fit snapshot. Rounded PTH fit, physical insertion, solder strength, knob/finger access and assembled Z remain open; DRC is not production-fit approval. |
| Other through-pad solder access | USB, Cap and encoder lands are represented on both faces. Native geometry passes at the new encoder site; actual tool access and assembly order remain open. The 1-mm screen is an engineering aid, not a factory requirement. L32's two endpoints are an unrouted NFC reserve, not fitted leads. |
...
| Microphone/speaker and relocated UHF | MK1 stays inside at the bottom edge with sound access through the designed open bottom gap. Moving U83 upward lengthens **HEADSET_MIC_RAW U83.1→U85.3 from 23.229 to 80.397 mm** straight-line distance; remote capture/bias paths also remain open. Local cell improvements do not qualify low-noise routing, return paths or acoustics. U39 still needs its RF-feed, ground-plane, power and inter-module coupling review. Speaker fixation/insulation and wire strain relief remain open; RF LS1 is wire termination, not the UI speaker body. |
| C5 service path | On 2026-09-14 the erroneous NC on U14.15/GPIO28 was corrected: both schematic and PCB pad now belong to C5_BOOT_N with R76/R79. Existing SW18, 1-kΩ series resistance and 10-kΩ pull-up are retained; GPIO27 has its own pull-up. This repairs net membership, not completed copper or tested boot behavior. [Mux selector/enable control](h6-r2-c5-mux-control-review.md), the 3-ms strap hold after EN and reset behavior under KILL remain open. |
| S3 recovery fallback | The 2026-09-14 correction assigns UI J2 via R4/R5 to ROM UART0 GPIO43/44 and moves Hub D2/D3 to GPIO7/8. Four native pad/net assignments change; placement and existing copper are preserved. Dedicated-path guards replace the unimplemented isolator requirement. Copper routing, reset/BOOT timing, Hub startup and end-to-end recovery remain open; see [programming and recovery](service-recovery.md). |
...
- Current owners: [placement contract](../hardware/layout/h6-r2-placement-contract.json), [under-Cap placement/pair review](../hardware/layout/h6-r2-encoder-under-cap-review.json), [native instance ledger](../hardware/ecad/generated/H2-R2-native-instance-ledger.json), [assembly coordinates](../hardware/product-design/assembly-coordinate-model.json), [display contract](../hardware/product-design/display-mount.json).
...
- Component images: the [view manifest](../hardware/layout/generated/H6-R2-component-views.json) binds PCB hashes and all five published SVG hashes: four faces plus overview; it must match the new placement before visual acceptance. The actual card notch is shown; the ejected-card position is a dashed explanatory overlay, not silkscreen or part of the PCB outline. Native component views do not qualify assembled bodies or full H1-to-PCB parity.
