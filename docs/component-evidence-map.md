...
- Still to prove: current-lot complete FPC construction and fit in the direct FH34SRJ-50S-0.5SH(50), including the folded maximum component stack, safe 180-degree bend radius, exact traceable stocked 50×50-mm acrylic-foam MPN/thickness and rounded-slot insertion; received ER-TFT035IPS-6 plus ER-TPC035-6 identity/readback, measured VDD/VDDI ramp equality and one-prototype adhesive/FPC assembly record. The 30.16±0.50-mm source tail, slot at [24.0,23.0], ZIF at [24.0,25.0], 1-to-1 / 50-to-50 no-twist orientation, maximum 24.66-mm measured route and minimum 5.00-mm relaxed slack are already fixed.
- Pass rule: before the PSA liner is removed, the factory dry fit confirms the exact ER-TFT035IPS-6 + ER-TPC035-6, one untwisted fold, panel pin 1 to connector pin 1, panel pin 50 to connector pin 50, neutral-axis route to contact stop <=24.66 mm, visibly relaxed slack >=5.00 mm and a latch carrying no tensile load. The assembly record then confirms one stocked 50×50-mm PSA rectangle correctly located on the PCB, free upper FPC zone and direct-FH34SRJ insertion, followed by panel placement on the second datum; owner bring-up of the sole prototype confirms ILI9488/FT6236, VDD/VDDI, reset, image and touch; a mismatch reopens H1/H2/H3
...
- `H5-MECH-DISPLAY-TAIL` — `EastRising ER-TFT035IPS-6 + ER-TPC035-6`; `Hirose FH34SRJ-50S-0.5SH(50)`; open: Current-lot folded FPC maximum component stack, actual bend radius and one exact traceable stocked 50×50-mm industrial acrylic-foam MPN plus the assembler's accepted material/process remain properties of the one-prototype route. Source geometry and placement are no longer open: 30.16±0.50-mm edge-to-tip length, slot [24.0,23.0], ZIF [24.0,25.0], one fold with no twist, FH34SRJ top-and-bottom contacts, explicit 1-to-1 / 50-to-50 order, measured neutral-axis route <=24.66 mm and relaxed slack >=5.00 mm. The production intent is one stock rectangle at [12.50,44.46], 2500 mm² or about 52.0% panel support, no separately handled top strip, no custom converter contour, and separate PSA and DISPLAY/FPC-UP silkscreen datums. A custom contour is allowed only if the same PCBA factory confirms that it will manufacture and install it inside the order; an external converter order is excluded. The old 0.17-mm 3M 9495LE candidate is not sufficient unless measurement unexpectedly proves the required 0.20-mm clearance.
