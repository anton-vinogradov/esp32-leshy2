...
- Отдельная плата-адаптер и оба DF40 удалены: высота стека снижается с `3.8` до `1.0 мм`, а цена компонентов одного прототипа — на `$1.07`. Панель держат рамка, PSA и мягкий подпор, а не контакты ZIF.
