...
- Серийная материальная база: **$292.23** на устройство; цены известны для `240/249` строк.
...
- Та же принятая ценовая база для десяти устройств: **$3,120.38**. Это линейное сравнение групп, а не обещание цены партии.
- Верхние 10 / 20 / 40 групп дают **36.61% / 54.77% / 74.01%** текущей известной базовой BOM.
...
- Сейчас базовая BOM содержит `247` MPN-групп и `1213` установленных компонентов. Принятые без потери функции маршруты AD8314 и Hirose U.FL уже экономят **$10.42** и оставляют текущий точный planning floor **$312.04**. Дешёвая пара SMA/RP-SMA проверена и отклонена, поэтому её предполагаемая экономия сюда не входит.
