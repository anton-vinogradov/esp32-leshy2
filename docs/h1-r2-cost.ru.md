...
- Исторический JLCPCB capture на пять плат: **$1,293.98** по `181` строкам; `23` live-проверок дают **$1,417.97**, ещё `29` строк не входят; это evidence, а не целевой quantity.
- Внешний антенный комплект вынесен отдельно: уже известно **$138.32**, ещё `4` позиции не оценены.
