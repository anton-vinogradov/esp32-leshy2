...
- Отдельная плата-адаптер и оба DF40 удалены: высота стека снижается с `3.8` до `1.0 мм`, а цена компонентов одного прототипа — на `$1.07`. Панель держит один готовый PSA-прямоугольник 50×50 мм; контакты ZIF нагрузки не несут.
