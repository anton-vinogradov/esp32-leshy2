# H1-R2.24 · стоимость компонентов
...
- Серийная материальная база: **$235.34** на устройство; цены известны для `198/210` строк.
- Достижимый плановый минимум: **$284.61** на устройство; ещё `5` базовых строк не оценены.
- С обязательным модулем K331, устанавливаемым после PCBA: **$314.60** на устройство или **$1,573.01** на пять устройств до стоимости плат, сборки, корпуса, антенн, доставки, налогов, брака и теста.
- Частичный JLCPCB-снимок партии из пяти устройств: **$1,311.51** по `178` найденным строкам; `6` live-проверок дают **$1,370.43**, ещё `32` строки не входят.
...
- `40` pre-order-строк стоят в снимке **$779.78** против **$338.05** на массовой материальной базе.
- Наблюдаемый штраф малой партии — **$441.73**. Это верхний приоритет: искать не «дешевле любой ценой», а эквивалентные stocked JLCPCB MPN внутри уже заданных substitution-классов.
...
1. ✅ **Заменить безопасно эквивалентные pre-order пассивы и обычную логику на складские JLCPCB** — После двух безопасных замен нормализованный снимок партии даёт $779,78 для 40 оставшихся pre-order строк против $338,05 по серийной материальной базе. Складские 74LVC2G126DP и YAGEO C131394 вместе убирают $45,0664 из наблюдаемой цены пробной партии и уменьшают публичную материальную базу ступени 100 штук на $1,3862 на устройство. Проверять каждую строку по её substitution-классу; принимать только точную либо не худшую параметрическую замену.
