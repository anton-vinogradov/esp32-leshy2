...
H3 принят. Все виртуальные проверки воспроизводимы и аналитически закрыты: сведения шести фаз имеют закрытый статус, полная сквозная сверка не имеет пропусков или hash mismatch, а все `85` physical-only строк сохраняют владельцев и pass rules H5/H6/H8.
