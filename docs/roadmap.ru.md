...
24. ▶ `H6.0.2-R1`: [fail-closed политика трассировки](h6-r2-routing-policy.ru.md) распределяет все 858 локальных экземпляров net / 823 канонические сети и разрешает автопредложения только для обычного низкоскоростного управления; развести все сети, доказать native schematic/PCB parity и получить DRC без замечаний, не нарушив H6.0.1.
