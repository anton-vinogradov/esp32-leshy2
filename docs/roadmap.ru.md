...
Сейчас — [автоматическая трассировка и приёмка проекта](h6-r2-routing-automation.ru.md). Геометрический рубеж на 474 связи и ограниченный пилот генерации индикаторов прошли проверки в своих рамках; электрическая квалификация открыта. Команда MAIN собирает 18 полных схемных вариантов в памяти, проверяет приёмники fault-линии и теперь сравнивает напряжение AON на всех 56 текущих нагрузочных профилях существующим расчётом H3. Условные расчётные диапазоны устраняют прежний конфликт напряжений при нулевом токе линии, но сопротивление защиты в установленных условиях и токи ветвей не доказаны; исходное предупреждение для диапазона требований сохранено. Независимый ngspice проверяет уравнения fault-линии, не физическое поведение микросхем. Повторные команды не обращаются к модели. Далее — привязать реальные границы AON/вспомогательных токов и режимы GPIO, затем проверить нагруженные уровни fault, запуск и динамическую защиту перед перестроением native. Существующие проблемы питания остаются видны; кандидаты MPN не принимаются. Рабочая трассировка приостановлена. Это внутренний шаг H6.0.3, не закрытие H6 и не разрешение изготовления.
