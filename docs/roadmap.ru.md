...
| Актуальная production ECAD-схема | ▶️ H2.3.2: USB-PD и зарядка двух элементов; H2.3.1 RF/power root проверен |
...
- ✅ `H2.2` — реализовать и проверить листы UI/control PCB.
  - ✅ `H2.2.1` — `UI_00_ROOT`: девять дочерних листов, 95 точных межлистовых
    ...
    пустых stubs нет, 387 предупреждений о копиях сгенерированных символов
    машинно учтены.
...
- ▶️ `H2.3` — реализовать и проверить листы RF/power PCB.
  - ✅ `H2.3.1` — `RF_00_ROOT`: 12 дочерних листов, 134 точные межлистовые
    цепи и 303 явных pins/labels; native KiCad принял иерархию, а все 303
    текущие ERC-находки — точный набор пустых функциональных child-stubs.
  - ▶️ **`H2.3.2` — сейчас:** `RF_01_USB_PD_CHARGE`.
