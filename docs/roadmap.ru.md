...
> **▶ Текущая аппаратная граница: `H1-R2.35`.** H0 проведено ревью. H1 готово к визуальному принятию, но ещё не прошло ревью.
...
| Физический дизайн | ▶ [H1-R2.35](h1-r2-physical-layout.ru.md): бортовое видео и требующая ручной пайки зона приёмника удалены; точная dual-RP/C5 authority, серийный EastRising-дисплей с адаптером, все 18 компонентов U219, NFC-loop и swept volume штатной антенны проходят fail-closed аудит; до явного принятия мокапа физических блокеров нет; [все 210 MPN-групп базового BOM ранжированы, второй RP и четыре DBG10 учтены, текущий минимум $283,84, а цель полного all-in-one $220–260 пока достигнута лишь частично; отдельный Core отложен до работающего EVT1](h1-r2-cost.ru.md) |
...
**Точный маркер: `H1-R2.35`.** Пакет компоновки готов к принятию; H1 не является
...
- ✅ Публичные схемы показывают по одной плате на изображение. Полная нумерованная
  проекция 215 references опубликована ссылкой как machine-review evidence.
...
| **H1 · Физический дизайн продукта** | **▶ Готово к принятию · `H1-R2.35`** | Внешний вид, отдельные внутренние стороны, разрезы, точные корпуса, RF-локальность, сервис и power-envelope | Нет коллизий bodies/fasteners/silkscreen/antennas/accessories/opposing sides; точный MPN или контролируемый reserve; мокап принят |
