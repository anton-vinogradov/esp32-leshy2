...
> **▶️ Текущая аппаратная граница: H1-R2.2 — точные вспомогательные детали и инкрементальное внутреннее размещение.**
...
| Физический дизайн устройства | ▶️ [H1-R2.2](h1-r2-physical-layout.ru.md): Hub, Airband, резерв FPV, размерный MMCX и LDO FPV размещены/выбраны; продолжается закрытие остальных корпусов и rail/thermal |
| Принципиальные диаграммы на сайте | Функциональная карта H0-R2 и внутреннее размещение H1-R2.2 актуальны; полные размерные виды R1 явно обозначены входами для пересборки |
...
## Текущий H1-R2.2 и сохранённое evidence R1

<!-- current-substep: H1-R2.2 -->

**Точный маркер: `H1-R2.2`** — [инкрементальный физический результат](h1-r2-physical-layout.ru.md)
...
Текущая граница — `H1-R2.2`. Второй Hub RP, точные активные детали Airband,
