...
| Электрические и переходные evidence | ▶️ H3.4.2: levels/defaults проверены; идут digital bandwidth/latency/timing |
...
**Точный маркер: `H3.4.2`** — [digital levels, reset defaults и no-back-power](digital-levels-verification.ru.md)
...
| **H3. Виртуальная электрическая проверка** | ▶️ Сейчас · `H3.4.2` | Расчёты и симуляции до дорогой физики | Проходят worst-case DC budget; startup/shutdown, USB↔battery handover, brownout, watchdog, eFuse и load-step; thermal/fault tree; все analog corners; timing/levels; RF corridors, returns и pre-layout constraints |
