...
| Production ECAD-схема | ✅ Текущая dual-SA818S ревизия H2 принята; firmware BSP синхронизирован побайтно |
...
| Evidence компонентов | ▶️ H5.0.3-R1 имеет корзину из 33 строк за `$286.43` и точные маршруты всех 210 строк BOM / 1052 установок; частичный ответ JLCPCB подтверждает MOQ/типичный срок SA818S-V; аккумуляторы — пользовательский `J5-U`; уточнение по двум designator и остальным J4-F/J4-P открыто; закупка заблокирована |
...
| **H5. Evidence компонентов** | ▶️ Сейчас `H5.0.3-R1`; карта 9 residuals/14 gates, корзина из 33 строк за `$286.43` и все 210 маршрутов готовы; частичный ответ JLCPCB записан, уточнение по двум designator/J4-F/J4-P открыто, аккумуляторы приняты как пользовательский `J5-U`; PCBWay не опрошен; закупка заблокирована | [Текущая корзина](component-sample-basket.ru.md), [карта 210 маршрутов](manufacturing-platform.ru.md) и [supplier gate](../hardware/verification/generated/H5-EVR07-supplier-response-gate.json); прежнее SA518 evidence отменено | У каждой production-BOM-строки есть точный маршрут без молчаливых замен; опубликованы точная стоимость корзины и принятых операций финальной сборки; граница выбранной фабрики явна; полученные образцы доказывают identity, mating, stack-up и критические размеры |
...
источники больше не закрывают остаток. Частичный ответ JLCPCB подтверждает MOQ 1
и типичные 8–15 рабочих дней pre-order SA818S-V и условную post-order цену
Function Test. Аккумуляторы — пользовательский `J5-U`, не gate; реальная схема
с двумя designator, остальные
J4-F/J4-P и exact-MPN control требуют подготовленного уточнения. Право Parts API отклонено
без указанной причины; информационный запрос в поддержку успешно отправлен
26 августа 2026 года, поэтому до ответа активным остаётся ручной evidence-путь; PCB placement/routing, quote/reservation и любой
заказ остаются заблокированы. `H5-EVR07` фиксирует 16 полей без ответа и один
явный отказ, не открывая H6.
