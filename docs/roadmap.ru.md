...
> **▶️ Текущая аппаратная граница: H1-R2.4 — rail/thermal-архитектура шести доменов и инкрементальное внутреннее размещение.**
...
| Физический дизайн устройства | ▶️ [H1-R2.4](h1-r2-physical-layout.ru.md): Hub, Airband, резерв FPV/MMCX/LDO и силовая ячейка 3,75 А continuous / 4,25 А step размещены/выбраны; продолжается закрытие остальных точных корпусов |
| Принципиальные диаграммы на сайте | Функциональная карта H0-R2 и диаграммы размещения, фильтра и rail/thermal H1-R2.4 актуальны; полные размерные виды R1 явно обозначены входами для пересборки |
...
## Текущий H1-R2.4 и сохранённое evidence R1

<!-- current-substep: H1-R2.4 -->

**Точный маркер: `H1-R2.4`** — [инкрементальный физический результат](h1-r2-physical-layout.ru.md)
...
исключают путаницу в комплекте из 12 предметов; цвет только дублирует код.
...
Текущая граница — `H1-R2.4`. Второй Hub RP, точные активные детали Airband,
декодер/резерв приёмника FPV, размерный боковой MMCX и точный LDO FPV прошли инкрементальный аудит
корпусов. Main-rail уже представляет размещённую архитектуру 3,75 А continuous /
