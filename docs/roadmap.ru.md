...
> **▶ Текущая аппаратная граница: `H1-R2.16`.** H0 проведено ревью. H1 — нет.
...
| Физический дизайн | ▶ [H1-R2.16](h1-r2-physical-layout.ru.md): полные функциональные острова, основные антенные банки `5 + 5` и отдельный вертикальный задний FPV MMCX |
...
**Точный маркер: `H1-R2.16`.** Это один подшаг физического дизайна, а не
закрытый отчёт H1.
...
- ✅ Отдельный вертикальный SMT MMCX Molex `73415-2063` (`C588480`) даёт
  `FPV RX · 5.8G` между задними группами SMA и над U214.
...
- ✅ Корпус FPV MMCX оставляет `5,72 мм` до ближайшего SMA.
- ✅ Ø12-зона обращения MMCX оставляет `1,95 мм` до SMA и `0,70 мм` до U214.
...
| **H1 · Физический дизайн продукта** | **▶ Сейчас · `H1-R2.16`** | Внешний вид, отдельные внутренние стороны, разрезы, точные корпуса, RF-локальность, сервис и power-envelope | Нет коллизий bodies/fasteners/silkscreen/antennas/accessories/opposing sides; точный MPN или контролируемый reserve; мокап принят |
