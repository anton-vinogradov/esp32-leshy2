...
> **▶️ Текущая аппаратная граница: H1-R2.6 — точная механика MMCX у кромки, service-keepout и инкрементальное размещение.**
...
| Физический дизайн устройства | ▶️ [H1-R2.6](h1-r2-physical-layout.ru.md): функциональная/пиновая совместимость K331 и точная антенна TBS FPV добавлены к размещённым Hub/Airband/power; точная геометрия MMCX, выводы wave soldering и wall/plug keepout проходят; сейчас H1 блокируют только контролируемый корпус и маршрут установки K331, затем пересобираются полные виды |
| Принципиальные диаграммы на сайте | Функциональная карта H0-R2, FPV/MMCX/размещение H1-R2.6 и диаграммы фильтра/rail H1-R2.4 актуальны; полные размерные виды R1 явно обозначены входами для пересборки |
...
## Текущий H1-R2.6 и сохранённое evidence R1

<!-- current-substep: H1-R2.6 -->

**Точный маркер: `H1-R2.6`** — [инкрементальный физический результат](h1-r2-physical-layout.ru.md)
...
Текущая граница — `H1-R2.6`. Второй Hub RP, точные активные детали Airband,
