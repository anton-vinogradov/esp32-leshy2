...
Сейчас — [автоматическая трассировка и приёмка проекта](h6-r2-routing-automation.ru.md). Геометрический рубеж на 474 связи и ограниченный пилот генерации индикаторов прошли проверки в своих рамках; электрическая квалификация открыта. Команда MAIN теперь собирает и проверяет 18 полных схемных вариантов в памяти из выбранных делителей, резисторов защиты и трёх источников питания монитора, сохраняя все незатронутые записи native-реестра. Явное назначение выводов устраняет конфликт PGTH/FLT в кандидатах без изменения рабочей схемы. Прошли все 288 выбранных тестов; два повтора команды совпали при времени около 8,5 секунды без обращений к модели. Далее — установить реальные границы вспомогательных токов и нагруженной fault-линии, проверить запуск и динамическую защиту перед перестроением native. Существующие проблемы питания остаются видны; кандидаты MPN не принимаются. Рабочая трассировка приостановлена. Это внутренний шаг H6.0.3, не закрытие H6 и не разрешение изготовления.
