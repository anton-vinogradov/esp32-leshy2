...
| Актуальная production ECAD-схема | ▶️ H2.2.2: точные S3 core, memory, boot, USB и service/reset; корень UI H2.2.1 проверен |
...
boot, native USB и service/reset на `UI_10_S3_CORE_MEMORY_BOOT`.
...
- `H2.0` — зафиксировать авторитетные входы схемы и структуру проектов.
  - ✅ `H2.0.1` — проверен полный реестр из 1002 строк: все 974 схемных
    экземпляров основного устройства, 26 общих компонентов LoRa Cap и 2
    альтернативных радиомодуля; сверены 178 тел H1 и 796 схемных компонентов.
  - ✅ `H2.0.2` — четыре проекта, границы плат, rails и net naming повторно
    сверены со всеми 1002 строками; четыре намеренно пустых root/test-листа
...
- `H2.2` — реализовать и проверить листы UI/control PCB.
  - ✅ `H2.2.1` — `UI_00_ROOT`: девять дочерних листов, 73 точные межлистовые
    цепи и 180 явных pins/labels; одна прямая root-rail на цепь, без скрытых
    global labels, native parse KiCad пройден. Ровно 180 findings пустых child-
    stubs учтены машинно и исчезают только при установке circuit pins в H2.2.2–H2.2.9.
  - ▶️ **`H2.2.2` — сейчас:** `UI_10_S3_CORE_MEMORY_BOOT`.
