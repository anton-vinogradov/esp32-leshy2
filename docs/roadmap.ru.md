...
> **▶️ Текущая аппаратная граница: H1-R2.8 — официальные свидетельства интеграции K331 и инкрементальное физическое размещение.**
...
| Физический дизайн устройства | ▶️ [H1-R2.8](h1-r2-physical-layout.ru.md): официальные материалы AKK подтверждают схему включения K331, функции всех 14 контактов и таблицу 24 каналов; пиновая/силовая совместимость, точная TBS FPV-антенна и механика MMCX проходят; сейчас H1 блокируют только контролируемый корпус и маршрут установки K331, затем пересобираются полные виды |
| Принципиальные диаграммы на сайте | Функциональная карта H0-R2, FPV/MMCX/размещение H1-R2.8 и диаграммы фильтра/rail H1-R2.4 актуальны; полные размерные виды R1 явно обозначены входами для пересборки |
...
## Текущий H1-R2.8 и сохранённое evidence R1

<!-- current-substep: H1-R2.8 -->

**Точный маркер: `H1-R2.8`** — [инкрементальный физический результат](h1-r2-physical-layout.ru.md)
...
принят по живой фабричной поверхности. Генерируемый аудит показывает
ноль коллизий корпусов на одной стороне и минимум 2,44 мм между встречными
проекциями. Генерируемая [проверка фильтра Airband](h1-airband-filter.ru.md)
...
размеров и фабричного маршрута JLCPCB либо явной установки после PCBA. Точные
...
Текущая граница — `H1-R2.8`. Второй Hub RP, точные активные детали Airband,
