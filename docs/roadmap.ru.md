...
> **▶️ Текущая аппаратная граница: H1-R2.3 — реализуемость фильтра Airband и инкрементальное внутреннее размещение.**
...
| Физический дизайн устройства | ▶️ [H1-R2.3](h1-r2-physical-layout.ru.md): Hub, Airband, расширенная ячейка фильтра, резерв FPV, размерный MMCX и LDO FPV размещены/выбраны; продолжается закрытие остальных корпусов и rail/thermal |
| Принципиальные диаграммы на сайте | Функциональная карта H0-R2 и внутреннее размещение/аудит фильтра H1-R2.3 актуальны; полные размерные виды R1 явно обозначены входами для пересборки |
...
## Текущий H1-R2.3 и сохранённое evidence R1

<!-- current-substep: H1-R2.3 -->

**Точный маркер: `H1-R2.3`** — [инкрементальный физический результат](h1-r2-physical-layout.ru.md)
...
Далее H1 закрывает остальные точные корпуса и rail/thermal envelope
...
Текущая граница — `H1-R2.3`. Второй Hub RP, точные активные детали Airband,
