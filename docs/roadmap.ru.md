...
stack U214 и custom D-pad actuator. Более поздние виды уже существуют как
черновые проекции, но не считаются проведёнными.
...
- `H1.1` — реестр физических первоисточников.
  ...
  - `H1.1.3` — классифицировать каждую физическую неопределённость; inferred-
    ...
    - ✅ `H1.1.3.2` — записать четыре blocker H1 и девять received-sample gate
      H5 в machine source.
    - ▶️ **`H1.1.3.3` — сейчас:** закрыть evidence-blocker дисплея, nRF и U214,
      а также custom D-pad actuator design.
...
procurement-правило, откладывающее все samples до P1-P6, конфликтует с этими
пререквизитами и должно быть разрешено явно. Закрытие любой подзадачи требует в
том же commit изменить точный маркер на стартовой странице и в роадмапе до
перехода к следующей работе. Поздняя правка повторно открывает затронутый
пользовательский gate и все зависящие от него gates.
