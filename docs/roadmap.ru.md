...
Сейчас — [автоматическая трассировка и приёмка проекта](h6-r2-routing-automation.ru.md). Геометрический рубеж на 474 связи и ограниченный пилот генерации индикаторов прошли проверки в своих рамках; электрическая квалификация открыта. Команда MAIN собирает 18 полных схемных вариантов в памяти и теперь проверяет точные приёмники fault-линии, их питание и условные требования к нагруженным уровням. Диагностика с нулевым током обнаруживает несовместимые углы диапазонов MAIN/AON; реальные токи линии и режимы GPIO неизвестны, поэтому это не измеренная неисправность платы. Независимый ngspice проверяет уравнения; два свежих полных повтора команды совпали при времени около 10,2 секунды без обращений к модели. Далее — установить реальные границы AON и вспомогательных токов, проверить нагруженную fault-линию, запуск и динамическую защиту перед перестроением native. Существующие проблемы питания остаются видны; кандидаты MPN не принимаются. Рабочая трассировка приостановлена. Это внутренний шаг H6.0.3, не закрытие H6 и не разрешение изготовления.
