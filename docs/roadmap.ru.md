...
> **▶️ Текущая аппаратная граница: H1-R2.1 — реестр корпусов и инкрементальное внутреннее размещение.**
...
| Физический дизайн устройства | ▶️ [H1-R2.1](h1-r2-physical-layout.ru.md): Hub, Airband, резерв FPV и MMCX размещены; продолжается закрытие остальных корпусов и rail/thermal |
| Принципиальные диаграммы на сайте | Функциональная карта H0-R2 и внутреннее размещение H1-R2.1 актуальны; полные размерные виды R1 явно обозначены входами для пересборки |
...
| Пересечение с прошивкой | ▶️ firmware F0-R2.0 заменяет five-image boundary R1 шестью доменами и новыми Hub/Airband-контрактами |
...
## Текущий H1-R2.1 и сохранённое evidence R1

<!-- current-substep: H1-R2.1 -->

**Точный маркер: `H1-R2.1`** — [инкрементальный физический результат](h1-r2-physical-layout.ru.md)
размещает новый Hub, тракт Airband, декодер/резерв приёмника аналогового FPV и
точный боковой MMCX в принятой системе координат. Генерируемый аудит показывает
ноль коллизий корпусов на одной стороне и минимум 2,44 мм между встречными
проекциями. Далее H1 закрывает остальные точные корпуса и rail/thermal envelope
3,5 А/4,0 А, затем пересобирает все внешние/внутренние виды и разрезы. ECAD R2
...
Текущая граница — `H1-R2.1`. Второй Hub RP, точные активные детали Airband,
декодер/резерв приёмника FPV и боковой MMCX прошли инкрементальный аудит
корпусов. Далее закрываются остальные точные корпуса и полная модель same-face,
cross-board, antenna, U214, battery, service, rail и thermal. Исторический
rail-лимит R1 2,5 А должен стать проверенным контрактом ≥3,5 А continuous /
≥4,0 А step. PCB routing, quote/reservation и любой заказ остаются заблокированы.
