...
| Электрические и переходные evidence | ▶️ H3.5.2: девять антенных трактов проверены; выводятся RF layout constraints |
...
**Точный маркер: `H3.5.2`** — [контракты всех девяти антенных трактов](rf-feed-constraints.ru.md)
...
| **H3. Виртуальная электрическая проверка** | ▶️ Сейчас · `H3.5.2` | Расчёты и симуляции до дорогой физики | Проходят worst-case DC budget; startup/shutdown, USB↔battery handover, brownout, watchdog, eFuse и load-step; thermal/fault tree; все analog corners; timing/levels; RF corridors, returns и pre-layout constraints |
