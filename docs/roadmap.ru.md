...
координат и внешние направления всех корпусов, используемых мокапом. Более
поздние виды уже существуют как черновые проекции, но ни один поздний подэтап
не считается проведённым, пока открыт этот gate исходных данных.
...
- `H1.1` — реестр физических первоисточников.
  ...
  - ▶️ **`H1.1.2` — сейчас:** для каждого отрисованного корпуса сверить
    опубликованные или evidence-backed `L×W×H`, начало координат, ориентацию и
    внешнее направление connector/actuator.
  - ⏳ `H1.1.3` — классифицировать каждую неопределённость как blocker H1 или
    явный gate полученного образца H5; inferred-размер не становится точным
    молча.
  - ⏳ `H1.1.4` — зафиксировать таблицу размеров и источников renderer.
...
- ⏳ `H1.3.0` — сгенерировать из единого source внешние стороны: экран, D-pad,
  keys, encoder, LEDs, стрелки, внешние интерфейсы и видимую, ничем не
  перекрытую шелкографию.
- 🔒 `H1.3.1` — **пользовательское согласование:** принять целиком лицевую и
  заднюю стороны, включая подписи и места органов управления.
...
`H1.1.2` завершается, когда у каждого отрисованного корпуса есть прослеживаемый
source, размеры, datum, ориентация и направление интерфейса либо названный
sample-only gate без выдуманной геометрии. Закрытие любой подзадачи требует в
том же commit изменить точный маркер на стартовой странице и в роадмапе до
перехода к следующей работе. Поздняя правка, затрагивающая согласованный вид,
повторно открывает этот пользовательский gate и все зависящие от него gates;
раннее согласование не переносится молча.
