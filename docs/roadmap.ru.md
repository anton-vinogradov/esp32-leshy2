...
> **▶️ Текущий аппаратный этап: H2 — production ECAD-схема.** H0 и H1 прошли
> ревью. Работа над схемой начата; PCB layout и разрешённого заказа пока нет.
...
| Актуальная production ECAD-схема | ▶️ H2.2.10: UI manufacturing/test points; UI root и H2.2.2–H2.2.9 проверены |
...
- `H2.0` — зафиксировать авторитетные входы схемы и структуру проектов.
  - ✅ `H2.0.1` — проверен полный реестр из 997 строк: все 969 схемных
    экземпляров основного устройства, 26 общих компонентов LoRa Cap и 2
    альтернативных радиомодуля; сверены 181 тело H1 и 788 схемных компонентов.
  - ✅ `H2.0.2` — четыре проекта, границы плат, rails и net naming повторно
    сверены со всеми 997 строками; четыре намеренно пустых root/test-листа
    классифицированы явно.
  - ✅ `H2.0.3` — проверены HW↔FW/BSP export на 123 контакта и drift checks двух репозиториев.
...
- `H2.2` — реализовать и проверить листы UI/control PCB.
  - ✅ `H2.2.1` — `UI_00_ROOT`: девять дочерних листов, 90 точных межлистовых
    цепей и 216 явных pins/labels; одна прямая root-rail на цепь, без скрытых
    global labels, native parse KiCad пройден. После заполнения UI10–UI40
    пустых child-stubs не осталось; 376 предупреждений о копиях сгенерированных
    символов машинно учтены.
  ...
  - ✅ `H2.2.4` — `UI_12_CONTROLS_INDICATORS`: 71 точный компонент, 15 серийных
    кнопок B3S-1100P, 44 hierarchy-интерфейса, девять actual-TX LED, один
  ...
  - ▶️ **`H2.2.10` — сейчас:** `UI_60_TESTPOINTS_MANUFACTURING`.
...
любой подзадачи меняет точный маркер на стартовой странице и в роадмапе в том
же commit. Поздняя правка повторно открывает затронутые gates и их зависимости.
...
flowchart TD
  subgraph HW["Аппаратный роадмап — последовательно"]
    ...
    H2["▶️ H2<br/>production ECAD-схема"]
...
Текущая работа — H2: создать точную production-схему и HW↔FW-контракт из
принятого H1. PCB placement/routing и закупка остаются заблокированы.
