...
H5. Более поздние виды уже существуют как черновые проекции, но ни один
поздний подэтап не считается проведённым, пока открыт этот gate исходных данных.
...
- `H1.1` — реестр физических первоисточников.
  ...
  - ✅ `H1.1.2` — у каждого отрисованного корпуса есть evidence-backed
    ...
    connector/actuator.
  - ▶️ **`H1.1.3` — сейчас:** классифицировать каждую неопределённость как
    blocker H1 или явный gate полученного образца H5; inferred-размер не
    становится точным молча.
...
источников есть один явный статус, граница доказанного и именованный gate
закрытия; H1 blocker нельзя назвать sample-only, если от неизвестной геометрии
зависит мокап. Закрытие любой подзадачи требует в том же commit изменить точный
маркер на стартовой странице и в роадмапе до перехода к следующей работе.
Поздняя правка, затрагивающая согласованный вид, повторно открывает этот
пользовательский gate и все зависящие от него gates; раннее согласование не
переносится молча.
