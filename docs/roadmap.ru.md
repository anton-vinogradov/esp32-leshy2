...
| Электрические и переходные evidence | ▶️ H3.4.3: digital timing проверен; идёт loading M1/expansion/service |
...
**Точный маркер: `H3.4.3`** — [digital bandwidth, latency и timing](digital-timing-verification.ru.md)
...
| **H3. Виртуальная электрическая проверка** | ▶️ Сейчас · `H3.4.3` | Расчёты и симуляции до дорогой физики | Проходят worst-case DC budget; startup/shutdown, USB↔battery handover, brownout, watchdog, eFuse и load-step; thermal/fault tree; все analog corners; timing/levels; RF corridors, returns и pre-layout constraints |
