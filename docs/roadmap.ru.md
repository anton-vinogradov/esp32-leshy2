...
> **▶️ Текущая аппаратная граница: H1-R2.11 — K331 остаётся основным после поиска полнофункциональных fallback.**
...
| Физический дизайн устройства | ▶️ [H1-R2.11](h1-r2-physical-layout.ru.md): пиновая/силовая совместимость K331, точная TBS FPV-антенна и механика MMCX проходят; AWM666V остаётся деградированным контролируемым fallback, а полнофункциональные SP166RX/MM238R не проходят размерные/документарные/supply gates; сейчас H1 блокируют только контролируемый корпус и маршрут установки K331, затем пересобираются полные виды |
| Принципиальные диаграммы на сайте | Функциональная карта H0-R2, FPV/MMCX/размещение H1-R2.11 и диаграммы фильтра/rail H1-R2.4 актуальны; полные размерные виды R1 явно обозначены входами для пересборки |
...
## Текущий H1-R2.11 и сохранённое evidence R1

<!-- current-substep: H1-R2.11 -->

**Точный маркер: `H1-R2.11`** — [инкрементальный физический результат](h1-r2-physical-layout.ru.md)
...
минимум 1,44 мм между встречными проекциями. Генерируемая [проверка фильтра Airband](h1-airband-filter.ru.md)
...
тепло корпуса. Далее H1 закрывает остальные точные корпуса, затем пересобирает
все внешние/внутренние виды и разрезы. ECAD R2
...
Текущая граница — `H1-R2.11`. Второй Hub RP, точные активные детали Airband,
декодер/резерв K331, размерный боковой MMCX, точный LDO FPV и точная антенна
TBS5G8MMCXA прошли свои текущие функциональные/корпусные/фабричные границы.
Main-rail уже представляет размещённую архитектуру 3,75 А continuous /
4,25 А step со всеми перечисленными разрешёнными группами. Далее закрываются
остальные точные корпуса и пересобираются виды same-face, cross-board, antenna,
U214, battery и service. Динамика и тепло корпуса проверяются в H3. PCB routing,
