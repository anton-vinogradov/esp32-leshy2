...
> **▶ Текущая аппаратная граница: `H2-R2.0`.** H0 и H1 прошли ревью; принятый итог H1 — [`H1-R2.37`](h1-r2-acceptance.ru.md). H2 закрывает три электрических prerequisites до native R2 ECAD export.
...
| Production ECAD | ▶ `H2-R2.0`: до нового six-domain dual-RP R2 export должны закрыться live-маршрут FSUSB42MUX, точный detector/latch service-VBUS и powered-off-Ioff граница Pack/Safety; сохранённый G2F/H2/KiCad — только историческое evidence R1 |
...
## Текущий H2-R2.0 · точный состав

**Точный маркер: `H2-R2.0`.** H2 начат только для закрытия prerequisites;
native R2 ECAD export ещё не начинался.

- ▶ **Точная текущая точка:** проверить live-доступность Standard PCBA,
  маршрут, MOQ и цену onsemi `FSUSB42MUX` / JLCPCB `C11355`.
- ⏳ Выбрать и проверить точный устанавливаемый фабрикой service-VBUS detector/latch.
...
| **H2 · Production ECAD-схема** | **▶ Сейчас · prerequisites `H2-R2.0`** | Точные R2 symbols, contacts, nets, values, protection и footprints | Три electrical prerequisite закрыты; затем ERC-clean sheets и machine-readable HW↔FW contract |
...
Проверить live-маршрут Standard PCBA, MOQ и цену onsemi `FSUSB42MUX` / JLCPCB
`C11355`. Затем закрыть точный detector/latch service-VBUS и powered-off-Ioff
границу Pack/Safety I²C. Native R2 ECAD/KiCad, quote и любой заказ остаются
заблокированы.
