...
| Актуальная production ECAD-схема | ▶️ H2.3.3: допуск pack и always-on safety; H2.3.2 USB-PD/зарядка проверен |
...
- ▶️ `H2.3` — реализовать и проверить листы RF/power PCB.
  - ✅ `H2.3.1` — `RF_00_ROOT`: 12 дочерних листов, 133 точные межлистовые
    цепи и 301 явный pin/label; native KiCad принял иерархию, а текущие
  ...
  - ▶️ **`H2.3.3` — сейчас:** `RF_02_PACK_SAFETY_AON`.
