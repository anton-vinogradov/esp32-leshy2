...
> **▶ Текущая аппаратная граница: `H1-R2.32`.** H0 проведено ревью. H1 — нет.
...
**Точный маркер: `H1-R2.32`.** Это один подшаг физического дизайна, а не
закрытый отчёт H1.
...
- ✅ Все 43 существующих Cap/evidence-корпуса имеют точную текущую G2F-
  идентичность, source-envelope, координату и консервативный placement courtyard.
  Пропуск, повторная проекция или подмена останавливают генерацию.
- ▶ Завершить значения/MPN и courtyards вспомогательных пассивов, геометрию и
...
- ▶ После закрытия оставшихся U219 support-passive, pickup и антенных gates проверить и явно
...
| **H1 · Физический дизайн продукта** | **▶ Сейчас · `H1-R2.32`** | Внешний вид, отдельные внутренние стороны, разрезы, точные корпуса, RF-локальность, сервис и power-envelope | Нет коллизий bodies/fasteners/silkscreen/antennas/accessories/opposing sides; точный MPN или контролируемый reserve; мокап принят |
