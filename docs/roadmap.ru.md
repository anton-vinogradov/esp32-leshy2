...
> **▶️ Текущая аппаратная граница: H1-R2.0 — полная перекомпоновка после принятого H0-R2.**
...
| Физический дизайн устройства | ▶️ H1-R2.0: новые компоненты, межплатные зазоры, RF-тракты и питание компонуются вместе |
| Принципиальные диаграммы на сайте | Функциональная карта H0-R2 актуальна; размерные виды R1 явно обозначены входами для пересборки |
...
## Текущий H1-R2.0 и сохранённое evidence R1

<!-- current-substep: H1-R2.0 -->

**Точный маркер: `H1-R2.0`** — [отчёт H0-R2](h0-r2-functional-architecture.ru.md)
фиксирует новый функциональный вход. H1 пересобирает физическую карту деталей,
все межплатные проверки коллизий, rail-envelope 3,5 А/4,0 А и каждый внешний/
внутренний вид до разрешения ECAD R2.
...
Текущая граница — `H1-R2.0`. Она должна разместить второй Hub RP, boundary
FPV decoder и точные активные детали Airband, затем повторить same-face,
cross-board, antenna, U214, battery, service и thermal clearance. Параллельно
исторический rail-лимит R1 2,5 А заменяется проверенным контрактом ≥3,5 А
continuous / ≥4,0 А step. Фабричное evidence R1 можно переиспользовать, но PCB
routing, quote/reservation и любой заказ остаются заблокированы.
