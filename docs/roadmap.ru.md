...
- ⏳ `H1.3` — внешние стороны: экран, D-pad, keys, encoder, LEDs, стрелки,
  внешние интерфейсы и видимая, ничем не перекрытая шелкография.
- ⏳ `H1.4` — зеркальные внутренние стороны: каждый корпус, speaker,
  microphone, RUN/KILL, service controls и межплатный stack без внутренней
  шелкографии.
- ⏳ `H1.5` — настоящий вид от антенного торца и отдельные разрезы зоны U214 и
  battery/control с траекториями установки и обслуживания.
...
- ⏳ `H1.7` — повторная проверка pin/resource allocation по физическому
  результату и генерация единого acceptance package из того же source.
- ⏳ `H1.8` — явная пользовательская приёмка; только после неё начинается H2.
