...
24. ▶ `H6.0.2-R1`: [fail-closed политика трассировки](h6-r2-routing-policy.ru.md) распределяет все 823 локальных для плат экземпляра net / 789 глобальных канонических сетей, разрешает автопредложения только для обычного низкоскоростного управления, фиксирует текущую RF/USB-геометрию `JLC06161H-3313` и представляет все 12 USB-сегментов нативными парами KiCad `_P/_N`; принятый срез `GENERAL_CONTROL` + `OSCILLATOR` + `SAFETY_CONTROL` закрывает 288 сетей / 652 соединения без нарушений DRC, а аналоговые/audio/sense-сети остаются текущей работой без нарушения H6.0.1.
