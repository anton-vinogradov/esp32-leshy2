...
> **▶️ Текущая аппаратная граница: H1-R2.14 — K331 остаётся основным после поиска полнофункциональных fallback.**
...
| Физический дизайн устройства | ▶️ [H1-R2.14](h1-r2-physical-layout.ru.md): все четыре USB смотрят вниз, восемь утопленных recovery-кнопок используют единый точный визуальный контракт, а неизменённая RF-топология помещает четыре SMA на UI и шесть SMA плюс верхний MMCX FPV на RF; H1 блокирует только единый контролируемый production-пакет AKK |
| Принципиальные диаграммы на сайте | Функциональная карта H0-R2, полные физические виды и FPV/MMCX-тракт H1-R2.14, а также диаграммы фильтра/rail H1-R2.4 актуальны; сгенерированные физические виды явно остаются in progress до фиксации границы K331 |
...
## Текущий H1-R2.14 и сохранённое evidence R1

<!-- current-substep: H1-R2.14 -->

**Точный маркер: `H1-R2.14`** — [текущий физический результат](h1-r2-physical-layout.ru.md)
...
Текущая граница — `H1-R2.14`. Второй Hub RP, его четвёртый независимый тракт
USB/RESET/BOOT/DBG10, точные активные детали Airband, декодер/резерв K331,
размерный MMCX на верхнем антенном торце, точный LDO FPV и точная антенна TBS5G8MMCXA прошли свои
