...
> **▶️ Текущая аппаратная граница: H1-R2.12 — K331 остаётся основным после поиска полнофункциональных fallback.**
...
| Физический дизайн устройства | ▶️ [H1-R2.12](h1-r2-physical-layout.ru.md): сгенерированы полные текущие внешние, зеркальные внутренние, сервисные, антенные виды и разрезы; восстановленный тракт Hub даёт всем четырём вычислителям независимые USB, RESET/BOOT и DBG10; H1 всё ещё блокируют только контролируемый корпус и маршрут установки K331 |
| Принципиальные диаграммы на сайте | Функциональная карта H0-R2, полные физические виды и FPV/MMCX-тракт H1-R2.12, а также диаграммы фильтра/rail H1-R2.4 актуальны; сгенерированные физические виды явно остаются in progress до фиксации границы K331 |
...
## Текущий H1-R2.12 и сохранённое evidence R1

<!-- current-substep: H1-R2.12 -->

**Точный маркер: `H1-R2.12`** — [текущий физический результат](h1-r2-physical-layout.ru.md)
...
`TBS5G8MMCXA`. K331 остаётся физическим резервом до появления контролируемых
максимальных размеров и фабричного маршрута JLCPCB либо явной установки после PCBA. Точные
запросы отправлены AKK и JLCPCB 27 августа 2026 года; оба ответа ожидаются и
заранее не закрывают ни один gate.
...
Это два текущих блокера H1. Характеристики собранного RF/video-тракта и запасная
...
пока не перестанут меняться контролируемый корпус K331 и маршрут его установки.
...
Текущая граница — `H1-R2.12`. Второй Hub RP, его четвёртый независимый тракт
...
контролируемые максимальный корпус/посадочные данные K331 и закрыть его JLCPCB
private/global-sourcing либо явный маршрут установки после PCBA; после этого те
же сгенерированные виды можно перевести в reviewed. Динамика и тепло корпуса
