...
| Актуальная production ECAD-схема | ▶️ H2.3.10: динамик, микрофон и voice-audio amplifier; H2.3.9 с задними органами управления проведено ревью |
...
**Точный маркер: `H2.3.10`** — реализовать и проверить динамик, направленный
вниз микрофон и усилитель voice-audio на `RF_36_AUDIO_IO_AMP`.
...
- `H2.0` — зафиксировать авторитетные входы схемы и структуру проектов.
  - ✅ `H2.0.1` — проверен полный реестр из 1 021 строки: все 993 схемных
    экземпляров основного устройства, 26 общих компонентов LoRa Cap и 2
    альтернативных радиомодуля; сверены 181 тело H1 и 812 схемных компонентов.
...
- ▶️ `H2.3` — реализовать и проверить листы RF/power PCB.
  ...
  - ▶️ **`H2.3.10` — сейчас:** `RF_36_AUDIO_IO_AMP`.
