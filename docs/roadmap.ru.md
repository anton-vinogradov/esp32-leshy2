...
24. ▶ `H6.0.2-R1`: [fail-closed политика трассировки](h6-r2-routing-policy.ru.md) распределяет все 858 локальных экземпляров net / 823 канонические сети, разрешает автопредложения только для обычного низкоскоростного управления, фиксирует текущую RF/USB-геометрию `JLC06161H-3313` и представляет все 12 USB-сегментов нативными парами KiCad `_P/_N`; развести все сети, доказать native schematic/PCB parity и получить DRC без замечаний, не нарушив H6.0.1.
