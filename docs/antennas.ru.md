...
и 12 физических антенн. Десять можно подключить одновременно, а для `SUB-GHz`
...
профилю 433 МГц, а второй — к UHF voice. Оба экземпляра устанавливаются постоянно:
перенос одной антенны между портами запрещён, потому что ошибочный выбор меню может
включить передатчик без нагрузки или с антенной не того диапазона. Два одинаковых
экземпляра уменьшают количество разных SKU без такого safety-риска.
...
Для каждого предмета выбран заказываемый резервный кандидат:
...
Первый и резервный варианты заданы для всех 12 предметов; 11 резервных
