...
The existing MAIN variant command now couples **feedback, series losses and monitor thresholds**. All three selected voltage-feasible resistor pairs fail the necessary ideal-divider screen for the reviewed monitors. A separate optimistic continuous-ratio ceiling prevents confusing one rejected pair with rejection of every possible pair: only the zero-eFuse-loss hypothesis leaves a TPS389001 static window in this seven-case portfolio. This is not a constructed or qualified circuit. [Reviewed sources](../hardware/verification/h6-main-monitor-sources.json) retain threshold tolerances, hysteresis and timing limitations; TPS389001 has no published maximum falling delay, and TPS3808EG01's 50-us maximum requires 5% overdrive and specified loading. Next, synthesize the full power cell against these coupled requirements, not voltage alone.

`python tools/compare_main_current_limit.py` in the same prepared EDG environment compares fitted **1.65 kΩ**, H1's intended **1.18 kΩ**, and automatically selected nominals using current native provenance and regenerated reports. It preserves all eight current power findings and the 4.25-A lower / strict 6-A upper requirements. Under the separately named solder/load-life stress example, 1.18 kΩ misses the lower limit. The first-fit comparison is retained, but a complete finite search of the pinned EDG E192 table now maximizes the worst current margin across both named budgets: **1.13 kΩ gives 148.3 mA**, versus **16.7 mA** for first-fit 1.10 kΩ. This optimum covers only the declared model and domain, not unknown physics or a production MPN. No schematic, PCB or firmware change. Both commands need no LLM calls, establish/release their own `caffeinate`, and retain exit 1 (`not_qualified`). **192 selected tests pass, no skips**; independent enumeration/corner arithmetic confirms the optimum, and two fresh current-limit runs reproduce the same results and 96 source hashes. [Measured timings, historical results and preparation-cost limits](../hardware/layout/benchmarks/2026-09-20-power-cell-coupling-results.json).
