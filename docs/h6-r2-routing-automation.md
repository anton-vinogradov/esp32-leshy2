...
Development proceeds as **mechanism → current project → independent validation**, repeating that loop before advancing. Acceptance is consolidated, not complete electrical correctness. The isolated generator work now has a reusable [MAIN feedback command](../tools/synthesize_main_feedback.py): run `python tools/synthesize_main_feedback.py` inside the prepared EDG 0.5.2 environment. Exact inverse bounds include initial tolerance and TCR; the existing EDG E192 selector proposes a nominal and separate exact forward arithmetic checks it. The command regenerates the H3 report in memory, rejects stale or altered evidence and writes a fresh `work/feedback-synthesis-*/result.json`. Exit 1 means conditional analysis **not qualified**, exit 2 an execution/evidence error; no PCB or MPN changes.

Current result: 58 MAIN load cases, including the 4.25-A declared resistive snapshot, require an average output of 3.2725–3.29 V. With the fixed bottom resistor and conditioned source model, the top nominal would need to be both ≥45.702 kΩ and ≤43.705 kΩ: no solution in this model. A raw-only diagnostic selects 43.2 kΩ but is explicitly **not** a solution for the complete requirements. Twenty-seven focused tests pass, including missing evidence, bad candidates and exact-decade EDG boundaries; three fresh processes reproduced the same semantic result in 0.197–0.204 s. VFB applicability at actual VIN, leakage, lifetime, ripple, hot RON, dynamics, current admission and physical behavior remain unqualified. There are no LLM calls inside this command; preparation cost and parent-token savings remain unmeasured. Next: use the diagnostic to compare power-model alternatives without silently relaxing requirements; part changes still require independent qualification and supply checks.
