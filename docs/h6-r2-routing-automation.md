...
`python tools/compare_main_current_limit.py` in the same prepared EDG environment compares fitted **1.65 kΩ**, H1's intended **1.18 kΩ**, and automatically selected nominals using current native provenance and regenerated reports. It preserves all eight current power findings and the 4.25-A lower / strict 6-A upper requirements. Under the separately named solder/load-life stress example, 1.18 kΩ misses the lower limit; EDG's first feasible preferred value, 1.10 kΩ, leaves only **16.7 mA** below the upper limit. It is explicitly **not a margin-optimized production recommendation**. No MPN, schematic, PCB or firmware change. Both commands need no LLM calls, establish/release their own `caffeinate`, and retain exit 1 (`not_qualified`). **185 selected tests pass, no skips**; two cold runs per command reproduce the same numerical results and source hashes. [Measured timings, source review and preparation-cost limits](../hardware/layout/benchmarks/2026-09-20-power-cell-coupling-results.json).
