# H1-R2.33 · рабочая компоновка целевого устройства
...
- FPV MMCX: корпус оставляет `2.07 мм` до ближайшего SMA; контролируемый угловой штекер — `2.40 мм` до SMA и `4.80 мм` до общего Cap-Bus-слота. H5 фиксирует geometry/sequence до заказа; Ø12 finger access, fit и retention проверяются H7/H8 после получения.
...
> Точный текущий маркер: **H1-R2.33**. H1 продолжается.
