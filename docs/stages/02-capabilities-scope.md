...
- `DEC-0007`: бортового LoRa нет; поддерживаются U214 и сменная wideband LoRa carrier через `EXT-RF14`, один активный backend;
...
`IMP-0007` выявил, что готовый U214 функционально близок к legacy SX1262 и добавляет GNSS, но его официальное окно `868–923 MHz` уже legacy E22 `850–930 MHz`. `DEC-0007` не распространяет это ограничение на весь продукт: для сохранения диапазона принят второй профиль с wideband E22 carrier; конкретный U214 работает только в собственном заявленном окне.

Владелец принял оба внешних варианта через `DEC-0007`: U214 остаётся первым готовым LoRa+GNSS backend, а wideband E22 carrier сохраняет 850–930 MHz через тот же `EXT-RF14`. Активен один LoRa backend; blanket-совместимость со всеми Cardputer Caps или UART LoRaWAN Units не обещается. Scope-подшаг прошёл `REV-0002C`.
