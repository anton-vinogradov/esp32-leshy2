...
Consumer IR prerequisite audit прошёл `REV-0002R`. `FND-0017` снял ложную `FAB-READY` пометку IR artifact и добавил Q58 base-emitter pull-down, но C5 ownership, exact emitter/driver/current, STOP, TX-state и optical HIL остаются открыты. `FND-0018` доказывает, что fixed `TSOP38238` выдаёт demodulated 38 kHz envelope и не измеряет carrier. `REQ-IR-0001` декомпозирует own remote/replay в Main, passive analysis в Lab, а unknown replay и TV-B-Gone/brute-force sweep — в Controlled Zone; mass sweep требует `BOTH`. **⚠️ Предложение `IMP-0015`** сравнивает dual `TSOP38238+TSMP95000` path, single-learning receiver и честный fixed-38 baseline; requirement set остаётся **«На ревью»** до выбора владельца.
