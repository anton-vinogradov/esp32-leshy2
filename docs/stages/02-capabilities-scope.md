...
`IMP-0007` предлагает вместо бортового E22 и отдельного GPS Unit один внешний M5Stack Cap LoRa-1262 `U214`. Он функционально близок по SX1262 и добавляет GNSS, но официальное окно `868–923 MHz` уже legacy E22 `850–930 MHz`. Поэтому scope не меняется автоматически: требуется решение владельца об обязательных региональных профилях, затем proof Cap-Bus/power/RF/mechanics.

`IMP-0008` расширяет предложение до модульного `EXT-RF14`: U214 остаётся первым готовым LoRa+GNSS backend, а wideband E22 carrier сохраняет 850–930 MHz через тот же слот. Это рекомендуемая архитектура, но она не считается принятой до решения владельца и не обещает blanket-совместимость со всеми Cardputer Caps или UART LoRaWAN Units.
