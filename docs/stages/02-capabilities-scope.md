...
3×nRF24 prerequisite audit прошёл `REV-0002T`. `FND-0019` показывает, что current `FAB-READY` claim был ложным: SPI/CE/CS/IRQ всё ещё принадлежат S3, modules являются generic 2×4 placeholders, а `TXDET_NRF1..3` не имеют RF detector source. Маркировка исправлена, общий CE получил 100 kΩ pull-down, но transport/pins/module/STOP/TX-live/RF proof открыты; после dual-path IR старый C5 pin budget дополнительно устарел. Владелец принял `IMP-0016/A` как `DEC-0019`: три одновременных тракта дают calibrated binary RPD hit-rate sector comparison с полными sampling/calibration metadata, но не RSSI/dBm/bearing/VSWR; real-power hardware остаётся отдельным будущим расширением. `FND-0020` закрыт на requirement-level. `FND-0021` разносит passive ESB/MouseJack discovery, sensitive KeySniffer capture, active injection, address brute-force и interference tests по Lab/Controlled Zone; jam/carrier/sweep доступны только `BOTH` в conducted/RF-shielded setup. GPL reference code не выдан за MIT reuse. Распространение прошло `REV-0002U`, `REQ-N24-0001` получил статус **«Проведено ревью»**. `FND-0019` и `FND-0021` остаются implementation gates. **⚠️ Предложение `IMP-0017`** направляет ordinary BLE на native ESP backend, оставляя nRF24 лишь limited compatibility, но ждёт отдельного BLE-owner review.
...
Native BLE prerequisite audit прошёл `REV-0002X` и сформировал draft `REQ-BLE-0001`, не меняя целевой README до решения владельца. S3 и C5 оба покрывают legacy ordinary BLE baseline, но C5 делит один 2.4 GHz radio с уже принятым Thread/Zigbee; `IMP-0019/A` поэтому рекомендует единственного S3 owner, C5 BLE default-off и лишь limited compatibility/research BLE path через nRF24. `FND-0026` отделяет advertising scan от недоступного штатному controller passive connection-follow, rotating identity и ложных метров/направления по RSSI. `FND-0027` требует corpus/spec/licence/peer proof и разносит ordinary GATT/HID/beacons, passive Lab и disruptive Controlled Zone `BOTH`. Dedicated nRF52 connection sniffer остаётся отдельным `IMP-0004`. Найденная, но не запрошенная Bluetooth Mesh функция явно зафиксирована как **⚠️ Предложение `IMP-0020`** и не включена до отдельного согласия после ownership.
