...
- `FND-0004`: legacy-текст объявляет GPS внешним, но актуальный legacy tsCircuit всё ещё содержит бортовой SAM-M8Q и не содержит обещанного `J_GPS`.
...
Решение по `FND-0004`, завершённая матрица `AUD-0001`, затем декомпозиция групповых кандидатов и пофункциональное решение `include` / `conditional` / `defer` / `exclude-proven` с проверяемыми гейтами и стоимостным драйвером.
