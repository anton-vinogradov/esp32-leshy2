...
По `DEC-0005` начат `AUD-0002`. Исходный схемный кандидат `IMP-0006` предлагал удалить третий PCA9555 и занять `U13` matrix+STOP+touch, но после `DEC-0009` эта pin-map признана конфликтующей. Переработанный `IMP-0010` сохраняет идею matrix, выносит STOP в независимую hardware safety-chain и перераспределяет audio-control; предложение не принято до решения владельца и UI/safety proof.
