...
- Статус: **Проведено ревью (`REV-0002AD`)**
...
- `DEC-0001`: IR остаётся C5; ранее принятое владение C5 для 3×nRF24 переоткрыто владельцем после `FND-0028`/`AUD-0003` и ждёт **⚠️ `IMP-0021`**;
...
- `DEC-0019`: 3×nRF24 hunt использует калиброванное сравнение бинарной RPD hit-rate по секторам без dBm, угла и VSWR;
- `DEC-0020`: OpenThread — открытый C5 baseline, Zigbee — optional conditional adapter; оба делят C5 2.4 GHz radio и не обещаются одновременно;
- `DEC-0021`: S3 — единственный baseline native-BLE owner, C5 BLE default-off; это не сокращает полный native nRF24 feature set;
- `DEC-0022`: сначала закрывается единый полный реестр хотелок и его owner freeze; MCU ownership, transport, GPIO и layout сравниваются только затем;
- `DEC-0023`: `INV-0004` заморозил 125/125 leaf-функций; MCU ownership, transport, GPIO и layout теперь являются входом этапа 3;
...
3×nRF24 prerequisite audit прошёл `REV-0002T`. `FND-0019` показывает, что current `FAB-READY` claim был ложным: SPI/CE/CS/IRQ всё ещё принадлежат S3, modules являются generic 2×4 placeholders, а `TXDET_NRF1..3` не имеют RF detector source. Маркировка исправлена, общий CE получил 100 kΩ pull-down, но transport/pins/module/STOP/TX-live/RF proof открыты. Владелец принял `IMP-0016/A` как `DEC-0019`: три одновременных тракта дают calibrated binary RPD hit-rate sector comparison с полными sampling/calibration metadata, но не RSSI/dBm/bearing/VSWR. `FND-0020` закрыт; `FND-0021` разводит passive discovery, sensitive capture, active exploitation и RF interference по Lab/Controlled Zone. Capability-контракт прошёл `REV-0002U`, но требование владельца о полном native feature set переоткрыло physical ownership. `REV-0002Z`/`AUD-0003` сравнили предварительные S3/C5 варианты; `DEC-0023` теперь требует минимум три полные компоновки и один сводный demand model до решения.

C5 Wi-Fi/IEEE 802.15.4 prerequisite audit прошёл `REV-0002V`. Legacy N8R4/`C49308183` source candidate заменён на current-standard N8R8/`C51950748`, а ложный `ANT2=u.FL` исправлен: штатный off-board path использует module-integrated `ANT1`, disabled `ANT2` остаётся неподключённым (`FND-0022`). Public Wi-Fi baseline умеет promiscuous management/control/data RX, но raw TX ограничен beacon/probe/action и non-QoS data; deauth/disassoc требует отдельного version-locked patched-binary решения, а `AUTO` не означает simultaneous dual-band (`FND-0023`). `FND-0024` добавляет country/DFS/PMF/privacy gates и запрещает DFS SoftAP. Passive-only ceiling 802.15.4 опровергнут: raw ED/CCA/RX/TX, OpenThread и Zigbee roles технически доступны без нового RF BOM, но делят один C5 2.4 GHz RF path, а официальный Zigbee core proprietary prebuilt (`FND-0025`). Владелец принял `IMP-0018/A` как `DEC-0020`: OpenThread — открытый baseline, Zigbee — optional conditional adapter, ordinary network — Main, passive raw — Lab, active tests — Controlled Zone. Распространение прошло `REV-0002W`, `FND-0025` закрыт на requirement-level, а `REQ-W5-0001` получил статус **«Проведено ревью»**. `FND-0022`–`FND-0024`, transport/STOP, binary lifecycle и coexistence HIL остаются implementation gates; `IMP-0003` и private patched Wi-Fi backend не приняты автоматически.

Native BLE prerequisite audit прошёл `REV-0002X`. Владелец принял `IMP-0019/A` и уточнённый `IMP-0017` как `DEC-0021`: S3 — единственный baseline native-BLE owner, C5 BLE default-off. Ограничен только дополнительный legacy-1M BLE-compatible subset nRF24; полный native nRF24 feature set сохраняется. `FND-0026` отделяет advertising scan от недоступного штатному controller passive connection-follow, rotating identity и ложных метров/направления по RSSI. `FND-0027` требует corpus/spec/licence/peer proof и разносит ordinary GATT/HID/beacons, passive Lab и disruptive Controlled Zone `BOTH`. Распространение прошло `REV-0002Y`, `FND-0002` закрыт, `REQ-BLE-0001` получил статус **«Проведено ревью»**. Dedicated nRF52 connection sniffer и Bluetooth Mesh сохранены `DEC-0023` как optional `defer-release`; они не входят в base BOM/core release.
...
## Итог и следующий артефакт

`INV-0004` проверяет 125/125 кандидатов, декомпозирует две смешанные extra-строки и фиксирует base/optional/deferred boundaries. `DEC-0023` замораживает wishlist, `REV-0002AD` закрывает этап 2. Следующий артефакт — единый demand model этапа 3; только после него сравниваются `IMP-0010`, `IMP-0021`, transports и pin maps.
