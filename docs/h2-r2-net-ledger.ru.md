...
сверяет входные данные для native-схем; он ещё не создаёт три проекта KiCad и
не разрешает placement, routing, печать или заказ.

## Результат

| Элемент | Проверенный результат |
|---|---:|
| Контакты текущих устанавливаемых экземпляров | 4 053 |
| Контакты, назначенные native nets | 3 814 |
| Явные board no-connects | 239 |
| Неразрешённые или скрытые внешние контакты | 0 |
| Канонические native nets | 908 |
...
Для переноса неизменившихся цепей обвязки использованы 3 369 подсказок
same-endpoint route из сохранённого контракта G2F R1 и ещё 55 подсказок
identical-device/same-pin из сохранённых KiCad-файлов R1. Остальные 151
...
- [Сгенерированный ledger 4 053 endpoints](../hardware/ecad/generated/H2-R2-native-net-ledger.json)
- [Генератор](../hardware/ecad/h2_r2_net_ledger.py)
- [Машинные тесты](../hardware/architecture/tests/test_h2_r2_net_ledger.py)

Текущее действие остаётся **`H2-R2.1.3`**: материализовать эти проверенные
definitions, references и nets в трёх native-проектах KiCad. ERC, placement,
