# Сводный результат thermal, fault и длительной работы · historical R1

H3.6 закрыт: три leaf-пакета дают `70` проходящих checks, а сведение добавляет `24` сквозных checks. Все `30` single-fault сценариев заканчиваются containment или запретом допуска; незакрытых аналитических findings и решений нет. Исторический маркер прогресса R1 — `H3.7.1`.
