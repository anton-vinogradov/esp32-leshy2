...
|---|---|
| Native-проекты | `LESHY2-UI-R2`, `LESHY2-RF-R2` |
| Функциональные sheets | 22 уникальных sheets с одним владельцем каждой ответственности |
| Вычислительные домены | 6: S3, C5, передний Hub RP, задний RF RP, Pack и Safety |
| Точные группы компонентов | 238 MPN-групп, 1 193 позиций на готовое устройство |
...
В принятом H1 — 210 сгруппированных строк BOM и 1 094 устанавливаемые базовые
позиции. Проверенная граница Pack/Safety, полный точный тракт Airband и прямой
ZIF дисплея дают текущий электрический итог: 238 точных групп и 1 193 позиции. Display, опциональный U214,
съёмные аккумуляторы и ручка энкодера сохраняют явные
non-PCBA/final-assembly dispositions.
...
Точный ledger этих 238 групп прошёл ревью как
[`H2-R2.1.2`](h2-r2-symbol-footprint-ledger.ru.md), а следующая
[сверка 4 239 endpoints](h2-r2-net-ledger.ru.md) и
[генерация native KiCad](h2-r2-native-kicad.ru.md) также пройдены. Сверка sheets
и HW↔FW также прошла в [H2-R2.1.5](h2-acceptance.ru.md). Теперь H3 фиксирует эти
входы; PCB placement, routing, печать и заказ остаются заблокированы.
