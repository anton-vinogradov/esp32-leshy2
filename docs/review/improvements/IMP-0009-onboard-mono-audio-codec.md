...
- codec enable и два selector control можно разместить на свободных медленных линиях `U13.P10..P17`.
...
Принять, отклонить или отложить бортовой ES8311 с аппаратным analog bypass как целевое закрытие `FND-0003`. До решения схема codec не добавляется, а связанные capability остаются `BLOCKED`.
