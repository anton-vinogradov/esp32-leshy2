...
- Этап решения: 2 — safety/scope; схемная реализация и proof — этапы 3–9
...
Два пробела можно закрыть одним контрактом: убрать STOP из GPIO-бюджета и сделать его независимым hardware path.
...
Конкретное распределение трёх audio-control между этими линиями остаётся выходом этапа 3. Настоящее предложение принимает ресурсный контракт, а не окончательную нумерацию net/pin.
...
7. Pin audit подтверждает отсутствие повторного назначения после удаления onboard LoRa и реализации `DEC-0009`.
