...
- Статус: **⚠️ Layout-кандидат; решение после wishlist freeze и сводного demand model (`DEC-0012`, `DEC-0022`)**
...
По `DEC-0012` и `DEC-0022` варианты ниже сейчас не выносятся владельцу. Сначала замораживается полный реестр хотелок `INV-0002`; затем предложение сравнивается внутри нескольких полных компоновок на сводном demand model: прямые GPIO S3/C5, expander lines, fixed-function/strap pins, S3↔C5 transport, audio, UI/touch, внешние модули и фактически освобождённые onboard GNSS/LoRa ресурсы. До этого `U14`, matrix и конкретная STOP-реализация не считаются выбранными.
