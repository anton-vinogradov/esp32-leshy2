...
- Статус: **Принято владельцем как `DEC-0021`**
...
- nRF24 сохраняет полный native feature set; ограничен лишь conditional legacy-1M BLE-advertising compatibility subset по `IMP-0017`, и это не второй BLE controller;
...
Вариант A принят: S3 — единственный baseline native-BLE owner, C5 BLE выключен по умолчанию. Полный native nRF24 scope сохраняется; ограничен только BLE-compatible subset.
