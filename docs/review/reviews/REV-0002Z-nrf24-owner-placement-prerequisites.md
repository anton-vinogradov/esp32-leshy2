...
Пререквизиты проведены ревью. `DEC-0001` остаётся исторически принятым, но его nRF24 ownership переоткрыто и не может считаться окончательным входом stage 3. Рекомендуется `IMP-0021/A`: S3 владеет всеми тремя полнофункциональными nRF24, IR остаётся C5; exact CE latch, UART-bridge removal и pin map проходят отдельный proof.
