...
| Reset safety | direct AON strap is always-on; three TPS564252 external 10-kOhm pulls are stronger than their 2-MOhm internal defaults |
...
| Machine source | nine separate physical resistors and exact two-terminal routes replace the abstract control networks |
| Generated/target diagrams | vertical atlas and both product landing diagrams show every new physical instance separately |
| BOM/cost | three already-used MPNs; no new feeder line; approximately `$0.006` checked material per board |
| Firmware | no new API or timing constant; fixed hardware truth table remains the runtime input |
