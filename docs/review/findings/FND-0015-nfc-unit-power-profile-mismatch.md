...
3. Выбор fixed port, jumper либо управляемого per-port power profile выполняется только в сводном pin/power/resource budget этапа 3; он не должен молча ломать generic Grove profile.
