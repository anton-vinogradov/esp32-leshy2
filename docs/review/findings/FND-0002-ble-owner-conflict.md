...
- Статус: **Закрыто на уровне требований (`DEC-0021`)**
...
Владелец принял `IMP-0019/A` как `DEC-0021`: S3 — единственный baseline native-BLE owner, C5 BLE выключен в обычном profile, а будущая C5-only функция требует отдельного требования. `REQ-BLE-0001` получил статус **«Проведено ревью»**. Выбор не сокращает native nRF24: ограничен только дополнительный BLE-compatible subset, а физический nRF24 owner переоткрыт отдельно в **⚠️ `IMP-0021`**.
