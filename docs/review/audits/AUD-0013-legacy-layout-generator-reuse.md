...
| один generic `Si4732` SMA | электрически неполон: exact device выводит разные `FMI` FM/SW и `AMI` AM/LW domains; `DEC-0049` требует два labelled ports в active adapted generator |
...
После этого source review владелец отдельно принял `DEC-0048`: внешний
SMA-bank является target invariant. Это сохраняет принцип макета, но не его
старое число портов, owners, порядок или generic module footprints.
