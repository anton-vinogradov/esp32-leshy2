...
| Модульный LoRa | совместимый с U214 внешний `EXT-RF14` и сменные radio backends | base device теряет E22/RF path; полная цена зависит от модуля | connector/power/mechanics и firmware abstraction; reference carrier ещё проектируется | `IMP-0008`; рекомендуемый способ не сделать диапазон U214 потолком продукта |
...
7. Для `IMP-0008` сравнить котировки U214, E22 reference carrier и основной PCB с `EXT-RF14`; base BOM и полный комплект не смешивать.
