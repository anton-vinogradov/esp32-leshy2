...
| 3×nRF24, owner и hunt | exact identical qualified PA/LNA modules; S3 shared-SPI owner против C5+SDIO; calibrated RPD sector comparison; independent CE latch/direct CS/decoder после pin budget | S3 сохраняет текущую routing side и preliminarily уменьшает reroute/raw-IPC NRE; hunt по `DEC-0019` не добавляет measurement BOM | общий SPI требует measured latency/loss; C5 требует SDIO и плотный pin budget; 1 radio+RF switch или общий неразделимый CE теряют функцию | **⚠️ `IMP-0021/A`** рекомендован, но не zero-loss до pin/recovery/performance HIL; число три и полный native feature set сохраняются; `AUD-0003` |
