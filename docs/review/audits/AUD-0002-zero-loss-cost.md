...
| Native BLE owner | один baseline owner на S3; C5 BLE default-off; nRF24 только limited compatibility | новый BLE BOM и C5↔S3 BLE IPC не нужны; меньше power/coexistence/HIL cost | выбор ожидает `IMP-0019`; exact host stack, S3 Wi-Fi coexistence и cross-MCU RF arbitration требуют proof | `IMP-0019/A` рекомендован: он сохраняет весь legacy BLE baseline; C5-only/split не дают hardware-saving и увеличивают software/test cost |
...
`IMP-0010` предлагает вынести STOP из I²C в обязательный аппаратный TX-kill, а две линии `U12`, освободившиеся после удаления onboard LoRa, отдать audio-control. Тогда `U14` снова можно удалить, но чистая экономия определяется только вместе со стоимостью fail-safe gates и HIL. По `DEC-0012` сравнение не выносится на решение до сводного pin/GPIO/resource budget этапа 3. Safety-цепь не является необязательной «надбавкой ради экономии»: `FND-0007` показывает, что текущий STOP не гасит TX без исправных S3/I²C/firmware.
...
9. После решения `IMP-0019` считать не только нулевой BLE radio BOM, но и flash/RAM, active current, IPC, coexistence и HIL matrix; наличие radio в обоих MCU не делает dual-owner бесплатным.
