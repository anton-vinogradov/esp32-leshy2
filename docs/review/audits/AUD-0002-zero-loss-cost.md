...
`IMP-0010` предлагает вынести STOP из I²C в обязательный аппаратный TX-kill, а две линии `U12`, освободившиеся после удаления onboard LoRa, отдать audio-control. Тогда `U14` снова можно удалить, но чистая экономия определяется только вместе со стоимостью fail-safe gates и HIL. По `DEC-0012`/`DEC-0022` сравнение не выносится на решение до wishlist freeze, общего demand model и нескольких компоновок этапа 3. Safety-цепь не является необязательной «надбавкой ради экономии»: `FND-0007` показывает, что текущий STOP не гасит TX без исправных S3/I²C/firmware.
