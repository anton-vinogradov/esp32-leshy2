...
| `REQ-BLE-13` | `C-BLE-07`, `C-X-09` | `conditional` | Основной | If BLE is selected for the `DEC-0038` phone companion, it uses locally initiated/accepted pairing, allowlist, visible active peer, authenticated encryption/LE Secure Connections, local disconnect/revoke and stale/replay rejection. Remote input is character transport only and cannot confirm pledge, Controlled entry, TX, destructive/security, FIDO, recovery or firmware-trust actions. Missing phone blocks only the declared text-dependent workflow. |
