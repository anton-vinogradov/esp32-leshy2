...
| `REQ-X-03` | `C-X-04` | `conditional` | Сквозной IPC | S3↔C5 API typed, bounded, versioned, authenticated where needed and idempotent; link loss/reset/update cancels leases and TX. Конкретный transport выбирается на этапе 3 после сравнения layouts. |
...
| `REQ-X-08` | `C-X-09` | `conditional` | Основной | BLE phone keyboard uses explicit pairing/allowlist, visible connected peer, local revoke and on-device fallback. It cannot confirm pledge, Controlled entry, TX arm, destructive write or firmware trust decisions. |
