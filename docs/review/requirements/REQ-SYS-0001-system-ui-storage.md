...
| `REQ-SYS-02` | `C-SYS-02` | `conditional` | Основной | Selected local control surfaces collectively provide complete operation without a phone. Dedicated physical safety/voice controls remain where separately required; touch, keyboard, D-pad, encoder and action-key mix are G3 archetype variables, not simultaneous mandatory parts. Shortcut не является единственным путём; обычный выбор списка не запускает TX. Exact mechanics and pins wait for G3–G7. |
