...
`DEC-0007` дополнительно принимает комбинированный M5Stack Cap LoRa-1262 `U214`: при выбранном U214 backend его GNSS заменяет отдельный GPS Unit в конкретной конфигурации. Настоящий `PORT.C` сохраняется для отдельного Unit GPS v1.1 и wideband LoRa-конфигураций; одновременное GNSS sensor fusion не принято.
