...
| C-GPS-01 | Position/navigation, module config, time sync | MAIN | внешний модуль в legacy hardware |
...
- Ни один legacy ceiling не наследуется без повторной проверки; реалистичный обход оформляется как `IMP-*`.
