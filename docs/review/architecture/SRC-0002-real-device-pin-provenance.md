...
| codec | Everest Semiconductor `ES8311`, QFN-20 3×3 mm with exposed pad | current manufacturer brief plus user guide expose all 20 contacts and EPAD; S3 I2C/I2S fit is exact, `CE` is address strap and `MCLK` can be omitted under BCLK-derived-clock mode | `verified paper candidate`; production order code/AVL, external power switch, differential analog conditioning and HIL remain `open/blocking`, see `AUDIO-0001/FND-0065` |
