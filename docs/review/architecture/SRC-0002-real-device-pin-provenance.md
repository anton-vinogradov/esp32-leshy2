...
| three production nRF paths | 3×`E01-ML01IPX`; 3×`74LVC126APW,118`; 3×`74LVC2G126DC,125`; 3×`DC2337J5010AHF`; 3×`AD8314ACPZ-RL7`; exact power/match/termination passives | manufacturer documents expose every module pad, TSSOP14/VSSOP8 buffer contact, six-land coupler port and LFCSP8/EPAD detector contact; Ioff, 2400–2525-MHz coupling, detector response and separate forward/reverse ports are machine-routed | `DEC-0091/REV-0005AV` paper electrical endpoint reviewed; Ebyte mating-family specimen, authorized module lot/alternate, pigtail/SMA loss, thresholds and T1 coexistence remain `open/blocking` |
| CC1101 path | `CC1101RGPR`; 2×`74LVC126APW,118`; `74LVC2G126DC,125`; `ABM8-26.000MHZ-10-D-1-G-T`; `B0310J50100AHF`; 2×`BGS13SN8E6327XTSA1`; exact Murata branch passives; `SESD0402X1UN-0020-090`; `AD8314ACPZ-RL7` | official TI/Infineon/TTM/Abracon/Littelfuse/ADI documents expose every silicon, buffer, switch, balun, crystal, protection and detector contact; dual-ended RF1/RF2/RF3 routing, Ioff, 00 isolation and final-line evidence are machine-routed | `DEC-0093/REV-0005AX` paper endpoint reviewed; VNA tuning, sensitivity/output/spurious/legal profile, threshold/coexistence HIL and final mechanics-selected standard-SMA MPN remain `open/blocking` |
...
| broadcast receiver | Skyworks `Si4732-A10-GSR`, SOIC16 | manufacturer ordering guide and package drawing expose `FMI=6`, `RFGND=7`, `AMI=8`; block diagram assigns FM/SW to FMI and AM/LW to AMI | `verified candidate boundary`; full SOIC-16 map corrected by `FND-0102`; two input domains proven and two ports accepted `DEC-0049`; exact lifecycle/AVL/frontends remain open |
...
| IR RX/TX | exact `TSOP95238TT`, `TSMP95000TT`, `VSMY14940`, `VEMD1060X01`, `TLV9061IDBVR` and support devices | manufacturer documents expose both Heimdall 1/2/3/4 contacts, emitter/detector lands, switch/buffer/MOSF/op-amp contacts; complete filtered power, Ioff, current-limit, STOP-gate and optical-TIA routes are machine-instantiated | `DEC-0095/REV-0005AZ` paper endpoint reviewed; optical/thermal/IEC/coexistence HIL and TSOP procurement lead remain open |
| display/touch | exact paper assembly `HMX035CTFT-001`; first mate `FH12-40S-0.5SH(55)`; backlight `TPS2553DRVR-1`, `ERJ-P08F10R0V`, `DMN2056U-7` and exact support passives | official QDtech/ST77922/Hirose/TI documents expose all 40 panel contacts, connector contacts, switch pins, reset timing and current-limit range; machine routes separate every physical part and reject a whole-panel switch that could be signal-back-powered | `DEC-0084/REV-0005AO` paper electrical endpoint reviewed; standalone panel identity, real-tail connector fit, shared-SPI/touch/backlight HIL and optics remain blocking |
| microSD | active Hirose `DM3AT-SF-PEJM5`; `SN74LVC3G34DCUR`, `SN74LVC1G125DCKR`, 2×`TPD4E05U06DQAR`, existing `TPS22919DCKR` and exact support passives | official Hirose/TI/Espressif documents expose all socket/buffer/ESD/switch contacts, partial-power behavior, mandatory SPI-mode pulls and initialization order; machine routes separate every physical part and every exposed-contact/detect ESD channel | `DEC-0085/REV-0005AP` paper electrical endpoint reviewed; socket placement/access, media/endurance, shared-SPI throughput/contention, hot-remove/fault and filesystem recovery HIL remain blocking |
...
| slow control | `TCA6424ARGJR` UQFN32 leading reference; `TCA9535PWR` TSSOP24 smaller alternative | official TI package tables expose respectively 24 and 16 ports, open-drain INT and I²C; all ports power up as inputs | `G2F-3I` now accounts `24 used / 0 reserved / 0 free`: P03/P04 are rail-off CC band truth bits and P05 is native-Unit power request; see `DEC-0098` |
| exact external M5 boundary | `M5Stack U214 Cap LoRa-1262`, `TCA4307DGKR`, 3×`74LVC126APW,118`, `TXS0102DCUR`, 2×`TPS259470LRPWR`, 2×`TPS3808G33DBVR`, 4×`TPD4E05U06DQAR` and exact support parts | official M5/TI/Nexperia sources expose all 14 U214 contacts and all silicon contacts; machine routes independent U214/native-Unit power, readiness, nine non-I2C U214 signals, isolated I2C and two native Unit signals | `DEC-0098/REV-0005BD` paper endpoint reviewed; connector MPN/specimen, reverse-source, hot-plug, timing, profile and coexistence HIL remain blocking |
| independent service/recovery | 2×`USB4105-GF-A`, 2×`TPD2EUSB30ADRTR`, 2×`FSUSB42MUX`, 3×`FTSH-105-01-L-DV-K-P-TR`, 6×`SKQGADE010`, 3×`TPD4E05U06DQAR`, `SN74LVC1G06DCKR`, 2×`2N7002DW-7-F` and exact passives | official GCT/TI/onsemi/Samtec/Alps/Espressif/Raspberry Pi documents expose every receptacle, switch, header, protector and real compute contact; machine routes board-off-isolated C5/RP USB, all 30 DBG10 contacts, all 24 switch lands, straps and passive-drain resets | `DEC-0099/REV-0005BE` paper endpoint reviewed; received footprint/mechanics, USB SI/backfeed/ESD, fixture and erased-image recovery HIL remain blocking |
...
| non-programmable safety | `TPS3808G33DBVR`, `SN74LVC1G74DCUR`, `74LVC2G14GW,125`, 2×`74LVC1G32GV,125`, `SN74LVC1G06DCKR`, 2×`2N7002DW-7-F`, 2×`SN74LVC08APWR`; exact evidence devices in `SAFE-0002/devices.json` | official pinouts, Ioff/open-drain behavior, three passive-drain reset sinks, nine gate functions and eight evidence channels are machine-instantiated | `I2 reviewed DEC-0061/REV-0005O`; reset implementation corrected by `DEC-0099/REV-0005BE`; exact rail source/load switches are I3 |
...
- [M5Stack Cap CC1101 U219 current WIP reference](https://docs.m5stack.com/en/cap/Cap_CC1101)
- [Infineon BGS13SN8 exact truth table and contacts](https://www.infineon.com/dgdl/Infineon-BGS13SN8-DataSheet-v02_04-EN.pdf?fileId=5546d462584d1d4a0158cf52e3ae03a7)
- [TTM B0310J50100AHF balun](https://cdn.ttm.com/repository/products/wireless-xinger/balun-transformers/B0310J50100AHF/B0310J50100AHF.pdf)
...
- [Hirose DM3AT-SF-PEJM5 exact active socket](https://www.hirose.com/en/product/p/CL0609-0031-0-00)
...
- [TI SN74LVC1G125 partial-power buffer](https://www.ti.com/lit/ds/symlink/sn74lvc1g125.pdf)
- [TI TPD4E05U06 low-capacitance ESD array](https://www.ti.com/lit/ds/symlink/tpd4e05u06.pdf)
