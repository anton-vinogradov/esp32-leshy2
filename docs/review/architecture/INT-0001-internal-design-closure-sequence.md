...
| `I3` | battery, charging, power path, rails, load switches, monitoring and thermal | `I1/I2`, `PWR-0001`, scenario ledger | **active; battery behavior and USB-PD frontend reviewed** by `DEC-0062/0063` and `REV-0005Q/R`: two replaceable 18650 cells behind fail-closed admission; exact sink-only 30-W TPS25751D/BQ25798 path accepted; `PWR-0005/FND-0075` review manager options and open `IMP-0054`; rails/passives/calculation/HIL open | complete rail tree and sequencing with exact first targets, loss/thermal/fault budget |
...
target. `IMP-0052/B` is accepted as `DEC-0062`: the two 18650 cells are
individually replaceable, but reverse insertion, mismatch, removal and contact
bounce must fail closed before charge/discharge admission. `IMP-0053/B` is now
...
S3 USB2 and leave GPIO47 free. I3 next closes the per-cell manager and complete
rail/loss/thermal/fault tree. `PWR-0005/FND-0075` prove that the loose-cell
admission function cannot be delegated to a conventional pack gauge or to S3
after startup. `IMP-0054` is therefore the current owner gate, with exact
MAX17320G20+T plus always-on MSPM0C1104SDGS20R recommended before the common
FET/fuse/NTC/shunt/diagnostic network is calculated.
