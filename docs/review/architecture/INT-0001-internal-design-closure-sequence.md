...
| `I8` | consolidated BOM evidence, lifecycle, availability, cost and alternates | `I1…I7` | **inventory coverage reviewed, qualification active** by `FND-0109/BOM-0008`: 816 instantiated placements / 187 used lines plus five explicit uninstantiated physical families; 34 source, every cost and every alternate disposition remain open | every base function maps to exact first target plus equivalence/alternate and sourcing gate |
...
machine map contains 816 placements across 187 used MPN lines, but
the pass also proves that this is not yet the complete supplied-product BOM:
SMA/cable/M5 bodies, threshold networks and antenna variants remain abstract
or qualification-gated. The narrow group of MAX17320 support parts that had
remained prose-only is now exact and reviewed. I8 inventory
