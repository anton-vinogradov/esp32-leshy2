...
Release does not re-arm. A physical re-arm action starts a fresh TX-off boot. Actual-TX evidence and the physical STOP-latched indicator do not derive solely from software command state. The supervised battery tree retains separate `3V3_LOGIC/RF`, `5V_EXT`, `VVOICE≈4.0 V`, audio and switched accessory domains; `DEC-0064/PWR-0006` compared its 2S versus controlled-1S input and `DEC-0065` confirmed supervised 2S for the base product. Exact converters/current limits remain candidate-package rows rather than assumed legacy circuits.
