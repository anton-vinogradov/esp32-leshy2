...
`DC2337J5010AHF`. Красный индикатор возле антенны сохранён для S3, C5, всех
трёх nRF24, CC1101 и voice. Подтверждение показывает факт и относительный
уровень передачи, но само никогда не разрешает TX.
...
Локальная панель содержит D-pad, `OK`, `BACK`, `OPT`, `F1`, `F2`, энкодер,
`PTT`, аппаратный `STOP` и `RE‑ARM`. Телефон может использоваться для редкого
...
- Отдельный M5 Unit port предоставляет защищённую, отключаемую 5-В ветвь и
  две изолированные сигнальные линии для квалифицированных GNSS, LoRa, NFC,
  iButton/1‑Wire и других модулей.
...
Красный индикатор возле разъёма показывает фактическую передачу именно этого
тракта. Он предусмотрен для S3, C5, трёх nRF24, CC1101 и voice; два входа
Si4732 работают только на приём. У IR есть отдельный оптический индикатор TX.
