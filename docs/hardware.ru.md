...
| Аппаратный STOP | `Panasonic AEQ10410` | Отдельный нормально-замкнутый safety-тракт |
...
вокруг заднего аккумулятора; энкодер находится над F1/F2. `STOP` — единственное
функциональное исключение: его близкая по размеру пользовательская зона
воздействует на отдельный нормально-замкнутый safety-механизм, поэтому обрыв
провода также считается остановом. Телефон может использоваться для редкого
длинного ввода текста, но не подтверждает опасные действия.
...
объём. Пунктир на задней стороне остаётся только для толкателя `STOP`, защитного
углубления `RE-ARM` и ручки энкодера; общей крышки у них нет.
...
На полном виде сверху задней стороны показаны точные корпуса всех органов
управления. Сплошные F1/F2/PTT/RE-ARM — сами непосредственно нажимаемые кнопки;
пунктирными остаются только толкатель STOP, защитное углубление RE-ARM и ручка
энкодера, для которых ещё проверяются глубина, усилие и герметизация.

![Размерная внешняя компоновка](images/current-clamshell.svg?layout=8)

![Полный размерный вид сверху на заднюю сторону](images/u214-dock-top-view.svg?layout=3)

![Размерная компоновка внутренних плат](images/internal-board-layout.svg?layout=5)

![Размерный поперечный разрез бутерброда](images/sandwich-section.svg?layout=7)
