...
высота из документа производителя. Генератор зеркально переводит RF/power-плату в
общую физическую систему координат UI-платы и проверяет все 17 несопрягаемых пар,
чьи XY-проекции пересекаются через точный 11-мм канал. Текущая ограничивающая
пара — разъём наушников напротив VHF/UHF-радио — сохраняет номинальный Z-зазор
2,85 мм при принудительном минимуме 0,7 мм. Совмещённые вилка и розетка FX8C
проверяются отдельно как единственная намеренно сопрягаемая пара. Оба точных
30-мм маршрута штатных RF-коаксиалов проверяются относительно корпусов на своей
стороне, крепёжных зон и всех встречных корпусов; их текущие коридоры диаметром
1,13 мм не пересекают ни одной встречной XY-проекции. Пассивы и производственные
допуски не входят в эту архитектурную проекцию и должны быть закрыты в ECAD до
изготовления.
...
![Размерная внешняя компоновка](images/current-clamshell.svg?layout=13)

![Размерная компоновка внутренних плат](images/internal-board-layout.svg?layout=10)
