# Source, charge, discharge and steady losses · historical R1
...
- Maximum SYS demand: `17.842 W`.
- Maximum series-pack current at 6.0 V: `2.974 A`; reserve to the 10-A cell contract is `236.247%`.
- Maximum steady rail-conversion loss: `2.676 W`; eFuse loss: `0.471 W`.
- Failed states: `0`; unresolved numeric inputs: `0`.

## Available-power control

5 V × 3 A is not treated as a universal source: `14` USB-only combinations are explicitly refused until load is reduced, a healthy pack is installed, or a higher PDO is selected. 9 V × 3 A and 15 V × 2 A run every declared profile. Charge is DPM-derated before system load in `274` combinations. Unknown 5-V fallback without a pack remains AON diagnostics only.
...
**Status:** `H3.1.3` is complete and reviewed; the historical R1 progression marker is `H3.6.1`.
