...
`H3.5` проведён ревью: три leaf-пакета дают `128` проходящих checks, сведение добавляет `22` сквозных checks. Незакрытых аналитических findings нет. Точный текущий маркер — `H3.6.1`.
