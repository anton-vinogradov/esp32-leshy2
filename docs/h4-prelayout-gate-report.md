...
The current project position is `H1-R2.33`: the exact `H1-R2.31` dual-RP GPIO/M1 and C5 SDIO/USB mux baseline plus the complete current Cap/evidence body register are closed. The new R2 H2 remains closed until the physical H1 blockers and all production gates close. The former transition to `H5.0.1-R1` was cancelled by the architecture change.
