...
Исправление микрофона ниже включено в обновлённые виды, привязанные к хешам;
свежие нативные проверки положений/надписей и визуальное ревью выполнены.

| Передняя/UI-плата | Задняя/RF-плата |
| --- | --- |
| **Наружная F — 30 позиций + 4 крепёжных footprint** | **Наружная F — 12 позиций + 4 крепёжных footprint** |
...
| **Внутренняя B — 400 позиций** | **Внутренняя B — 768 позиций** |
...
[Значения подписей и покрытие интерфейсов](h6-r2-interface-silkscreen.ru.md):
пользовательские, сервисные и монтажные надписи проверяются отдельно от Fab.
...
Показаны все **1210 позиций** текущего нативного инвентаря и 8 крепёжных
...
На внутренней стороне UI также показан отдельный корпус динамика с пометкой
`SPEAKER / ASSEMBLY`; это не дополнительный footprint. RF `LS1` — место
подключения двух проводов, а не корпус динамика. Максимальные габариты и
границы проверки монтажного пространства описаны в [исправлениях расстановки](h6-r2-placement-repair.ru.md).

MK1 должен быть на **внутренней RF B0 [47,147.4]**, не на наружном виде;
прежнее **F180 [8,112]** было ошибкой и отменено. **UI F `MIC` [33,148.9]**
обозначает нижний доступ к этому микрофону RF, не footprint UI. Верхний порт
капсюля смотрит в межплатное пространство, с доступом звука через предусмотренный
открытый нижний зазор, не нормалью вниз. [Дополнение по семи обозначениям](../hardware/layout/h6-r2-microphone-bottom-candidate.json)
не меняет MPN, сети или медь. В капсюле есть внутренний FET с нагрузкой 2.2 кОм;
шумовые характеристики, фактическая разводка и передача звука ещё не подтверждены.
...
теперь обновляет разводку, эти пять SVG с компонентами и
[текущий внешний обзор](images/h6-r2-product-exterior.svg), а также проверяет
независимые [требования к расстановке](../hardware/layout/generated/H6-R2-placement-intent.json):
