...
Current focus: [automatic routing and design acceptance](h6-r2-routing-automation.md). The 474-connection geometry milestone and bounded indicator-generation pilot passed their scopes; electrical qualification remains open. The MAIN command generates 18 complete in-memory topology variants and now checks their exact fault-bus receivers, supply ownership and conditional loaded-level requirements. A zero-current diagnostic exposes incompatible MAIN/AON voltage-envelope corners; actual bus currents and GPIO modes remain unknown, so this is not a measured hardware failure. Independent ngspice checks the equations; two fresh complete command replays agree in about 10.2 seconds with no model calls. Next: establish real AON and auxiliary-current bounds, loaded fault-bus behavior, startup and dynamic protection before native reconstruction. Existing power failures remain visible; no candidate MPN is adopted. Production routing stays paused. This is an internal H6.0.3 step, not H6 closure or fabrication authorization.
