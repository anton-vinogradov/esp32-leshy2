...
| H3 · Virtual electrical verification | Reviewed historical baseline `H3-R2.7`; **current `review_required`** ([native power](h6-r2-electrical-semantics.md)) | correct and bind MAIN/AON models to the fitted circuit, then repeat dependent analytical comparisons; retained calculations do not yet qualify current power; physical residuals remain separately owned | [Current H3](h3-r2-acceptance.md) · [Rail margins](power-rail-margins.md) |
