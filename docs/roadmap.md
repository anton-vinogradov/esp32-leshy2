...
| Electrical and transient evidence | ▶️ H3.6.1: RF reviewed; worst-case thermal model in progress |
...
**Exact marker: `H3.6.1`** — [RF verification is consolidated](rf-verification-result.md)
with 125 leaf plus 22 cross-domain checks and no unresolved analytical finding;
the worst-case board, battery and enclosure thermal model is being built.
...
| **H3. Virtual electrical verification** | ▶️ Current · `H3.6.1` | Calculations and simulations before expensive physical work | Worst-case DC budget; startup/shutdown, USB↔battery handover, brownout, watchdog, eFuse and load steps; thermal/fault tree; all analog corners; timing/levels; RF corridors, returns and pre-layout constraints pass |
