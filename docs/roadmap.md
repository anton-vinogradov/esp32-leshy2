...
| H3 · Virtual electrical verification | Reviewed historical baseline `H3-R2.7`; **current `review_required`** ([native power](h6-r2-electrical-semantics.md)) | fitted MAIN arithmetic and dependent comparisons corrected; resolve exposed current/voltage shortfalls, MAIN thermal applicability and AON model before current-power qualification; physical residuals remain separately owned | [Current H3](h3-r2-acceptance.md) · [Rail margins](power-rail-margins.md) |
...
Current focus: [automatic routing and design acceptance](h6-r2-routing-automation.md). The 474-connection geometry milestone and bounded indicator-generation pilot passed their scopes; electrical qualification remains open. The MAIN command generates 18 complete in-memory topology variants, checks their fault-bus receivers and now compares AON delivery on all 56 current load profiles using the existing H3 calculation. Conditional delivery projections avoid the earlier zero-current voltage conflict, but fitted-condition eFuse resistance and branch currents are not proved; the original requirement-envelope warning remains. Independent ngspice checks the fault-line equations, not physical IC behavior. Repeat commands need no model calls. Next: bind actual AON/auxiliary-current limits and GPIO modes, then check loaded fault levels, startup and dynamic protection before native reconstruction. Existing power failures remain visible; no candidate MPN is adopted. Production routing stays paused. This is an internal H6.0.3 step, not H6 closure or fabrication authorization.
