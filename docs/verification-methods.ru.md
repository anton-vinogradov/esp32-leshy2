...
H3 использует воспроизводимые worst-case расчёты, а не оптимистичные typical-значения. Каждый результат показывает входные источники, проверенные состояния, худший corner, численный запас и физическую проверку, которая всё ещё нужна после изготовления.

## Методы

| Область | Метод | Что запрещено подменять |
|---|---|---|
...
**Статус:** `H3.0.3` завершено и проверено. Текущий точный маркер — `H3.3.4`, battery sensing, thermistors и analog fault thresholds.
