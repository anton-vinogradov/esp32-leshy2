# H1-R2.25 · component cost ranking
...
- Volume material basis: **$235.18** per device; `198/210` lines are priced.
- Reachable planning subtotal: **$284.46** per device, with `5` base-product lines still unpriced.
- With the required post-PCBA K331: **$314.45** per device or **$1,572.24** for five devices before PCB/PCBA, enclosure, antennas, freight, tax, yield and test.
- Partial five-device JLCPCB capture: **$1,258.32** for `178` matched lines; `12` live checks move it to **$1,317.24**, with `32` rows excluded.
...
- The `34` pre-order rows cost **$726.04** in the capture versus **$336.74** on their volume material basis.
- The observed small-lot premium is **$389.31**. This is the first priority: seek stocked JLCPCB MPNs that remain inside the existing substitution envelopes.
...
1. ✅ **Replace safe equivalent pre-order passives and ordinary logic with in-stock JLCPCB parts** — After three safe replacement batches, 34 pre-order rows cost USD 726.0450 in the normalized five-device evidence versus USD 336.7395 on their quantity-100 material basis. The stocked Nexperia buffer, YAGEO bypass capacitor and six UNI-ROYAL resistor identities together remove approximately USD 98.2581 from the observed trial route and reduce the public material basis by USD 1.5404 per device. Review every pre-order row against its substitution class; only exact or no-worse parametric replacements may be accepted.
...
> Marker: **H1-R2.25**. H1 remains open pending the complete mock-up decision.
