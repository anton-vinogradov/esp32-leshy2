...
**Статус:** ✅ проведено ревью **`H1-R2.39`**; 80-мм геометрия перепроверена 2026-09-05.

- [Отчёт по завершённой фазе H1](h1-r2-acceptance.ru.md)
- [Текущий физический дизайн](h1-r2-physical-layout.ru.md)
- [Внешние стороны](images/h1-r2-external-layout.svg?rev=h1-r2.39-80mm-1)
- [Внутренняя сторона передней платы](images/h1-r2-inner-ui.svg)
- [Внутренняя сторона задней платы](images/h1-r2-inner-rf.svg)
- [PSA дисплея, ненатянутый путь FPC и прямой ZIF](images/display-mount.svg?rev=h1-r2.39-80mm-1)
- [Настоящие разрезы бутерброда](images/h1-r2-inner-sections.svg?rev=h1-r2.39-80mm-1)
- [Внешний сервисный доступ](images/h1-r2-service-access.svg?rev=h1-r2.39-80mm-1)
...
exact ledger symbols/contacts/footprints и native-проект с 4 306 pins прошли
ревью. Два native-проекта KiCad проходят ERC без замечаний; сверка sheets и
HW↔FW проходит. На момент закрытия H2 placement/routing не начинались;
текущий статус H6 записан ниже.
...
4. ✅ `H2-R2.1.1`: проведено ревью 2 проектов, 22 sheets, 6 владельцев доменов
   и 251 точной component-group;
5. ✅ `H2-R2.1.2`: 245 board groups, шесть явных non-PCBA groups и 1 617
   логических контактов отображены без незакрытых групп;
6. ✅ Definitions/instances `H2-R2.1.3`: 245 controlled symbols, 1 571 symbol
   pins и все 1 208 устанавливаемых экземпляров проходят раскладку по двум проектам;
7. ✅ Nets `H2-R2.1.3`: 4 302 контакта устанавливаемых экземпляров сведены в
   789 глобальных канонических / 823 локальных для плат nets либо 238 явных board no-connects; незакрытых endpoints нет;
8. ✅ `H2-R2.1.3`: два native-проекта KiCad материализуют 4 306 физических pins
   и проходят ERC без ошибок и предупреждений;
9. ✅ `H2-R2.1.4`: шесть доменов, 173 строки контроллеров, 34 межпроектные и
...
точного происхождения параметров/моделей 251 группы и 1 208 устанавливаемых
позиций. [`H3-R2.0.3`](verification-methods.ru.md) фиксирует девять методов и
двенадцать pass/fail rules для всех 251 группы. [`H3-R2.1.1`](power-state-register.ru.md)
проводит ревью всех 2 266 разрешённых source, charge, fault и operating states.
[`H3-R2.1.2`](power-load-binding.ru.md) проводит ревью явной привязки 623
...
**Статус:** ✅ актуальные R2-маршруты проведены ревью как **`H5-R2.1`**.

[Актуальный двуязычный итог H5-R2](h5-r2-current-route.ru.md) перепроверяет все
249 закупаемых групп / 1 216 изделий без неназначенных маршрутов. Сохраняются
[историческое evidence H5-R1](h5-r1-acceptance.ru.md), owner final assembly и
точные 11-мм проходные упоры `Ettinger 007.02.611`. Остаётся один order-time
gate: обнулённый на складе `WBC16-1TLC` должен получить явно подтверждённый
маршрут JLCPCB Global Sourcing либо отдельно квалифицированную и принятую
замену; молчаливая подстановка запрещена. Коммерческих действий не создавалось.

H6 владеет зависящей от корпуса длиной винтов и quoteable Gerber/BOM/CPL.
Финальные условия `SA818S-V`, sourcing `WBC16-1TLC` и полный live stock/price
recheck остаются gates прямо перед заказом.

<a id="h6"></a>
## H6 · KiCad placement, routing и release candidate

**Статус:** ▶️ сейчас **`H6.0.3-R1`**.

[Текущий результат размещения точных footprints](h6-r2-exact-placement.ru.md)
уже содержит две нативные шестислойные платы KiCad. Материализованы все
1 208/1 208 устанавливаемых экземпляров и все 789 глобальных канонических / 823 локальных для плат сети; машинный
аудит показывает ноль жёстких courtyard-коллизий, ноль потерянных корпусов и
ноль ошибок отображения. Генерация совпадает байт-в-байт, KiCad 10 разбирает
обе платы. [Точный результат механического стека](h6-r2-mechanical-stack.ru.md)
фиксирует 20-мм нейлоновые винты M2.5,
захваченные гайки, 11-мм упоры, опоры стенок, четыре плечика и независимые
краевые захваты; полный угол допусков сохраняет 2,18 мм резьбы в гайке и прячет
кончик винта без несущей роли M1. [Результат сервиса microcoax](h6-r2-microcoax-service.ru.md)
добавляет пять ненатянутых коридоров, пять свободных седел из ленты и десять
окон осмотра антенных паек при минимальном запасе кабеля 7,69 мм. H6.0.1
завершён; H6.0.3 переводит обе платы на 80 × 150 мм и выделяет x=35…40 мм под
настоящий внутренний коридор трассировки. Все 1 208 позиций остаются точными,
жёстких коллизий placement нет. [Актуальный срез разводки](h6-r2-current-routing.ru.md)
связывает 5 373 чистых по DRC элементов дорожек/via с текущими hash плат:
719 физических соединений замкнуто, 2 546 осталось. Класс UI
`ANALOG_AUDIO_SENSE` завершён; после 111 новых чистых по DRC соединений на
RF/power в нём осталось только 16. Срез также включает полный путь
`VOICE_EFUSE_BACKUP_EN_N` через новый коридор обычными дорожками 0,15 мм и
via 0,4/0,2 мм. Свежий DRC KiCad
совпадает с чистой 80-мм базой: ноль замечаний UI и только два уже назначенных
исключения держателя аккумуляторов на RF/power. Числа разводки H6.0.2 остаются
историческим evidence; сейчас выполняются повторная квалификация и завершение трассировки.

### Живой вид 80-мм базы разводки

Это прямые SVG-экспорты из сохранённых `.kicad_pcb`, а не иллюстративные
мокапы. Синий — лицевая медь, красный — обратная; исходный hash платы записан
в каждый файл, поэтому проверка H6 отклоняет устаревшую картинку после любого
изменения PCB. Изображения можно открыть отдельно в полном размере; более
подробное объяснение слоёв и команда воспроизводимого экспорта находятся в
[актуальном срезе разводки H6.0.3](h6-r2-current-routing.ru.md#живые-изображения).

**Передняя UI-плата**

[![Текущая реальная разводка передней UI-платы](images/h6-r2-routing-ui.svg)](images/h6-r2-routing-ui.svg)

**Задняя RF/power-плата**

[![Текущая реальная разводка задней RF/power-платы](images/h6-r2-routing-rf.svg)](images/h6-r2-routing-rf.svg)
