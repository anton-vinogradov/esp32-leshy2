...
заднего RP и одиннадцать контактов M1 остаются резервами; скрытой ручной пайки нет.
...
exact ledger symbols/contacts/footprints и сверка 4 323 endpoints nets прошли
...
4. ✅ `H2-R2.1.1`: проведено ревью 3 проектов, 23 sheets, 6 владельцев доменов
   и 242 точные MPN-группы;
5. ✅ `H2-R2.1.2`: 237 board groups, пять явных non-PCBA groups и 1 662
   логических контактов отображены без незакрытых групп;
6. ✅ Definitions/instances `H2-R2.1.3`: 237 controlled symbols, 1 618 PCB-pad
   pins и все 1 187 устанавливаемых экземпляров проходят раскладку по трём проектам;
7. ✅ Nets `H2-R2.1.3`: 4 323 контакта устанавливаемых экземпляров сведены в
   827 канонических nets либо 260 явных board no-connects; незакрытых endpoints нет;
8. ✅ `H2-R2.1.3`: три native-проекта KiCad материализуют 4 327 физических pins
   и проходят ERC без ошибок и предупреждений;
9. ✅ `H2-R2.1.4`: шесть доменов, 173 строки контроллеров, 50 межпроектных и
...
**Статус:** ▶️ сейчас **`H3-R2.2.1`** внутри H3-R2.2.
...
точного происхождения параметров/моделей 242 групп и 1 187 устанавливаемых
позиций. [`H3-R2.0.3`](verification-methods.ru.md) фиксирует девять методов и
двенадцать pass/fail rules для всех 242 групп. [`H3-R2.1.1`](power-state-register.ru.md)
проводит ревью всех 2 266 разрешённых source, charge, fault и operating states.
[`H3-R2.1.2`](power-load-binding.ru.md) проводит ревью явной привязки 612
...
[Cross-check H3-R2.1](power-dc-source-result.ru.md) сводит все 618 нагрузок,
224 rail-профиля и 2 266 состояний в 15 проходящих проверках, поэтому H3-R2.1
проведён ревью. Текущий H3-R2.2.1 проверяет упорядоченные startup, shutdown,
reset и recovery. Ожидаемый результат — полная проверка power, digital, RF,
