...
**Статус:** ▶️ сейчас `H5.0.3`; контрольный BOM Tool прогон завершён, 33 явных
outlier проходят локальную квалификацию; закупка не разрешена.
...
из 209 и распознал все 1019 установок; 33 явных outlier держат H5.0.3 открытым.
Семантических подмен MPN не обнаружено. Замены, quote/reservation и закупка не
разрешены. Это не production order.
