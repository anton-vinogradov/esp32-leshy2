...
**Статус:** ▶️ сейчас `H5.0.3`; минимальный BOM upload разрешён и подготовлен,
но ожидает пользовательского входа; закупка не разрешена.
...
209 с `J0`–`J4`; оставшиеся 199 строк держат H5.0.3 открытым. Минимальный upload
ещё не передан; замены и закупка не разрешены. Это не production order.
