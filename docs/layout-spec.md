...
Distinct from the drawing mirror of §3. When the two boards close inner-face-to-inner-face (top stays up), the C5 board seats **X-mirrored** relative to the main board — a C5 part at part-x sits at **device-x = 75 − part-x** in the assembled device. So a component on the C5's **right** side edge ends up on the device's **left** side edge, sharing that case slot (and the ~11 mm gap band) with whatever the main board puts on its own left edge. This is why edge connectors on the two boards can collide even though each passes its own per-face checks — **check 6** (§8) catches it, and it **binds the physical PCB layout**: on the real boards, place the two boards' side-edge exits so they do not fold onto the same slot. (Bottom-edge exits sit ~11 mm apart in depth on the ~36 mm-deep bottom face, so they may share an X position.)
...
| **Mezzanine connector** (SMD board-to-board, in the gap, no outer-face protrusion) | ~ | mez | both · I | interior-lower, centred | — |
| **4× M2.5 mounting holes** | ⌀ 2.7 | hole | both | four corners | — |

*The internal-component rows above (MCUs, radios, ICs, decoders, power, u.FL, BT1) are **not placed individually** — they live inside the **component area** on their inner face and are positioned during PCB layout (stage 8). Only interfaces, antennas and the mezzanine have fixed positions. Rules of thumb: modules dominate the inner faces; the display and the 2× 18650 dominate the outer faces; the nRF height drives the mezzanine gap.*
