...
Открыто: `startup current exceeds minimum hardware limit on 3V3_MAIN`; `load-step endpoint does not pass on 3V3_MAIN`; `inrush`; `load_steps`; `numerical:existing_checks`; `applicability:inrush_inputs`; `applicability:sequence_inputs`; `applicability:handover_inputs`.
...
Численных нарушений в сохранённом сводном сравнении — 4, путей автоматического перезапуска в модели — 0. Нулевой численный счётчик не устраняет открытые вопросы применимости питания.
