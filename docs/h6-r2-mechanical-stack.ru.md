...
**Статус:** ✅ локальная геометрия винта, упора, опор корпуса и независимых захватов PCB зафиксирована и проверена машинно. [Пять сервисных петель microcoax](h6-r2-microcoax-service.ru.md) теперь закрывают H6.0.1; **сейчас выполняется трассировка H6.0.2.** Закупка и производство не разрешены.
...
[Результат сервиса microcoax](h6-r2-microcoax-service.ru.md) заменяет старые иллюстративные линии кабелей пятью точными H6-коридорами и позициями седел из ленты. Он доказывает ненатянутую длину, доступ к коннекторам, карман display/FPC и 2D-зазор механических keepout. H6.0.1 закрыт; H6.0.2 теперь разводит платы, а собранный STEP повторяет точную проверку встречных тел в H6.0.6.
