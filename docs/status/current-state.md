...
The 3×nRF24 prerequisite audit is **Reviewed** under `REV-0002T`; draft [`REQ-N24-0001`](../review/requirements/REQ-N24-0001-three-nrf24-raw-2g4.md) is **In review**. Current hardware still routes the three generic module placeholders through S3, has no real `TXDET_NRF1..3` detector circuits, and has no proved post-dual-IR C5 pin budget. Source labels and the shared-CE reset default were corrected, but `FND-0019` remains an implementation blocker. RPD is now specified as calibrated binary hit-rate statistics, never RSSI/dBm/bearing; the constant-carrier mode is only an external-instrument source, not a VSWR meter (`FND-0020`). Passive ESB/MouseJack discovery, sensitive KeySniffer capture, active injection, address brute-force, and interference tests have distinct gates; interference/carrier sweeps are conducted or RF-shielded `BOTH` only, and GPL references are not treated as MIT code (`FND-0021`). **⚠️ Proposal [`IMP-0016`](../review/improvements/IMP-0016-calibrated-three-antenna-2g4-hunt.md)** recommends a no-new-BOM comparative RPD hit-rate baseline; adding real-power RF hardware is the alternative. **⚠️ Proposal [`IMP-0017`](../review/improvements/IMP-0017-native-ble-plus-nrf24-compatibility.md)** is deferred to BLE-owner review.
