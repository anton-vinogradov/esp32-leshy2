...
All candidates pass paper memory and admitted-throughput arithmetic; `SYN-2A` retains the largest S3 contention risk, `SYN-2B` the largest single-core C5 latency risk, and `SYN-3A` the additional signed firmware target. The next active work is power/RF qualification and a dated comparable cost snapshot. Target READMEs remain unchanged until atomic acceptance under `DEC-0026`.
