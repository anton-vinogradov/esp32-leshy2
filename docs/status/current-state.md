...
All prerequisite models for the atomic package are now reviewed. The next active artifact combines capability coverage, exact pins, memory/traffic, power, RF, recurring cost, update/recovery and sourcing into one architecture decision. Target READMEs remain unchanged until that package is accepted under `DEC-0026`.
