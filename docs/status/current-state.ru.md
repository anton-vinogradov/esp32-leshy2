...
- `FND-0025`: raw/full-stack 802.15.4 технически шире legacy passive-only, но ordinary Thread/Zigbee scope и shared-radio coexistence ждут решения `IMP-0018`.
...
Prerequisite audit 3×nRF24 получил статус **«Проведено ревью»** в `REV-0002T`, а финальное распространение в `REV-0002U` дало [`REQ-N24-0001`](../review/requirements/REQ-N24-0001-three-nrf24-raw-2g4.md) статус **«Проведено ревью»**. Владелец принял `IMP-0016/A` как [`DEC-0019`](../review/decisions/DEC-0019-calibrated-rpd-three-antenna-hunt.md): три одновременных тракта дают calibrated binary RPD hit-rate sector comparison с sampling/calibration metadata, но никогда не RSSI/dBm/bearing/VSWR; real-power hardware остаётся отдельным будущим решением. `FND-0020` закрыт на requirement-level. Текущее железо всё ещё проводит три generic module placeholder через S3, не имеет реальных `TXDET_NRF1..3` detector circuits и не доказывает post-dual-IR C5 pin budget, поэтому `FND-0019` открыт. Passive ESB/MouseJack discovery, sensitive KeySniffer capture, active injection, address brute-force и interference tests имеют разные gates; interference/carrier sweep допустимы только conducted/RF-shielded `BOTH`, а GPL references не считаются MIT code, поэтому `FND-0021` остаётся до implementation proof. **⚠️ Предложение [`IMP-0017`](../review/improvements/IMP-0017-native-ble-plus-nrf24-compatibility.md)** отложено до BLE-owner review.

C5 Wi-Fi/IEEE 802.15.4 prerequisite audit получил статус **«Проведено ревью пререквизитов»** в [`REV-0002V`](../review/reviews/REV-0002V-c5-wifi-802154-prerequisites.md). Исправлен source candidate N8R4→N8R8 и ANT2→штатный off-board ANT1 path, но final RF artifact остаётся открытым (`FND-0022`). Public baseline разделён с patched binary: promiscuous RX доступен, raw TX ограничен beacon/probe/action и non-QoS data, deauth/disassoc не обещаны; `AUTO` не simultaneous dual-band (`FND-0023`). DFS/country/PMF/privacy gates обязательны, DFS SoftAP исключён (`FND-0024`). Raw 802.15.4, OpenThread и Zigbee roles доступны без нового RF BOM, но делят один 2.4 GHz path; Zigbee core proprietary prebuilt (`FND-0025`). [`REQ-W5-0001`](../review/requirements/REQ-W5-0001-c5-wifi-ieee802154.md) остаётся **«На ревью»**. **⚠️ Предложение [`IMP-0018`](../review/improvements/IMP-0018-open-first-thread-and-zigbee-scope.md)** просит выбрать open-first Thread + conditional Zigbee, open-only Thread/raw или passive-only. Target README не изменён до решения владельца.
