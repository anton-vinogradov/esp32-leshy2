...
Все prerequisite models атомарного package теперь проверены. Следующий активный артефакт объединяет capability coverage, exact pins, memory/traffic, power, RF, recurring cost, update/recovery и sourcing в одно архитектурное решение. Целевые README не меняются до его принятия по `DEC-0026`.
