...
Генератор учитывает **133** установленных конденсаторов по текущему R2-реестру соединений и применяет указанные допуски ёмкости. Сохранены отдельные расчёты пяти защищённых выходов. Для MAIN, voice и внешних 5 В используется номинальная модель скорости нарастания с минимальной управляющей ёмкостью; AON рассматривается как ограниченный током запуск.
...
|---|---:|---:|---:|---:|---|
| `AON_SAFE_3V3` | 42.820000 | 72.100 | 92.900 | 0.000 | пройдено |
