...
нагрузки. H6.0.1 теперь остаётся открытым только для пяти ненатянутых коридоров
сервисных петель microcoax, позиций фиксаторов и зазоров корпуса/окон осмотра.
Только после закрытия этих физических ограничений H6.0.2 начинает трассировку
и принимает DRC без findings с паритетом схема↔PCB.
