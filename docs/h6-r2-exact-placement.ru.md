...
коллизий. **H6.0.1 и вся H6 остаются в работе.** Трассировка ещё не начиналась;
результат не разрешает изготовление или закупку.
...
- все **823** канонические сети H2 назначены реальным контактам footprints;
- на каждой плате есть четыре оси M2.5-упоров, скруглённый контур 75 × 150 мм,
...
- генератор, машинный контракт и аудит с хешами воспроизводимы.
...
ошибок отображения net/footprint. Повторная генерация совпадает байт-в-байт.
KiCad 10 разбирает обе платы и экспортирует из них placement-файлы.
...
## Кандидат stackup и остаток H6.0.1

Платы имеют шесть медных слоёв и толщину 1,6 мм. [Шестислойный кандидат JLC3313](https://jlcpcb.com/impedance)
пока только placement-time вариант: H6.0.4/H6.0.5 обязаны связать реальные
ширины дорожек, диэлектрики и значения калькулятора заказа перед выпуском.

В H6.0.1 ещё надо точно задать захваты корпуса, anti-shear datums, опорные стенки,
свободные сервисные петли microcoax и окончательную геометрию нейлоновых винтов
и фиксаторов. Лишь после закрытия этих ограничений H6.0.2 начнёт трассировку и
будет отвечать за DRC без findings и паритет схема↔PCB.
