# Проверка единичных отказов · historical R1

`H3.6.2` проведён ревью: 30 сценариев и 25 машинных checks проходят. Точный маркер — `H3.6.3`.

Для каждого отказа указаны обнаружение, основной путь отключения, независимый или fail-safe путь, безопасный результат и восстановление. Проверка fault-plane на границе KILL → RUN прерывает работу, но не повреждает железо. Автоматического повторного запуска нет.

| Область | Единичный отказ | Результат |
|---|---|---|
