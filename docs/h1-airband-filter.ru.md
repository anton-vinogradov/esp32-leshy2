...
Это не BOM фильтра: строки доказывают, что нужные классы точных серийных RF-индуктивностей существуют на фабричной поверхности. Полный набор MPN принимается только после H3.
...
H3 должен подобрать один фиксированный фабричный BOM-state с учётом паразитик платы и допусков. Если полная маска не сойдётся, возвращаемся к точному покупному фильтру или меняем границу приёмника; номинальный результат не будет выдан за готовое решение.
