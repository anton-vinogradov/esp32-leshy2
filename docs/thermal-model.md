...
The model enumerates every H3.1 power state and separates electrical capacity from thermal permission. The electrical anti-hidden-load corner is `VOICE/RX/SUPPORT_WORST` at `14.871 W` of conservative base heat; it is **not** a continuous operating claim. The hottest `SUPPORT_IDLE` group is `VOICE` at `5.509 W`; quiet idle is `1.795 W`. External accessory output is excluded only after its converter/eFuse and base support heat have been retained.
