...
The current H2 net inventory confirms the entire OE fan-out: U59.6 drives U22.6, Q3.G and R91; HUB_HOLD from U59.8 drives only Q2.G2/5, with no external pull resistor. Q3's primary table specifies gate leakage ±100 nA at VGS=±8 V, VDS=0 V, **25 °C**. Substituting that value plus the mux's tabulated 1 µA into the conditional resistor budget gives 35.201 µA, leaving 14.799 µA below LV20A's 50 µA condition. This is a conditional screening calculation: applying those leakage test points to operating voltages, resistor temperature drift and the full temperature range remains unqualified. [DMN2056U electrical table, p.3](https://www.diodes.com/datasheet/download/DMN2056U.pdf).

Reset-low/high qualification must include more than the switch transistor: each reset has a 10 kΩ ±1% MAIN pull-up (C5 R93, Hub R139), controller and U20 inputs, off-state sinks, and an ESD/button/programmer branch through 1 kΩ ±1% (C5 R82, Hub R137). C5 additionally has U56's open-drain output. Every extra 1 µA sinking leakage lowers the released level by up to 10.1 mV from the external pull-up alone; controller internal pulls and fixture loading must also be accounted for. Neither a button path nor a debug connector bypasses KILL.
